"""WinLoop V143 exact continuation: epoch-94 thirty-third-source handoff, sixty-eighth cold restart, and root-36 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V143'
BASE_DIGEST = '5a774e0cd53a4547d916be1d8dfc945be4149fc7bd99085436b2d9891ecfb8fe'
BASE_IMPL_SHA = 'e700555b44e98620ec073a2379c686971d6619cc6677fe28884f91a0ef8367bf'
D = 3
...
def gc94():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*145+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**143) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch93_complete_seed_states': seed,
        'delay_vectors': 4**143,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch94_bound_thirty_third_source_handoff_states': counts[0][1],
        'epoch94_bound_thirty_third_source_binding_states': counts[1][1],
        'epoch94_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication68():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**140) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_sixty_seventh_restart_seed_states': seed,
        'delay_vectors': 4**140,
        'deadline_vectors': z,
        ...
        'bound_sixty_eighth_restart_states': counts[3][1],
        'bound_sixty_eighth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership36():
    ...
    def ok(phase, state, generation=4, carried_root=36, target_root=36,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=35),
        not ok(6,expected[6],target_root=35),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**138) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**138,
        'deadline_vectors': z,
        'bound_root36_witness_rebind_states': counts[0][1],
        'bound_root36_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc94(), publication68(), membership36()
    o = {
        'version': V,
        'base': {
            'version':'V142',
        ...
        'epoch94': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch93_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_third_source_handoff_states':t['epoch94_bound_thirty_third_source_handoff_states'],
            'bound_thirty_third_source_binding_states':t['epoch94_bound_thirty_third_source_binding_states'],
            'bound_verifier_binding_states':t['epoch94_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication68': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_sixty_seventh_restart_seed_states'],
            ...
            'bound_sixty_eighth_restart_states':s['bound_sixty_eighth_restart_states'],
            'bound_sixty_eighth_restart_recoveries':s['bound_sixty_eighth_restart_recoveries'],
        ...
        'membership36': {
            ...
            'bound_root36_witness_rebind_states':b['bound_root36_witness_rebind_states'],
            'bound_root36_witness_binding_states':b['bound_root36_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 95 by rotating the thirty-third-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose sixty-eighth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a sixty-ninth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-36 witness rebind, replace the witness source, roll to root 37, bind root 37, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V143 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-94 GC to {t['accepted']:,} states with {t['epoch94_bound_thirty_third_source_handoff_states']:,} bound thirty-third-source handoffs, "
        f"{t['epoch94_bound_thirty_third_source_binding_states']:,} bound thirty-third-source bindings, and {t['epoch94_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_sixty_eighth_restart_recoveries']:,} fully bound sixty-eighth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root36_witness_rebind_states']:,} bound root-36 witness rebinds, "
        f"{b['bound_root36_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
