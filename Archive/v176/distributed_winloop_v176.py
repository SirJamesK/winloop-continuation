"""WinLoop V176 exact continuation: epoch-127 forty-ninth-lineage rotation, one-hundred-first cold restart, and root-53 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V176'
BASE_DIGEST = 'b3589cf18702887ac108c28ac50ca62e5530367da4141655b686b73d281fe2a1'
BASE_IMPL_SHA = 'ac80ad7bbc8cad5b33a74e38433ff63aae0d201fdad6876cb327ac8c909db39d'
D = 3
...
def gc127():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*244+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**209) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch126_complete_seed_states': seed,
        'delay_vectors': 4**209,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch127_bound_forty_ninth_lineage_rotation_states': counts[0][1],
        'epoch127_bound_forty_ninth_lineage_binding_states': counts[1][1],
        'epoch127_bound_handed_proof_rebind_states': counts[2][1],
        'epoch127_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication101():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**206) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundredth_restart_seed_states': seed,
        'delay_vectors': 4**206,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_first_restart_states': counts[3][1],
        'bound_one_hundred_first_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership53():
    ...
    def ok(phase, state, generation=4, carried_root=52, target_root=53,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=51),
        not ok(10,expected[10],target_root=52),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**204) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root53_rollover_states': counts[2][1],
        'bound_root53_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc127(), publication101(), membership53()
    o = {
        'version': V,
        'base': {
            'version':'V175',
        ...
        'epoch127': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch126_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_ninth_lineage_rotation_states':t['epoch127_bound_forty_ninth_lineage_rotation_states'],
            'bound_forty_ninth_lineage_binding_states':t['epoch127_bound_forty_ninth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch127_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch127_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication101': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundredth_restart_seed_states'],
            ...
            'bound_one_hundred_first_restart_states':s['bound_one_hundred_first_restart_states'],
            'bound_one_hundred_first_restart_recoveries':s['bound_one_hundred_first_restart_recoveries'],
        ...
        'membership53': {
            ...
            'bound_root53_rollover_states':b['bound_root53_rollover_states'],
            'bound_root53_binding_states':b['bound_root53_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 128 by handing the rebound proof to a fiftieth source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-first-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-second verifier cold restart without cached authority promotion',
            'keep generation 4 after root-53 rollover, rebind the witness to root 53, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V176 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-127 GC to {t['accepted']:,} states with {t['epoch127_bound_forty_ninth_lineage_rotation_states']:,} bound forty-ninth-lineage rotations, "
        f"{t['epoch127_bound_forty_ninth_lineage_binding_states']:,} bound lineage bindings, {t['epoch127_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch127_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_one_hundred_first_restart_recoveries']:,} fully bound one-hundred-first-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root53_rollover_states']:,} bound root-53 rollovers, "
        f"{b['bound_root53_binding_states']:,} bound root-53 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
