...
D = 3
BASE_V104_EPOCH55_COMPLETE_STATIC_STATES = 576
...
def _gc56_ok(phase, fourteenth_source_handoff, fourteenth_source_binding,
             ...
             thirteenth_handed_proof_rebind, deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    trust = (
        fourteenth_source_handoff, fourteenth_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V104 completed epoch 55 only after the thirteenth-source lineage was
    # rotated and bound and the handed proof was rebound. Epoch 56 hands that
    # rebound proof to a fourteenth source, binds it, and then permits verifier completion.
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC56_EXPECTED.get(phase) == (
        fourteenth_source_handoff, fourteenth_source_binding, verifier_binding
    )
...
def gc56():
    fixed = (2, 1, *([2] * 46), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC56_EXPECTED.items()]
    assert all(_gc56_ok(*x) for x in tails)
    z = q(67)
    seed = BASE_V104_EPOCH55_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(6, 52)]
    deadline_bad = sum(x[52] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc56_ok(0, 0, 0, 0, *fixed),
        _gc56_ok(2, 2, 0, 0, *fixed),
        _gc56_ok(4, 2, 2, 0, *fixed),
        _gc56_ok(6, 2, 2, 2, *fixed),
        not _gc56_ok(6, 3, 2, 2, *fixed),
        not _gc56_ok(6, 2, 3, 2, *fixed),
        not _gc56_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc56_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** 46) * 2
    patterns = static_patterns * (4 ** 67) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch55_complete_seed_states': seed,
        'delay_vectors': 4 ** 67,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch56_fourteenth_source_handoff_states': handoff * z,
        'epoch56_bound_fourteenth_source_handoff_states': bound_handoff * z,
        'epoch56_fourteenth_source_binding_states': source_binding_count * z,
        'epoch56_bound_fourteenth_source_binding_states': bound_source_binding * z,
        'epoch56_verifier_binding_states': verifier * z,
        'epoch56_bound_verifier_binding_states': bound_verifier * z,
        'epoch56_complete_states': complete * z,
        'unbound_or_conflicting_fourteenth_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_fourteenth_source_binding_acceptances': transition_bad[1] * z,
