...
BASE_V104_BOUND_TWENTY_NINTH_RESTART_STATIC_STATES = 27648
...
def _pub105_ok(phase, successor_disappearance, replacement_binding,
               dual, restart30, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart30,
        consume, quorum
    )):
        return False
    # V104's twenty-ninth restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V105 handles successor disappearance,
    # replacement binding, fresh reconciliation, and restart 30.
    if cache_authority != 0:
        return False
    return _PUB105_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart30,
        consume, quorum
    )


def successor_disappearance_thirtieth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB105_EXPECTED.items()
    ]
    assert all(_pub105_ok(*x) for x in tails)
    z = q(64)
    seed = BASE_V104_BOUND_TWENTY_NINTH_RESTART_STATIC_STATES
    ...
    restart30 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart30 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub105_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub105_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub105_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub105_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub105_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub105_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub105_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub105_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub105_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub105_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub105_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub105_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub105_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_twenty_ninth_restart_seed_states': seed,
        'delay_vectors': 4 ** 64,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'thirtieth_verifier_cold_restart_states': restart30 * z,
        'bound_thirtieth_restart_states': bound_restart30 * z,
        'bound_thirtieth_restart_recoveries': bound_recovery * z,
        'cached_thirtieth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_thirtieth_restart_acceptances': restart_bad * z,
