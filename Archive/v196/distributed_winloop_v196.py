"""WinLoop V196: epoch-147 fifty-ninth-lineage rotation, publication-121 cold restart, and root-63 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V196'
BASE_DIGEST = 'da273f02429d022773cca5ecb8c28ed0ea127a309969e092cdb1190736259617'
BASE_IMPL_SHA = '67b389a983e1e4f5ee9eb8dd03eff4f63d5e8298c25e5da6c84fa2c5ff74af9e'
D = 3
...
def gc147():
    ...
    checks += [
        ...
        not ok(8,expected[8],carried=(2,)*304+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**249) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch146_complete_seed_states': seed,
        'delay_vectors': 4**249,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch147_bound_fifty_ninth_lineage_rotation_states': counts[0][1],
        'epoch147_bound_fifty_ninth_lineage_binding_states': counts[1][1],
        'epoch147_bound_handed_proof_rebind_states': counts[2][1],
        'epoch147_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication121():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**246) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_twentieth_restart_seed_states': seed,
        'delay_vectors': 4**246,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_twenty_first_restart_states': counts[3][1],
        'bound_one_hundred_twenty_first_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership63():
    ...
    def ok(phase, state, generation=4, carried_root=62, target_root=63,
        ...
        return (
            ...
            and (generation, carried_root, target_root, replication, tombstone,
                 witness, prior_source, active_byzantine) == (4,62,63,2,1,2,2,0)
        )
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [
        not ok(10,(3,2,2,2,2)),
        ...
        not ok(10,expected[10],carried_root=61),
        not ok(10,expected[10],target_root=62),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**244) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root63_rollover_states': counts[2][1],
        'bound_root63_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc147(), publication121(), membership63()
    o = {
        'version': V,
        'base': {
            'version':'V195',
        ...
        'epoch147': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch146_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fifty_ninth_lineage_rotation_states':t['epoch147_bound_fifty_ninth_lineage_rotation_states'],
            'bound_fifty_ninth_lineage_binding_states':t['epoch147_bound_fifty_ninth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch147_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch147_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication121': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_twentieth_restart_seed_states'],
            ...
            'bound_one_hundred_twenty_first_restart_states':s['bound_one_hundred_twenty_first_restart_states'],
            'bound_one_hundred_twenty_first_restart_recoveries':s['bound_one_hundred_twenty_first_restart_recoveries'],
        ...
        'membership63': {
            ...
            'bound_root63_rollover_states':b['bound_root63_rollover_states'],
            'bound_root63_binding_states':b['bound_root63_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 148 by handing the rebound proof to a sixtieth source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-twenty-first-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-twenty-second verifier cold restart without cached authority promotion',
            'keep generation 4 after root-63 rollover, rebind the witness to root 63, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V196 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-147 GC to {t['accepted']:,} states with {t['epoch147_bound_fifty_ninth_lineage_rotation_states']:,} bound fifty-ninth-lineage rotations, "
        f"{t['epoch147_bound_fifty_ninth_lineage_binding_states']:,} bound lineage bindings, "
        f"{t['epoch147_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, and "
        f"{t['epoch147_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_twenty_first_restart_recoveries']:,} fully bound one-hundred-twenty-first-cold-restart recoveries; "
        ...
        f"{b['bound_root63_rollover_states']:,} bound root-63 rollovers, {b['bound_root63_binding_states']:,} bound root-63 bindings, "
