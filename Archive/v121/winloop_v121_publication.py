...
BASE_V120_BOUND_FORTY_FIFTH_RESTART_STATIC_STATES = 27648
...
def _pub121_ok(phase, successor_disappearance, replacement_binding,
               dual, restart46, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart46,
        consume, quorum
    )):
        return False
    # V120's forty-fifth restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V121 handles successor disappearance,
    # replacement binding, fresh reconciliation, and restart 46.
    if cache_authority != 0:
        return False
    return _PUB121_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart46,
        consume, quorum
    )


def successor_disappearance_forty_sixth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB121_EXPECTED.items()
    ]
    assert all(_pub121_ok(*x) for x in tails)
    z = q(96)
    seed = BASE_V120_BOUND_FORTY_FIFTH_RESTART_STATIC_STATES
    ...
    restart46 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart46 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub121_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub121_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub121_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub121_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub121_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub121_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub121_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub121_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub121_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub121_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub121_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub121_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub121_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_forty_fifth_restart_seed_states': seed,
        'delay_vectors': 4 ** 96,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'forty_sixth_verifier_cold_restart_states': restart46 * z,
        'bound_forty_sixth_restart_states': bound_restart46 * z,
        'bound_forty_sixth_restart_recoveries': bound_recovery * z,
        'cached_forty_sixth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_forty_sixth_restart_acceptances': restart_bad * z,
