from winloop_v121_core import q

BASE_V120_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem121_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V120 completed the root-25 rollover with replacement/prior-source bindings
    # intact. V121 keeps generation 4 / root 25 fixed, rebinds the witness to
    # root 25, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 25 or target_root != 25:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM121_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root25_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 25, 25, 2, 1, 2, 2, 0)
        for phase, state in _MEM121_EXPECTED.items()
    ]
    assert all(_mem121_ok(*x) for x in tails)
    z = q(94)
    seed = BASE_V120_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 25 for x in tails) * seed
    target_root_regression = sum(x[6] < 25 for x in tails) * seed
    ...
    checks = [
        _mem121_ok(0, 0, 0, 0, 4, 25, 25, 2, 1, 2, 2, 0),
        _mem121_ok(2, 2, 0, 0, 4, 25, 25, 2, 1, 2, 2, 0),
        _mem121_ok(4, 2, 2, 0, 4, 25, 25, 2, 1, 2, 2, 0),
        _mem121_ok(6, 2, 2, 2, 4, 25, 25, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 3, 2, 2, 4, 25, 25, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 3, 2, 4, 25, 25, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 3, 4, 25, 25, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 3, 25, 25, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 24, 25, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 25, 24, 2, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 25, 25, 1, 1, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 25, 25, 2, 2, 2, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 25, 25, 2, 1, 3, 2, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 25, 25, 2, 1, 2, 3, 0),
        not _mem121_ok(6, 2, 2, 2, 4, 25, 25, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 7 * (4 ** 3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 94) * z
    return {
        'patterns': patterns,
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root25_witness_rebind_states': rebind * z,
        'bound_root25_witness_rebind_states': bound_rebind * z,
        'root25_witness_binding_states': witness_binding * z,
        'bound_root25_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root25_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root25_witness_binding_acceptances': witness_bad * z,
