"""WinLoop V142 exact continuation: epoch-93 thirty-second-lineage rotation, sixty-seventh cold restart, and root-36 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V142'
BASE_DIGEST = '76c0966e707da82a1ce441e559e87e70a9f6b5d4e62c94e3496c763f79dd8b72'
BASE_IMPL_SHA = '91bcf4fc858cec4637f5994bba758074b7c9551b15c6b8e74ab8cfc50cfadb25'
D = 3
...
def gc93():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*142+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**141) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch92_complete_seed_states': seed,
        'delay_vectors': 4**141,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch93_bound_thirty_second_lineage_rotation_states': counts[0][1],
        'epoch93_bound_thirty_second_lineage_binding_states': counts[1][1],
        'epoch93_bound_handed_proof_rebind_states': counts[2][1],
        'epoch93_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication67():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**138) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_sixty_sixth_restart_seed_states': seed,
        'delay_vectors': 4**138,
        'deadline_vectors': z,
        ...
        'bound_sixty_seventh_restart_states': counts[3][1],
        'bound_sixty_seventh_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership36():
    ...
    def ok(phase, state, generation=4, carried_root=35, target_root=36,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=34),
        not ok(10,expected[10],target_root=35),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**136) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root36_rollover_states': counts[2][1],
        'bound_root36_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc93(), publication67(), membership36()
    o = {
        'version': V,
        'base': {
            'version':'V141',
        ...
        'epoch93': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch92_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_second_lineage_rotation_states':t['epoch93_bound_thirty_second_lineage_rotation_states'],
            'bound_thirty_second_lineage_binding_states':t['epoch93_bound_thirty_second_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch93_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch93_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication67': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_sixty_sixth_restart_seed_states'],
            ...
            'bound_sixty_seventh_restart_states':s['bound_sixty_seventh_restart_states'],
            'bound_sixty_seventh_restart_recoveries':s['bound_sixty_seventh_restart_recoveries'],
        ...
        'membership36': {
            ...
            'bound_root36_rollover_states':b['bound_root36_rollover_states'],
            'bound_root36_binding_states':b['bound_root36_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 94 by handing the rebound proof to a thirty-third source, binding that source, and preserving the epoch-12 deadline',
            'compose sixty-seventh-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a sixty-eighth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-36 rollover, rebind the witness to root 36, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V142 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-93 GC to {t['accepted']:,} states with {t['epoch93_bound_thirty_second_lineage_rotation_states']:,} bound thirty-second-lineage rotations, "
        f"{t['epoch93_bound_thirty_second_lineage_binding_states']:,} bound lineage bindings, {t['epoch93_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch93_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_sixty_seventh_restart_recoveries']:,} fully bound sixty-seventh-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root36_rollover_states']:,} bound root-36 rollovers, "
        f"{b['bound_root36_binding_states']:,} bound root-36 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
