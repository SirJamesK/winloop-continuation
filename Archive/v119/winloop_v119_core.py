...
D = 3
BASE_V118_EPOCH69_COMPLETE_STATIC_STATES = 576
...
def _gc70_ok(phase, twenty_first_source_handoff, twenty_first_source_binding,
             verifier_binding, carried_root_binding, continuity, *rest):
    if len(rest) != len(CARRIED_NAMES) + 1:
        return False
    *carried, deadline_reset = rest
    trust = (
        twenty_first_source_handoff, twenty_first_source_binding, verifier_binding,
        carried_root_binding, continuity, *carried,
    )
    if any(x == 3 for x in trust) or deadline_reset != 0:
        return False
    # V118 completes epoch 69 only after twentieth-source lineage rotation,
    # lineage binding, handed-proof rebind, and verifier completion. Epoch 70
    # hands that rebound proof to a twenty-first source, binds it, and only then
    # permits verifier completion while retaining the epoch-12 deadline.
    if carried_root_binding != 2 or continuity != 1 or any(x != 2 for x in carried):
        return False
    return _GC70_EXPECTED.get(phase) == (
        twenty_first_source_handoff, twenty_first_source_binding, verifier_binding
    )
...
def gc70():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC70_EXPECTED.items()]
    assert all(_gc70_ok(*x) for x in tails)
    z = q(95)
    seed = BASE_V118_EPOCH69_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc70_ok(0, 0, 0, 0, *fixed),
        _gc70_ok(2, 2, 0, 0, *fixed),
        _gc70_ok(4, 2, 2, 0, *fixed),
        _gc70_ok(6, 2, 2, 2, *fixed),
        not _gc70_ok(6, 3, 2, 2, *fixed),
        not _gc70_ok(6, 2, 3, 2, *fixed),
        not _gc70_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc70_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 95) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch69_complete_seed_states': seed,
        'delay_vectors': 4 ** 95,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch70_twenty_first_source_handoff_states': handoff * z,
        'epoch70_bound_twenty_first_source_handoff_states': bound_handoff * z,
        'epoch70_twenty_first_source_binding_states': source_binding_count * z,
        'epoch70_bound_twenty_first_source_binding_states': bound_source_binding * z,
        'epoch70_verifier_binding_states': verifier * z,
        'epoch70_bound_verifier_binding_states': bound_verifier * z,
        'epoch70_complete_states': complete * z,
        'unbound_or_conflicting_twenty_first_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twenty_first_source_binding_acceptances': transition_bad[1] * z,
