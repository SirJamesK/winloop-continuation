...
BASE_V89_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem90_ok(phase, witness_source_replacement, replacement_source_binding,
              root10_rollover, root10_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root10_rollover, root10_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V89 leaves a rebound witness current at generation 4 / root 9. V90 binds
    # a new witness source, rolls to root 10, binds that root, and only then lets
    # a replication-quorum churn complete.
    if generation != 4 or carried_root != 9 or target_root != 10:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM90_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root10_rollover, root10_binding, quorum_churn
    )


def root10_rollover_after_root9_witness_source_replacement():
    tails = [
        (phase, *state, 4, 9, 10, 2, 1, 2, 2, 0)
        for phase, state in _MEM90_EXPECTED.items()
    ]
    assert all(_mem90_ok(*x) for x in tails)
    z = q(32)
    seed = BASE_V89_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 9 for x in tails) * seed
    target_root_regression = sum(x[8] < 10 for x in tails) * seed
    ...
    checks = [
        _mem90_ok(0, 0, 0, 0, 0, 0, 4, 9, 10, 2, 1, 2, 2, 0),
        _mem90_ok(2, 2, 0, 0, 0, 0, 4, 9, 10, 2, 1, 2, 2, 0),
        _mem90_ok(4, 2, 2, 0, 0, 0, 4, 9, 10, 2, 1, 2, 2, 0),
        _mem90_ok(6, 2, 2, 2, 0, 0, 4, 9, 10, 2, 1, 2, 2, 0),
        _mem90_ok(8, 2, 2, 2, 2, 0, 4, 9, 10, 2, 1, 2, 2, 0),
        _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 3, 2, 2, 2, 2, 4, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 3, 2, 2, 2, 4, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 3, 2, 2, 4, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 3, 2, 4, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 3, 4, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 3, 9, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 8, 10, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 9, 2, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 10, 1, 1, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 10, 2, 2, 2, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 10, 2, 1, 1, 2, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 10, 2, 1, 2, 3, 0),
        not _mem90_ok(10, 2, 2, 2, 2, 2, 4, 9, 10, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 11 * 11 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 32) * z
    return {
        'patterns': patterns,
        ...
        'root10_rollover_states': rollover * z,
        'bound_root10_rollover_states': bound_rollover * z,
        'root10_binding_states': root_binding * z,
        'bound_root10_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root10_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root10_binding_acceptances': root_binding_bad * z,
