...
D = 3
BASE_V111_EPOCH62_COMPLETE_STATIC_STATES = 576
...
def _gc63_ok(phase, seventeenth_lineage_rotation, seventeenth_lineage_binding,
    ...
    *carried, deadline_reset = rest
    trust = (
        seventeenth_lineage_rotation, seventeenth_lineage_binding,
        new_handed_proof_rebind, verifier_binding, carried_root_binding,
        continuity, *carried,
    )
    ...
    if deadline_reset != 0:
        return False
    # V111 completed epoch 62 only after the rebound proof was handed to and
    # bound by a seventeenth source. Epoch 63 rotates and binds that source
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC63_EXPECTED.get(phase) == (
        seventeenth_lineage_rotation, seventeenth_lineage_binding,
...
def gc63():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC63_EXPECTED.items()]
    assert all(_gc63_ok(*x) for x in tails)
    z = q(81)
    seed = BASE_V111_EPOCH62_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc63_ok(0, 0, 0, 0, 0, *fixed),
        _gc63_ok(2, 2, 0, 0, 0, *fixed),
        _gc63_ok(4, 2, 2, 0, 0, *fixed),
        _gc63_ok(6, 2, 2, 2, 0, *fixed),
        _gc63_ok(8, 2, 2, 2, 2, *fixed),
        not _gc63_ok(8, 3, 2, 2, 2, *fixed),
        not _gc63_ok(8, 2, 3, 2, 2, *fixed),
        not _gc63_ok(8, 2, 2, 3, 2, *fixed),
        not _gc63_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc63_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 81) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch62_complete_seed_states': seed,
        'delay_vectors': 4 ** 81,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch63_seventeenth_lineage_rotation_states': rotation * z,
        'epoch63_bound_seventeenth_lineage_rotation_states': bound_rotation * z,
        'epoch63_seventeenth_lineage_binding_states': lineage_binding * z,
        'epoch63_bound_seventeenth_lineage_binding_states': bound_lineage_binding * z,
        'epoch63_handed_proof_rebind_states': proof_rebind * z,
        'epoch63_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch63_verifier_binding_states': verifier * z,
        'epoch63_bound_verifier_binding_states': bound_verifier * z,
        'epoch63_complete_states': complete * z,
        'unbound_or_conflicting_seventeenth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_seventeenth_lineage_binding_acceptances': transition_bad[1] * z,
