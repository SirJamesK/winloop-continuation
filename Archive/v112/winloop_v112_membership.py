from winloop_v112_core import q

BASE_V111_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem112_ok(phase, witness_source_replacement, replacement_source_binding,
               root21_rollover, root21_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root21_rollover, root21_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V111 leaves a rebound witness current at generation 4 / root 20. V112
    # binds a new witness source, rolls to root 21, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 20 or target_root != 21:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM112_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root21_rollover, root21_binding, quorum_churn
    )


def root21_rollover_after_root20_witness_source_replacement():
    tails = [
        (phase, *state, 4, 20, 21, 2, 1, 2, 2, 0)
        for phase, state in _MEM112_EXPECTED.items()
    ]
    assert all(_mem112_ok(*x) for x in tails)
    z = q(76)
    seed = BASE_V111_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 20 for x in tails) * seed
    target_root_regression = sum(x[8] < 21 for x in tails) * seed
    ...
    checks = [
        _mem112_ok(0, 0, 0, 0, 0, 0, 4, 20, 21, 2, 1, 2, 2, 0),
        _mem112_ok(2, 2, 0, 0, 0, 0, 4, 20, 21, 2, 1, 2, 2, 0),
        _mem112_ok(4, 2, 2, 0, 0, 0, 4, 20, 21, 2, 1, 2, 2, 0),
        _mem112_ok(6, 2, 2, 2, 0, 0, 4, 20, 21, 2, 1, 2, 2, 0),
        _mem112_ok(8, 2, 2, 2, 2, 0, 4, 20, 21, 2, 1, 2, 2, 0),
        _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 3, 2, 2, 2, 2, 4, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 3, 2, 2, 2, 4, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 3, 2, 2, 4, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 3, 2, 4, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 3, 4, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 3, 20, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 19, 21, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 20, 2, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 21, 1, 1, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 21, 2, 2, 2, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 21, 2, 1, 1, 2, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 21, 2, 1, 2, 3, 0),
        not _mem112_ok(10, 2, 2, 2, 2, 2, 4, 20, 21, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 76) * z
    return {
        'patterns': patterns,
        ...
        'root21_rollover_states': rollover * z,
        'bound_root21_rollover_states': bound_rollover * z,
        'root21_binding_states': root_binding * z,
        'bound_root21_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root21_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root21_binding_acceptances': root_binding_bad * z,
