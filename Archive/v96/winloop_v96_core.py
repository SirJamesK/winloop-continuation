...
D = 3
BASE_V95_EPOCH46_COMPLETE_STATIC_STATES = 576
...
def _gc47_ok(phase, ninth_lineage_rotation, ninth_lineage_binding,
             ...
             ninth_source_handoff, ninth_source_binding, deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    trust = (
        ninth_lineage_rotation, ninth_lineage_binding, new_handed_proof_rebind,
    ...
    if deadline_reset != 0:
        return False
    # V95 completed epoch 46 only after the rebound proof was handed to and
    # bound by a ninth source. Epoch 47 rotates and binds that source lineage,
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC47_EXPECTED.get(phase) == (
...
def gc47():
    fixed = (2, 1, *([2] * 23), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC47_EXPECTED.items()]
    assert all(_gc47_ok(*x) for x in tails)
    z = q(49)
    seed = BASE_V95_EPOCH46_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(7, 30)]
    deadline_bad = sum(x[30] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc47_ok(0, 0, 0, 0, 0, *fixed),
        _gc47_ok(2, 2, 0, 0, 0, *fixed),
        _gc47_ok(4, 2, 2, 0, 0, *fixed),
        _gc47_ok(6, 2, 2, 2, 0, *fixed),
        _gc47_ok(8, 2, 2, 2, 2, *fixed),
        not _gc47_ok(8, 3, 2, 2, 2, *fixed),
        not _gc47_ok(8, 2, 3, 2, 2, *fixed),
        not _gc47_ok(8, 2, 2, 3, 2, *fixed),
        not _gc47_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc47_ok(8, 2, 2, 2, 2, *broken))

    # Phase, four epoch-47 transitions, carried root, continuity, twenty-three
    # carried source/key/lineage/proof/handoff bindings, and deadline-reset bit.
    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** 23) * 2
    patterns = static_patterns * (4 ** 49) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch46_complete_seed_states': seed,
        'delay_vectors': 4 ** 49,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch47_ninth_lineage_rotation_states': rotation * z,
        'epoch47_bound_ninth_lineage_rotation_states': bound_rotation * z,
        'epoch47_ninth_lineage_binding_states': lineage_binding * z,
        'epoch47_bound_ninth_lineage_binding_states': bound_lineage_binding * z,
        'epoch47_handed_proof_rebind_states': proof_rebind * z,
        'epoch47_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch47_verifier_binding_states': verifier * z,
        'epoch47_bound_verifier_binding_states': bound_verifier * z,
        'epoch47_complete_states': complete * z,
        'unbound_or_conflicting_ninth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_ninth_lineage_binding_acceptances': transition_bad[1] * z,
