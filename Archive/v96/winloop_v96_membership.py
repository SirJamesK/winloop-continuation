...
BASE_V95_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem96_ok(phase, witness_source_replacement, replacement_source_binding,
              root13_rollover, root13_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root13_rollover, root13_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V95 leaves a rebound witness current at generation 4 / root 12. V96
    # binds a new witness source, rolls to root 13, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 12 or target_root != 13:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM96_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root13_rollover, root13_binding, quorum_churn
    )


def root13_rollover_after_root12_witness_source_replacement():
    tails = [
        (phase, *state, 4, 12, 13, 2, 1, 2, 2, 0)
        for phase, state in _MEM96_EXPECTED.items()
    ]
    assert all(_mem96_ok(*x) for x in tails)
    z = q(44)
    seed = BASE_V95_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 12 for x in tails) * seed
    target_root_regression = sum(x[8] < 13 for x in tails) * seed
    ...
    checks = [
        _mem96_ok(0, 0, 0, 0, 0, 0, 4, 12, 13, 2, 1, 2, 2, 0),
        _mem96_ok(2, 2, 0, 0, 0, 0, 4, 12, 13, 2, 1, 2, 2, 0),
        _mem96_ok(4, 2, 2, 0, 0, 0, 4, 12, 13, 2, 1, 2, 2, 0),
        _mem96_ok(6, 2, 2, 2, 0, 0, 4, 12, 13, 2, 1, 2, 2, 0),
        _mem96_ok(8, 2, 2, 2, 2, 0, 4, 12, 13, 2, 1, 2, 2, 0),
        _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 3, 2, 2, 2, 2, 4, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 3, 2, 2, 2, 4, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 3, 2, 2, 4, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 3, 2, 4, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 3, 4, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 3, 12, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 11, 13, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 12, 2, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 13, 1, 1, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 13, 2, 2, 2, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 13, 2, 1, 1, 2, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 13, 2, 1, 2, 3, 0),
        not _mem96_ok(10, 2, 2, 2, 2, 2, 4, 12, 13, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 14 * 14 * 4 * 3 * 4 * 4 * 2
    ...
    return {
        ...
        'root13_rollover_states': rollover * z,
        'bound_root13_rollover_states': bound_rollover * z,
        'root13_binding_states': root_binding * z,
        'bound_root13_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root13_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root13_binding_acceptances': root_binding_bad * z,
