...
D = 3
BASE_V119_EPOCH70_COMPLETE_STATIC_STATES = 576
...
def _gc71_ok(phase, twenty_first_lineage_rotation, twenty_first_lineage_binding,
    ...
    *carried, deadline_reset = rest
    trust = (
        twenty_first_lineage_rotation, twenty_first_lineage_binding,
        new_handed_proof_rebind, verifier_binding, carried_root_binding,
        continuity, *carried,
    )
    if any(x == 3 for x in trust) or deadline_reset != 0:
        return False
    # V119 completes epoch 70 only after twenty-first-source handoff, source
    # binding, and verifier completion. Epoch 71 rotates that source lineage,
    ...
    if carried_root_binding != 2 or continuity != 1 or any(x != 2 for x in carried):
        return False
    return _GC71_EXPECTED.get(phase) == (
        twenty_first_lineage_rotation, twenty_first_lineage_binding,
...
def gc71():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC71_EXPECTED.items()]
    assert all(_gc71_ok(*x) for x in tails)
    z = q(97)
    seed = BASE_V119_EPOCH70_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc71_ok(0, 0, 0, 0, 0, *fixed),
        _gc71_ok(2, 2, 0, 0, 0, *fixed),
        _gc71_ok(4, 2, 2, 0, 0, *fixed),
        _gc71_ok(6, 2, 2, 2, 0, *fixed),
        _gc71_ok(8, 2, 2, 2, 2, *fixed),
        not _gc71_ok(8, 3, 2, 2, 2, *fixed),
        not _gc71_ok(8, 2, 3, 2, 2, *fixed),
        not _gc71_ok(8, 2, 2, 3, 2, *fixed),
        not _gc71_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc71_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 97) * z
    out = {
        'patterns': patterns, 'accepted': static_accepted * z,
        'base_states': static_accepted, 'epoch70_complete_seed_states': seed,
        'delay_vectors': 4 ** 97, 'deadline_vectors': z, 'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch71_twenty_first_lineage_rotation_states': rotation * z,
        'epoch71_bound_twenty_first_lineage_rotation_states': bound_rotation * z,
        'epoch71_twenty_first_lineage_binding_states': lineage_binding * z,
        'epoch71_bound_twenty_first_lineage_binding_states': bound_lineage_binding * z,
        'epoch71_handed_proof_rebind_states': proof_rebind * z,
        'epoch71_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch71_verifier_binding_states': verifier * z,
        'epoch71_bound_verifier_binding_states': bound_verifier * z,
        'epoch71_complete_states': complete * z,
        'unbound_or_conflicting_twenty_first_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twenty_first_lineage_binding_acceptances': transition_bad[1] * z,
