from winloop_v104_core import q

BASE_V103_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem104_ok(phase, witness_source_replacement, replacement_source_binding,
               root17_rollover, root17_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root17_rollover, root17_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V103 leaves a rebound witness current at generation 4 / root 16. V104
    # binds a new witness source, rolls to root 17, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 16 or target_root != 17:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM104_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root17_rollover, root17_binding, quorum_churn
    )


def root17_rollover_after_root16_witness_source_replacement():
    tails = [
        (phase, *state, 4, 16, 17, 2, 1, 2, 2, 0)
        for phase, state in _MEM104_EXPECTED.items()
    ]
    assert all(_mem104_ok(*x) for x in tails)
    z = q(60)
    seed = BASE_V103_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 16 for x in tails) * seed
    target_root_regression = sum(x[8] < 17 for x in tails) * seed
    ...
    checks = [
        _mem104_ok(0, 0, 0, 0, 0, 0, 4, 16, 17, 2, 1, 2, 2, 0),
        _mem104_ok(2, 2, 0, 0, 0, 0, 4, 16, 17, 2, 1, 2, 2, 0),
        _mem104_ok(4, 2, 2, 0, 0, 0, 4, 16, 17, 2, 1, 2, 2, 0),
        _mem104_ok(6, 2, 2, 2, 0, 0, 4, 16, 17, 2, 1, 2, 2, 0),
        _mem104_ok(8, 2, 2, 2, 2, 0, 4, 16, 17, 2, 1, 2, 2, 0),
        _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 3, 2, 2, 2, 2, 4, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 3, 2, 2, 2, 4, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 3, 2, 2, 4, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 3, 2, 4, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 3, 4, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 3, 16, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 15, 17, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 16, 2, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 17, 1, 1, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 17, 2, 2, 2, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 17, 2, 1, 1, 2, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 17, 2, 1, 2, 3, 0),
        not _mem104_ok(10, 2, 2, 2, 2, 2, 4, 16, 17, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 60) * z
    return {
        'patterns': patterns,
        ...
        'root17_rollover_states': rollover * z,
        'bound_root17_rollover_states': bound_rollover * z,
        'root17_binding_states': root_binding * z,
        'bound_root17_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root17_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root17_binding_acceptances': root_binding_bad * z,
