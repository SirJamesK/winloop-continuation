"""WinLoop V192: epoch-143 fifty-seventh-lineage rotation, publication-117 cold restart, and root-61 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V192'
BASE_DIGEST = '74e491bd9d07ad9ad7ae4a2acf1015ab516f7d5ff146aa24416d47e3fa4c7e99'
BASE_IMPL_SHA = '84fe7bd6b475f7e56461ae38815227f08097f764a670f6f36f7ccda1208ee3a9'
D = 3
...
def gc143():
    ...
    checks += [
        ...
        not ok(8,expected[8],carried=(2,)*292+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**241) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch142_complete_seed_states': seed,
        'delay_vectors': 4**241,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch143_bound_fifty_seventh_lineage_rotation_states': counts[0][1],
        'epoch143_bound_fifty_seventh_lineage_binding_states': counts[1][1],
        'epoch143_bound_handed_proof_rebind_states': counts[2][1],
        'epoch143_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication117():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**238) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_sixteenth_restart_seed_states': seed,
        'delay_vectors': 4**238,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_seventeenth_restart_states': counts[3][1],
        'bound_one_hundred_seventeenth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership61():
    ...
    def ok(phase, state, generation=4, carried_root=60, target_root=61,
        ...
        return (
            ...
            and (generation, carried_root, target_root, replication, tombstone,
                 witness, prior_source, active_byzantine) == (4,60,61,2,1,2,2,0)
        )
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [
        not ok(10,(3,2,2,2,2)),
        ...
        not ok(10,expected[10],carried_root=59),
        not ok(10,expected[10],target_root=60),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**236) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root61_rollover_states': counts[2][1],
        'bound_root61_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc143(), publication117(), membership61()
    o = {
        'version': V,
        'base': {
            'version':'V191',
        ...
        'epoch143': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch142_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fifty_seventh_lineage_rotation_states':t['epoch143_bound_fifty_seventh_lineage_rotation_states'],
            'bound_fifty_seventh_lineage_binding_states':t['epoch143_bound_fifty_seventh_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch143_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch143_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication117': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_sixteenth_restart_seed_states'],
            ...
            'bound_one_hundred_seventeenth_restart_states':s['bound_one_hundred_seventeenth_restart_states'],
            'bound_one_hundred_seventeenth_restart_recoveries':s['bound_one_hundred_seventeenth_restart_recoveries'],
        ...
        'membership61': {
            ...
            'bound_root61_rollover_states':b['bound_root61_rollover_states'],
            'bound_root61_binding_states':b['bound_root61_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 144 by handing the rebound proof to a fifty-eighth source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-seventeenth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-eighteenth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-61 rollover, rebind the witness to root 61, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V192 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-143 GC to {t['accepted']:,} states with {t['epoch143_bound_fifty_seventh_lineage_rotation_states']:,} bound fifty-seventh-lineage rotations, "
        f"{t['epoch143_bound_fifty_seventh_lineage_binding_states']:,} bound lineage bindings, "
        f"{t['epoch143_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, and "
        f"{t['epoch143_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_seventeenth_restart_recoveries']:,} fully bound one-hundred-seventeenth-cold-restart recoveries; "
        ...
        f"{b['bound_root61_rollover_states']:,} bound root-61 rollovers, {b['bound_root61_binding_states']:,} bound root-61 bindings, "
