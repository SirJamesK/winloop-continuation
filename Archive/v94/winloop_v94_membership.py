from winloop_v94_core import q

BASE_V93_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem94_ok(phase, witness_source_replacement, replacement_source_binding,
              root12_rollover, root12_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root12_rollover, root12_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V93 leaves a rebound witness current at generation 4 / root 11. V94
    # binds a new witness source, rolls to root 12, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 11 or target_root != 12:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM94_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root12_rollover, root12_binding, quorum_churn
    )


def root12_rollover_after_root11_witness_source_replacement():
    tails = [
        (phase, *state, 4, 11, 12, 2, 1, 2, 2, 0)
        for phase, state in _MEM94_EXPECTED.items()
    ]
    assert all(_mem94_ok(*x) for x in tails)
    z = q(40)
    seed = BASE_V93_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 11 for x in tails) * seed
    target_root_regression = sum(x[8] < 12 for x in tails) * seed
    ...
    checks = [
        _mem94_ok(0, 0, 0, 0, 0, 0, 4, 11, 12, 2, 1, 2, 2, 0),
        _mem94_ok(2, 2, 0, 0, 0, 0, 4, 11, 12, 2, 1, 2, 2, 0),
        _mem94_ok(4, 2, 2, 0, 0, 0, 4, 11, 12, 2, 1, 2, 2, 0),
        _mem94_ok(6, 2, 2, 2, 0, 0, 4, 11, 12, 2, 1, 2, 2, 0),
        _mem94_ok(8, 2, 2, 2, 2, 0, 4, 11, 12, 2, 1, 2, 2, 0),
        _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 3, 2, 2, 2, 2, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 3, 2, 2, 2, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 3, 2, 2, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 3, 2, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 3, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 3, 11, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 10, 12, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 11, 2, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 12, 1, 1, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 12, 2, 2, 2, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 12, 2, 1, 1, 2, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 12, 2, 1, 2, 3, 0),
        not _mem94_ok(10, 2, 2, 2, 2, 2, 4, 11, 12, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 13 * 13 * 4 * 3 * 4 * 4 * 2
    ...
    return {
        ...
        'root12_rollover_states': rollover * z,
        'bound_root12_rollover_states': bound_rollover * z,
        'root12_binding_states': root_binding * z,
        'bound_root12_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root12_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root12_binding_acceptances': root_binding_bad * z,
