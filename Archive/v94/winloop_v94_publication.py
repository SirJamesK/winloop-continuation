...
BASE_V93_BOUND_EIGHTEENTH_RESTART_STATIC_STATES = 27648
...
def _pub94_ok(phase, replacement_churn, successor_binding,
              dual, restart19, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        replacement_churn, successor_binding, dual, restart19, consume, quorum
    )):
        return False
    # V93's eighteenth restart is a seed only after successor disappearance,
    # replacement binding, and reconciliation. V94 requires replacement-source
    # churn, successor binding, fresh reconciliation, and restart 19.
    if cache_authority != 0:
        return False
    return _PUB94_EXPECTED.get(phase) == (
        replacement_churn, successor_binding, dual, restart19, consume, quorum
    )


def replacement_churn_nineteenth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB94_EXPECTED.items()
    ]
    assert all(_pub94_ok(*x) for x in tails)
    z = q(42)
    seed = BASE_V93_BOUND_EIGHTEENTH_RESTART_STATIC_STATES
    ...
    restart19 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart19 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub94_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub94_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub94_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub94_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub94_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub94_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub94_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub94_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub94_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub94_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub94_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub94_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub94_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_eighteenth_restart_seed_states': seed,
        'delay_vectors': 4 ** 42,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'nineteenth_verifier_cold_restart_states': restart19 * z,
        'bound_nineteenth_restart_states': bound_restart19 * z,
        'bound_nineteenth_restart_recoveries': bound_recovery * z,
        'cached_nineteenth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_nineteenth_restart_acceptances': restart_bad * z,
