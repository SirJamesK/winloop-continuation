"""WinLoop V162 exact continuation: epoch-113 forty-second-lineage rotation, eighty-seventh cold restart, and root-46 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V162'
BASE_DIGEST = '86c9f6d0247009fcb27d1fa3577c4a4c5d8e47d2bb681113b4abdeffb478dd6d'
BASE_IMPL_SHA = 'ec309f91f343346336ab04eec8c5741ed7da0011a5eed47faeb247a93fd619e1'
D = 3
...
def gc113():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*202+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**181) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch112_complete_seed_states': seed,
        'delay_vectors': 4**181,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch113_bound_forty_second_lineage_rotation_states': counts[0][1],
        'epoch113_bound_forty_second_lineage_binding_states': counts[1][1],
        'epoch113_bound_handed_proof_rebind_states': counts[2][1],
        'epoch113_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication87():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**178) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_sixth_restart_seed_states': seed,
        'delay_vectors': 4**178,
        'deadline_vectors': z,
        ...
        'bound_eighty_seventh_restart_states': counts[3][1],
        'bound_eighty_seventh_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership46():
    ...
    def ok(phase, state, generation=4, carried_root=45, target_root=46,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=44),
        not ok(10,expected[10],target_root=45),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**176) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root46_rollover_states': counts[2][1],
        'bound_root46_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc113(), publication87(), membership46()
    o = {
        'version': V,
        'base': {
            'version':'V161',
        ...
        'epoch113': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch112_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_second_lineage_rotation_states':t['epoch113_bound_forty_second_lineage_rotation_states'],
            'bound_forty_second_lineage_binding_states':t['epoch113_bound_forty_second_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch113_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch113_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication87': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_sixth_restart_seed_states'],
            ...
            'bound_eighty_seventh_restart_states':s['bound_eighty_seventh_restart_states'],
            'bound_eighty_seventh_restart_recoveries':s['bound_eighty_seventh_restart_recoveries'],
        ...
        'membership46': {
            ...
            'bound_root46_rollover_states':b['bound_root46_rollover_states'],
            'bound_root46_binding_states':b['bound_root46_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 114 by handing the rebound proof to a forty-third source, binding that source, and preserving the epoch-12 deadline',
            'compose eighty-seventh-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and an eighty-eighth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-46 rollover, rebind the witness to root 46, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V162 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-113 GC to {t['accepted']:,} states with {t['epoch113_bound_forty_second_lineage_rotation_states']:,} bound forty-second-lineage rotations, "
        f"{t['epoch113_bound_forty_second_lineage_binding_states']:,} bound lineage bindings, {t['epoch113_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch113_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_eighty_seventh_restart_recoveries']:,} fully bound eighty-seventh-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root46_rollover_states']:,} bound root-46 rollovers, "
        f"{b['bound_root46_binding_states']:,} bound root-46 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
