"""WinLoop V177 exact continuation: epoch-128 fiftieth-source handoff, one-hundred-second cold restart, and root-53 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V177'
BASE_DIGEST = 'de949b3d39a81afdd9231d464506ed6de8fbb9003cafa8dc88ac3720f9f68d92'
BASE_IMPL_SHA = '15189eab2c3026217ba9eb12cac0ee009d066c9d7d5cc43388b0e0ad9c29ad8e'
D = 3
...
def gc128():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*247+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**211) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch127_complete_seed_states': seed,
        'delay_vectors': 4**211,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch128_bound_fiftieth_source_handoff_states': counts[0][1],
        'epoch128_bound_fiftieth_source_binding_states': counts[1][1],
        'epoch128_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication102():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**208) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_first_restart_seed_states': seed,
        'delay_vectors': 4**208,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_second_restart_states': counts[3][1],
        'bound_one_hundred_second_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership53():
    ...
    def ok(phase, state, generation=4, carried_root=53, target_root=53,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=52),
        not ok(6,expected[6],target_root=52),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**206) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**206,
        'deadline_vectors': z,
        'bound_root53_witness_rebind_states': counts[0][1],
        'bound_root53_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc128(), publication102(), membership53()
    o = {
        'version': V,
        'base': {
            'version':'V176',
        ...
        'epoch128': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch127_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fiftieth_source_handoff_states':t['epoch128_bound_fiftieth_source_handoff_states'],
            'bound_fiftieth_source_binding_states':t['epoch128_bound_fiftieth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch128_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication102': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_first_restart_seed_states'],
            ...
            'bound_one_hundred_second_restart_states':s['bound_one_hundred_second_restart_states'],
            'bound_one_hundred_second_restart_recoveries':s['bound_one_hundred_second_restart_recoveries'],
        ...
        'membership53': {
            ...
            'bound_root53_witness_rebind_states':b['bound_root53_witness_rebind_states'],
            'bound_root53_witness_binding_states':b['bound_root53_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 129 by rotating the fiftieth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose one-hundred-second-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a one-hundred-third verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-53 witness rebind, replace the witness source, roll to root 54, bind root 54, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V177 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-128 GC to {t['accepted']:,} states with {t['epoch128_bound_fiftieth_source_handoff_states']:,} bound fiftieth-source handoffs, "
        f"{t['epoch128_bound_fiftieth_source_binding_states']:,} bound fiftieth-source bindings, and {t['epoch128_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_second_restart_recoveries']:,} fully bound one-hundred-second-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root53_witness_rebind_states']:,} bound root-53 witness rebinds, "
        f"{b['bound_root53_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
