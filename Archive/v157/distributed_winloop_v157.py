"""WinLoop V157 exact continuation: epoch-108 fortieth-source handoff, eighty-second cold restart, and root-43 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V157'
BASE_DIGEST = '3db99058ca4aefcf159517a3afec5b912f3ec19b25c8079f480d56b3db2f33da'
BASE_IMPL_SHA = '90da6d7348aeb725e3b1c65b826ec6547e7761ac49ac26ed1480d1c0b47451ee'
D = 3
...
def gc108():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*187+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**171) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch107_complete_seed_states': seed,
        'delay_vectors': 4**171,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch108_bound_fortieth_source_handoff_states': counts[0][1],
        'epoch108_bound_fortieth_source_binding_states': counts[1][1],
        'epoch108_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication82():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**168) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_first_restart_seed_states': seed,
        'delay_vectors': 4**168,
        'deadline_vectors': z,
        ...
        'bound_eighty_second_restart_states': counts[3][1],
        'bound_eighty_second_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership43():
    ...
    def ok(phase, state, generation=4, carried_root=43, target_root=43,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=42),
        not ok(6,expected[6],target_root=42),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**166) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**166,
        'deadline_vectors': z,
        'bound_root43_witness_rebind_states': counts[0][1],
        'bound_root43_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc108(), publication82(), membership43()
    o = {
        'version': V,
        'base': {
            'version':'V156',
        ...
        'epoch108': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch107_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fortieth_source_handoff_states':t['epoch108_bound_fortieth_source_handoff_states'],
            'bound_fortieth_source_binding_states':t['epoch108_bound_fortieth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch108_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication82': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_first_restart_seed_states'],
            ...
            'bound_eighty_second_restart_states':s['bound_eighty_second_restart_states'],
            'bound_eighty_second_restart_recoveries':s['bound_eighty_second_restart_recoveries'],
        ...
        'membership43': {
            ...
            'bound_root43_witness_rebind_states':b['bound_root43_witness_rebind_states'],
            'bound_root43_witness_binding_states':b['bound_root43_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 109 by rotating the fortieth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose eighty-second-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and an eighty-third verifier cold restart without cached authority promotion',
            'keep generation 4 after root-43 witness rebind, replace the witness source, roll to root 44, bind root 44, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V157 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-108 GC to {t['accepted']:,} states with {t['epoch108_bound_fortieth_source_handoff_states']:,} bound fortieth-source handoffs, "
        f"{t['epoch108_bound_fortieth_source_binding_states']:,} bound fortieth-source bindings, and {t['epoch108_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_eighty_second_restart_recoveries']:,} fully bound eighty-second-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root43_witness_rebind_states']:,} bound root-43 witness rebinds, "
        f"{b['bound_root43_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
