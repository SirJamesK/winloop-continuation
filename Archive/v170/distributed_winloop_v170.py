"""WinLoop V170 exact continuation: epoch-121 forty-sixth-lineage rotation, ninety-fifth cold restart, and root-50 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V170'
BASE_DIGEST = '0f9ac6b0c9a51bb20da324c5d429b049c1055fa649b6a1ec5208e2106fdc8ffb'
BASE_IMPL_SHA = '134550703fd4eaa035434431dd3c505d5b56f2561ae84d34f58c5f4a611ec282'
D = 3
...
def gc121():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*226+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**197) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch120_complete_seed_states': seed,
        'delay_vectors': 4**197,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch121_bound_forty_sixth_lineage_rotation_states': counts[0][1],
        'epoch121_bound_forty_sixth_lineage_binding_states': counts[1][1],
        'epoch121_bound_handed_proof_rebind_states': counts[2][1],
        'epoch121_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication95():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**194) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_fourth_restart_seed_states': seed,
        'delay_vectors': 4**194,
        'deadline_vectors': z,
        ...
        'bound_ninety_fifth_restart_states': counts[3][1],
        'bound_ninety_fifth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership50():
    ...
    def ok(phase, state, generation=4, carried_root=49, target_root=50,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=48),
        not ok(10,expected[10],target_root=49),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**192) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root50_rollover_states': counts[2][1],
        'bound_root50_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc121(), publication95(), membership50()
    o = {
        'version': V,
        'base': {
            'version':'V169',
        ...
        'epoch121': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch120_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_sixth_lineage_rotation_states':t['epoch121_bound_forty_sixth_lineage_rotation_states'],
            'bound_forty_sixth_lineage_binding_states':t['epoch121_bound_forty_sixth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch121_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch121_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication95': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_fourth_restart_seed_states'],
            ...
            'bound_ninety_fifth_restart_states':s['bound_ninety_fifth_restart_states'],
            'bound_ninety_fifth_restart_recoveries':s['bound_ninety_fifth_restart_recoveries'],
        ...
        'membership50': {
            ...
            'bound_root50_rollover_states':b['bound_root50_rollover_states'],
            'bound_root50_binding_states':b['bound_root50_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 122 by handing the rebound proof to a forty-seventh source, binding that source, and preserving the epoch-12 deadline',
            'compose ninety-fifth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a ninety-sixth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-50 rollover, rebind the witness to root 50, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V170 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-121 GC to {t['accepted']:,} states with {t['epoch121_bound_forty_sixth_lineage_rotation_states']:,} bound forty-sixth-lineage rotations, "
        f"{t['epoch121_bound_forty_sixth_lineage_binding_states']:,} bound lineage bindings, {t['epoch121_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch121_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_ninety_fifth_restart_recoveries']:,} fully bound ninety-fifth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root50_rollover_states']:,} bound root-50 rollovers, "
        f"{b['bound_root50_binding_states']:,} bound root-50 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
