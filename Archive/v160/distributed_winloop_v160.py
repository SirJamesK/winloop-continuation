"""WinLoop V160 exact continuation: epoch-111 forty-first-lineage rotation, eighty-fifth cold restart, and root-45 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V160'
BASE_DIGEST = 'a0d1d498c0fae6bfc4d134c6f9db1560f66258ac31fc4ade99e81faeba13b004'
BASE_IMPL_SHA = 'c1e5d339dd6ec2a97d525b083fd9771a67831e486a1dc22129e15fdc4379a588'
D = 3
...
def gc111():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*196+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**177) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch110_complete_seed_states': seed,
        'delay_vectors': 4**177,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch111_bound_forty_first_lineage_rotation_states': counts[0][1],
        'epoch111_bound_forty_first_lineage_binding_states': counts[1][1],
        'epoch111_bound_handed_proof_rebind_states': counts[2][1],
        'epoch111_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication85():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**174) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_fourth_restart_seed_states': seed,
        'delay_vectors': 4**174,
        'deadline_vectors': z,
        ...
        'bound_eighty_fifth_restart_states': counts[3][1],
        'bound_eighty_fifth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership45():
    ...
    def ok(phase, state, generation=4, carried_root=44, target_root=45,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=43),
        not ok(10,expected[10],target_root=44),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**172) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root45_rollover_states': counts[2][1],
        'bound_root45_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc111(), publication85(), membership45()
    o = {
        'version': V,
        'base': {
            'version':'V159',
        ...
        'epoch111': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch110_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_first_lineage_rotation_states':t['epoch111_bound_forty_first_lineage_rotation_states'],
            'bound_forty_first_lineage_binding_states':t['epoch111_bound_forty_first_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch111_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch111_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication85': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_fourth_restart_seed_states'],
            ...
            'bound_eighty_fifth_restart_states':s['bound_eighty_fifth_restart_states'],
            'bound_eighty_fifth_restart_recoveries':s['bound_eighty_fifth_restart_recoveries'],
        ...
        'membership45': {
            ...
            'bound_root45_rollover_states':b['bound_root45_rollover_states'],
            'bound_root45_binding_states':b['bound_root45_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 112 by handing the rebound proof to a forty-second source, binding that source, and preserving the epoch-12 deadline',
            'compose eighty-fifth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and an eighty-sixth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-45 rollover, rebind the witness to root 45, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V160 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-111 GC to {t['accepted']:,} states with {t['epoch111_bound_forty_first_lineage_rotation_states']:,} bound forty-first-lineage rotations, "
        f"{t['epoch111_bound_forty_first_lineage_binding_states']:,} bound lineage bindings, {t['epoch111_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch111_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_eighty_fifth_restart_recoveries']:,} fully bound eighty-fifth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root45_rollover_states']:,} bound root-45 rollovers, "
        f"{b['bound_root45_binding_states']:,} bound root-45 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
