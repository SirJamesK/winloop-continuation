"""WinLoop V141 exact continuation: epoch-92 thirty-second-source handoff, sixty-sixth cold restart, and root-35 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V141'
BASE_DIGEST = '26f33253f2a902c2938a7cea71690b590d831e83749ada51f221c1d307e5a43f'
BASE_IMPL_SHA = '927f63c99c57eeacef147f1256838e65af6f428c9e66cb56016bb5605bde14a0'
D = 3
...
def gc92():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*139+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**139) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch91_complete_seed_states': seed,
        'delay_vectors': 4**139,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch92_bound_thirty_second_source_handoff_states': counts[0][1],
        'epoch92_bound_thirty_second_source_binding_states': counts[1][1],
        'epoch92_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication66():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**136) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_sixty_fifth_restart_seed_states': seed,
        'delay_vectors': 4**136,
        'deadline_vectors': z,
        ...
        'bound_sixty_sixth_restart_states': counts[3][1],
        'bound_sixty_sixth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership35():
    ...
    def ok(phase, state, generation=4, carried_root=35, target_root=35,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=34),
        not ok(6,expected[6],target_root=34),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**134) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**134,
        'deadline_vectors': z,
        'bound_root35_witness_rebind_states': counts[0][1],
        'bound_root35_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc92(), publication66(), membership35()
    o = {
        'version': V,
        'base': {
            'version':'V140',
        ...
        'epoch92': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch91_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_second_source_handoff_states':t['epoch92_bound_thirty_second_source_handoff_states'],
            'bound_thirty_second_source_binding_states':t['epoch92_bound_thirty_second_source_binding_states'],
            'bound_verifier_binding_states':t['epoch92_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication66': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_sixty_fifth_restart_seed_states'],
            ...
            'bound_sixty_sixth_restart_states':s['bound_sixty_sixth_restart_states'],
            'bound_sixty_sixth_restart_recoveries':s['bound_sixty_sixth_restart_recoveries'],
        ...
        'membership35': {
            ...
            'bound_root35_witness_rebind_states':b['bound_root35_witness_rebind_states'],
            'bound_root35_witness_binding_states':b['bound_root35_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 93 by rotating the thirty-second-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose sixty-sixth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a sixty-seventh verifier cold restart without cached authority promotion',
            'keep generation 4 after root-35 witness rebind, replace the witness source, roll to root 36, bind root 36, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V141 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-92 GC to {t['accepted']:,} states with {t['epoch92_bound_thirty_second_source_handoff_states']:,} bound thirty-second-source handoffs, "
        f"{t['epoch92_bound_thirty_second_source_binding_states']:,} bound thirty-second-source bindings, and {t['epoch92_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_sixty_sixth_restart_recoveries']:,} fully bound sixty-sixth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root35_witness_rebind_states']:,} bound root-35 witness rebinds, "
        f"{b['bound_root35_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
