"""WinLoop V151 exact continuation: epoch-102 thirty-seventh-source handoff, seventy-sixth cold restart, and root-40 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V151'
BASE_DIGEST = '398889d8d3f100250b9ef2cce03089ceba73c90894b6abed7c919e959e2aed4f'
BASE_IMPL_SHA = '777cc3ff4d9c844083997dcbc7fd1f1a4da8ae574a759506687ca01eebda2f37'
D = 3
...
def gc102():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*169+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**159) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch101_complete_seed_states': seed,
        'delay_vectors': 4**159,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch102_bound_thirty_seventh_source_handoff_states': counts[0][1],
        'epoch102_bound_thirty_seventh_source_binding_states': counts[1][1],
        'epoch102_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication76():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**156) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_fifth_restart_seed_states': seed,
        'delay_vectors': 4**156,
        'deadline_vectors': z,
        ...
        'bound_seventy_sixth_restart_states': counts[3][1],
        'bound_seventy_sixth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership40():
    ...
    def ok(phase, state, generation=4, carried_root=40, target_root=40,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=39),
        not ok(6,expected[6],target_root=39),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**154) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**154,
        'deadline_vectors': z,
        'bound_root40_witness_rebind_states': counts[0][1],
        'bound_root40_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc102(), publication76(), membership40()
    o = {
        'version': V,
        'base': {
            'version':'V150',
        ...
        'epoch102': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch101_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_seventh_source_handoff_states':t['epoch102_bound_thirty_seventh_source_handoff_states'],
            'bound_thirty_seventh_source_binding_states':t['epoch102_bound_thirty_seventh_source_binding_states'],
            'bound_verifier_binding_states':t['epoch102_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication76': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_fifth_restart_seed_states'],
            ...
            'bound_seventy_sixth_restart_states':s['bound_seventy_sixth_restart_states'],
            'bound_seventy_sixth_restart_recoveries':s['bound_seventy_sixth_restart_recoveries'],
        ...
        'membership40': {
            ...
            'bound_root40_witness_rebind_states':b['bound_root40_witness_rebind_states'],
            'bound_root40_witness_binding_states':b['bound_root40_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 103 by rotating the thirty-seventh-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose seventy-sixth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a seventy-seventh verifier cold restart without cached authority promotion',
            'keep generation 4 after root-40 witness rebind, replace the witness source, roll to root 41, bind root 41, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V151 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-102 GC to {t['accepted']:,} states with {t['epoch102_bound_thirty_seventh_source_handoff_states']:,} bound thirty-seventh-source handoffs, "
        f"{t['epoch102_bound_thirty_seventh_source_binding_states']:,} bound thirty-seventh-source bindings, and {t['epoch102_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_seventy_sixth_restart_recoveries']:,} fully bound seventy-sixth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root40_witness_rebind_states']:,} bound root-40 witness rebinds, "
        f"{b['bound_root40_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
