"""WinLoop V171 exact continuation: epoch-122 forty-seventh-source handoff, ninety-sixth cold restart, and root-50 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V171'
BASE_DIGEST = 'bde19cca2adacecbdaccb7ec8124bfbf6933750bd81749be0ae2c98c688a209f'
BASE_IMPL_SHA = 'b0b69c9be3d5bc641f1058df0cac7cd51fd1b98e26b037b79c4f85f124a1d378'
D = 3
...
def gc122():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*229+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**199) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch121_complete_seed_states': seed,
        'delay_vectors': 4**199,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch122_bound_forty_seventh_source_handoff_states': counts[0][1],
        'epoch122_bound_forty_seventh_source_binding_states': counts[1][1],
        'epoch122_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication96():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**196) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_fifth_restart_seed_states': seed,
        'delay_vectors': 4**196,
        'deadline_vectors': z,
        ...
        'bound_ninety_sixth_restart_states': counts[3][1],
        'bound_ninety_sixth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership50():
    ...
    def ok(phase, state, generation=4, carried_root=50, target_root=50,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=49),
        not ok(6,expected[6],target_root=49),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**194) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**194,
        'deadline_vectors': z,
        'bound_root50_witness_rebind_states': counts[0][1],
        'bound_root50_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc122(), publication96(), membership50()
    o = {
        'version': V,
        'base': {
            'version':'V170',
        ...
        'epoch122': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch121_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_seventh_source_handoff_states':t['epoch122_bound_forty_seventh_source_handoff_states'],
            'bound_forty_seventh_source_binding_states':t['epoch122_bound_forty_seventh_source_binding_states'],
            'bound_verifier_binding_states':t['epoch122_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication96': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_fifth_restart_seed_states'],
            ...
            'bound_ninety_sixth_restart_states':s['bound_ninety_sixth_restart_states'],
            'bound_ninety_sixth_restart_recoveries':s['bound_ninety_sixth_restart_recoveries'],
        ...
        'membership50': {
            ...
            'bound_root50_witness_rebind_states':b['bound_root50_witness_rebind_states'],
            'bound_root50_witness_binding_states':b['bound_root50_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 123 by rotating the forty-seventh-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose ninety-sixth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a ninety-seventh verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-50 witness rebind, replace the witness source, roll to root 51, bind root 51, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V171 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-122 GC to {t['accepted']:,} states with {t['epoch122_bound_forty_seventh_source_handoff_states']:,} bound forty-seventh-source handoffs, "
        f"{t['epoch122_bound_forty_seventh_source_binding_states']:,} bound forty-seventh-source bindings, and {t['epoch122_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_ninety_sixth_restart_recoveries']:,} fully bound ninety-sixth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root50_witness_rebind_states']:,} bound root-50 witness rebinds, "
        f"{b['bound_root50_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
