...
D = 3
BASE_V107_EPOCH58_COMPLETE_STATIC_STATES = 576
...
def _gc59_ok(phase, fifteenth_lineage_rotation, fifteenth_lineage_binding,
             ...
             fifteenth_source_handoff, fifteenth_source_binding,
             deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    trust = (
        fifteenth_lineage_rotation, fifteenth_lineage_binding,
        new_handed_proof_rebind, verifier_binding, carried_root_binding,
    ...
    if deadline_reset != 0:
        return False
    # V107 completed epoch 58 only after the rebound proof was handed to and
    # bound by a fifteenth source. Epoch 59 rotates and binds that source
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC59_EXPECTED.get(phase) == (
        fifteenth_lineage_rotation, fifteenth_lineage_binding,
...
def gc59():
    fixed = (2, 1, *([2] * 53), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC59_EXPECTED.items()]
    assert all(_gc59_ok(*x) for x in tails)
    z = q(73)
    seed = BASE_V107_EPOCH58_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(7, 60)]
    deadline_bad = sum(x[60] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc59_ok(0, 0, 0, 0, 0, *fixed),
        _gc59_ok(2, 2, 0, 0, 0, *fixed),
        _gc59_ok(4, 2, 2, 0, 0, *fixed),
        _gc59_ok(6, 2, 2, 2, 0, *fixed),
        _gc59_ok(8, 2, 2, 2, 2, *fixed),
        not _gc59_ok(8, 3, 2, 2, 2, *fixed),
        not _gc59_ok(8, 2, 3, 2, 2, *fixed),
        not _gc59_ok(8, 2, 2, 3, 2, *fixed),
        not _gc59_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc59_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** 53) * 2
    patterns = static_patterns * (4 ** 73) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch58_complete_seed_states': seed,
        'delay_vectors': 4 ** 73,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch59_fifteenth_lineage_rotation_states': rotation * z,
        'epoch59_bound_fifteenth_lineage_rotation_states': bound_rotation * z,
        'epoch59_fifteenth_lineage_binding_states': lineage_binding * z,
        'epoch59_bound_fifteenth_lineage_binding_states': bound_lineage_binding * z,
        'epoch59_handed_proof_rebind_states': proof_rebind * z,
        'epoch59_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch59_verifier_binding_states': verifier * z,
        'epoch59_bound_verifier_binding_states': bound_verifier * z,
        'epoch59_complete_states': complete * z,
        'unbound_or_conflicting_fifteenth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_fifteenth_lineage_binding_acceptances': transition_bad[1] * z,
