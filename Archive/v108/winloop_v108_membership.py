...
BASE_V107_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem108_ok(phase, witness_source_replacement, replacement_source_binding,
               root19_rollover, root19_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root19_rollover, root19_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V107 leaves a rebound witness current at generation 4 / root 18. V108
    # binds a new witness source, rolls to root 19, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 18 or target_root != 19:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM108_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root19_rollover, root19_binding, quorum_churn
    )


def root19_rollover_after_root18_witness_source_replacement():
    tails = [
        (phase, *state, 4, 18, 19, 2, 1, 2, 2, 0)
        for phase, state in _MEM108_EXPECTED.items()
    ]
    assert all(_mem108_ok(*x) for x in tails)
    z = q(68)
    seed = BASE_V107_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 18 for x in tails) * seed
    target_root_regression = sum(x[8] < 19 for x in tails) * seed
    ...
    checks = [
        _mem108_ok(0, 0, 0, 0, 0, 0, 4, 18, 19, 2, 1, 2, 2, 0),
        _mem108_ok(2, 2, 0, 0, 0, 0, 4, 18, 19, 2, 1, 2, 2, 0),
        _mem108_ok(4, 2, 2, 0, 0, 0, 4, 18, 19, 2, 1, 2, 2, 0),
        _mem108_ok(6, 2, 2, 2, 0, 0, 4, 18, 19, 2, 1, 2, 2, 0),
        _mem108_ok(8, 2, 2, 2, 2, 0, 4, 18, 19, 2, 1, 2, 2, 0),
        _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 3, 2, 2, 2, 2, 4, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 3, 2, 2, 2, 4, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 3, 2, 2, 4, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 3, 2, 4, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 3, 4, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 3, 18, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 17, 19, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 18, 2, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 19, 1, 1, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 19, 2, 2, 2, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 19, 2, 1, 1, 2, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 19, 2, 1, 2, 3, 0),
        not _mem108_ok(10, 2, 2, 2, 2, 2, 4, 18, 19, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 68) * z
    return {
        'patterns': patterns,
        ...
        'root19_rollover_states': rollover * z,
        'bound_root19_rollover_states': bound_rollover * z,
        'root19_binding_states': root_binding * z,
        'bound_root19_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root19_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root19_binding_acceptances': root_binding_bad * z,
