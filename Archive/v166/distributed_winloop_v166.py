"""WinLoop V166 exact continuation: epoch-117 forty-fourth-lineage rotation, ninety-first cold restart, and root-48 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V166'
BASE_DIGEST = '2ce9e465accb92df1e7c88102df13a716a827798c34c81f3809fb24a34602723'
BASE_IMPL_SHA = '58c9fe655298e48389bec9a03884f924b382c8d538296b8d3fef8145ece2be2c'
D = 3
...
def gc117():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*214+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**189) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch116_complete_seed_states': seed,
        'delay_vectors': 4**189,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch117_bound_forty_fourth_lineage_rotation_states': counts[0][1],
        'epoch117_bound_forty_fourth_lineage_binding_states': counts[1][1],
        'epoch117_bound_handed_proof_rebind_states': counts[2][1],
        'epoch117_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication91():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**186) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninetieth_restart_seed_states': seed,
        'delay_vectors': 4**186,
        'deadline_vectors': z,
        ...
        'bound_ninety_first_restart_states': counts[3][1],
        'bound_ninety_first_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership48():
    ...
    def ok(phase, state, generation=4, carried_root=47, target_root=48,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=46),
        not ok(10,expected[10],target_root=47),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**184) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root48_rollover_states': counts[2][1],
        'bound_root48_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc117(), publication91(), membership48()
    o = {
        'version': V,
        'base': {
            'version':'V165',
        ...
        'epoch117': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch116_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_fourth_lineage_rotation_states':t['epoch117_bound_forty_fourth_lineage_rotation_states'],
            'bound_forty_fourth_lineage_binding_states':t['epoch117_bound_forty_fourth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch117_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch117_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication91': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninetieth_restart_seed_states'],
            ...
            'bound_ninety_first_restart_states':s['bound_ninety_first_restart_states'],
            'bound_ninety_first_restart_recoveries':s['bound_ninety_first_restart_recoveries'],
        ...
        'membership48': {
            ...
            'bound_root48_rollover_states':b['bound_root48_rollover_states'],
            'bound_root48_binding_states':b['bound_root48_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 118 by handing the rebound proof to a forty-fifth source, binding that source, and preserving the epoch-12 deadline',
            'compose ninety-first-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a ninety-second verifier cold restart without cached authority promotion',
            'keep generation 4 after root-48 rollover, rebind the witness to root 48, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V166 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-117 GC to {t['accepted']:,} states with {t['epoch117_bound_forty_fourth_lineage_rotation_states']:,} bound forty-fourth-lineage rotations, "
        f"{t['epoch117_bound_forty_fourth_lineage_binding_states']:,} bound lineage bindings, {t['epoch117_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch117_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_ninety_first_restart_recoveries']:,} fully bound ninety-first-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root48_rollover_states']:,} bound root-48 rollovers, "
        f"{b['bound_root48_binding_states']:,} bound root-48 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
