"""WinLoop V144 exact continuation: epoch-95 thirty-third-lineage rotation, sixty-ninth cold restart, and root-37 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V144'
BASE_DIGEST = 'b1a9bd98e58e2c8f9da9c853c1894471776af1aa6781ea183b7bf9fda1ad3017'
BASE_IMPL_SHA = '46112be6a5c931b7bfb053e162c3e4f4a1fed81ca22df85a7aaef16b0308929b'
D = 3
...
def gc95():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*148+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**145) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch94_complete_seed_states': seed,
        'delay_vectors': 4**145,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch95_bound_thirty_third_lineage_rotation_states': counts[0][1],
        'epoch95_bound_thirty_third_lineage_binding_states': counts[1][1],
        'epoch95_bound_handed_proof_rebind_states': counts[2][1],
        'epoch95_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication69():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**142) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_sixty_eighth_restart_seed_states': seed,
        'delay_vectors': 4**142,
        'deadline_vectors': z,
        ...
        'bound_sixty_ninth_restart_states': counts[3][1],
        'bound_sixty_ninth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership37():
    ...
    def ok(phase, state, generation=4, carried_root=36, target_root=37,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=35),
        not ok(10,expected[10],target_root=36),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**140) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root37_rollover_states': counts[2][1],
        'bound_root37_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc95(), publication69(), membership37()
    o = {
        'version': V,
        'base': {
            'version':'V143',
        ...
        'epoch95': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch94_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_third_lineage_rotation_states':t['epoch95_bound_thirty_third_lineage_rotation_states'],
            'bound_thirty_third_lineage_binding_states':t['epoch95_bound_thirty_third_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch95_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch95_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication69': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_sixty_eighth_restart_seed_states'],
            ...
            'bound_sixty_ninth_restart_states':s['bound_sixty_ninth_restart_states'],
            'bound_sixty_ninth_restart_recoveries':s['bound_sixty_ninth_restart_recoveries'],
        ...
        'membership37': {
            ...
            'bound_root37_rollover_states':b['bound_root37_rollover_states'],
            'bound_root37_binding_states':b['bound_root37_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 96 by handing the rebound proof to a thirty-fourth source, binding that source, and preserving the epoch-12 deadline',
            'compose sixty-ninth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a seventieth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-37 rollover, rebind the witness to root 37, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V144 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-95 GC to {t['accepted']:,} states with {t['epoch95_bound_thirty_third_lineage_rotation_states']:,} bound thirty-third-lineage rotations, "
        f"{t['epoch95_bound_thirty_third_lineage_binding_states']:,} bound lineage bindings, {t['epoch95_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch95_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_sixty_ninth_restart_recoveries']:,} fully bound sixty-ninth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root37_rollover_states']:,} bound root-37 rollovers, "
        f"{b['bound_root37_binding_states']:,} bound root-37 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
