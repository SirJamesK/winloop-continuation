"""WinLoop V138 exact continuation: epoch-89 thirtieth-lineage rotation, sixty-third cold restart, and root-34 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V138'
BASE_DIGEST = '5cc2db015b3c0df4faa87efb3dc927fb80f5f87d31790cab791788059e7b5352'
BASE_IMPL_SHA = '802bed4662e3cb1c297c2f80684bcb7bc83674549cfcc2b5b2dc21e415ea1107'
D = 3
...
def gc89():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*130+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**133) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch88_complete_seed_states': seed,
        'delay_vectors': 4**133,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch89_bound_thirtieth_lineage_rotation_states': counts[0][1],
        'epoch89_bound_thirtieth_lineage_binding_states': counts[1][1],
        'epoch89_bound_handed_proof_rebind_states': counts[2][1],
        'epoch89_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication63():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**130) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_sixty_second_restart_seed_states': seed,
        'delay_vectors': 4**130,
        'deadline_vectors': z,
        ...
        'bound_sixty_third_restart_states': counts[3][1],
        'bound_sixty_third_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership34():
    ...
    def ok(phase, state, generation=4, carried_root=33, target_root=34,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=32),
        not ok(10,expected[10],target_root=33),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**128) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root34_rollover_states': counts[2][1],
        'bound_root34_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc89(), publication63(), membership34()
    o = {
        'version': V,
        'base': {
            'version':'V137',
        ...
        'epoch89': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch88_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirtieth_lineage_rotation_states':t['epoch89_bound_thirtieth_lineage_rotation_states'],
            'bound_thirtieth_lineage_binding_states':t['epoch89_bound_thirtieth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch89_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch89_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication63': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_sixty_second_restart_seed_states'],
            ...
            'bound_sixty_third_restart_states':s['bound_sixty_third_restart_states'],
             'bound_sixty_third_restart_recoveries':s['bound_sixty_third_restart_recoveries'],
        ...
        'membership34': {
            ...
            'bound_root34_rollover_states':b['bound_root34_rollover_states'],
            'bound_root34_binding_states':b['bound_root34_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 90 by handing the rebound proof to a thirty-first source, binding that source, and preserving the epoch-12 deadline',
            'compose sixty-third-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a sixty-fourth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-34 rollover, rebind the witness to root 34, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V138 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-89 GC to {t['accepted']:,} states with {t['epoch89_bound_thirtieth_lineage_rotation_states']:,} bound thirtieth-lineage rotations, "
        f"{t['epoch89_bound_thirtieth_lineage_binding_states']:,} bound lineage bindings, {t['epoch89_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch89_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_sixty_third_restart_recoveries']:,} fully bound sixty-third-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root34_rollover_states']:,} bound root-34 rollovers, "
        f"{b['bound_root34_binding_states']:,} bound root-34 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
