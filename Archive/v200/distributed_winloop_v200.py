"""WinLoop V200 exact continuation: epoch-151 sixty-first-lineage rotation, one-hundred-twenty-fifth cold restart, and root-65 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V200'
BASE_DIGEST = '501c8219d470a49bbc8e25bbac739a1fc39f3aa4193b26f07908b1272f5b45a5'
BASE_IMPL_SHA = '8fa3bcedd8ef97911088ed2657bbf560f5a8d354e10647253015e1e1026b1d9c'
D = 3
...
def gc151():
    ...
    checks += [
        ...
        not ok(8,expected[8],carried=(2,)*316+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**257) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch150_complete_seed_states': seed,
        'delay_vectors': 4**257,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch151_bound_sixty_first_lineage_rotation_states': counts[0][1],
        'epoch151_bound_sixty_first_lineage_binding_states': counts[1][1],
        'epoch151_bound_handed_proof_rebind_states': counts[2][1],
        'epoch151_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication125():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**254) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_twenty_fourth_restart_seed_states': seed,
        'delay_vectors': 4**254,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_twenty_fifth_restart_states': counts[3][1],
        'bound_one_hundred_twenty_fifth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership65():
    ...
    def ok(phase, state, generation=4, carried_root=64, target_root=65,
        ...
        return (
            ...
            and (generation, carried_root, target_root, replication, tombstone,
                 witness, prior_source, active_byzantine) == (4,64,65,2,1,2,2,0)
        )
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [
        not ok(10,(3,2,2,2,2)),
        ...
        not ok(10,expected[10],carried_root=63),
        not ok(10,expected[10],target_root=64),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**252) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root65_rollover_states': counts[2][1],
        'bound_root65_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc151(), publication125(), membership65()
    o = {
        'version': V,
        'base': {
            'version':'V199',
        ...
        'epoch151': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch150_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_sixty_first_lineage_rotation_states':t['epoch151_bound_sixty_first_lineage_rotation_states'],
            'bound_sixty_first_lineage_binding_states':t['epoch151_bound_sixty_first_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch151_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch151_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication125': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_twenty_fourth_restart_seed_states'],
            ...
            'bound_one_hundred_twenty_fifth_restart_states':s['bound_one_hundred_twenty_fifth_restart_states'],
            'bound_one_hundred_twenty_fifth_restart_recoveries':s['bound_one_hundred_twenty_fifth_restart_recoveries'],
        ...
        'membership65': {
            ...
            'bound_root65_rollover_states':b['bound_root65_rollover_states'],
            'bound_root65_binding_states':b['bound_root65_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 152 by handing the rebound proof to a sixty-second source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-twenty-fifth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-twenty-sixth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-65 rollover, rebind the witness to root 65, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V200 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-151 GC to {t['accepted']:,} states with {t['epoch151_bound_sixty_first_lineage_rotation_states']:,} bound sixty-first-lineage rotations, "
        f"{t['epoch151_bound_sixty_first_lineage_binding_states']:,} bound lineage bindings, "
        f"{t['epoch151_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, and "
        f"{t['epoch151_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_twenty_fifth_restart_recoveries']:,} fully bound one-hundred-twenty-fifth-cold-restart recoveries; "
        ...
        f"{b['bound_root65_rollover_states']:,} bound root-65 rollovers, {b['bound_root65_binding_states']:,} bound root-65 bindings, "
