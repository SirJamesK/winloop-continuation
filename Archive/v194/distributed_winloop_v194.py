"""WinLoop V194: epoch-145 fifty-eighth-lineage rotation, publication-119 cold restart, and root-62 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V194'
BASE_DIGEST = '17082b9af88ca3c8369c7ff6963056246b5785e6d327af54fb71e68f1d7ebee8'
BASE_IMPL_SHA = '18a47dfae29776de7231afc51b108169d0ff8dc8fdb97d8480f67781e078fbc8'
D = 3
...
def gc145():
    ...
    checks += [
        ...
        not ok(8,expected[8],carried=(2,)*298+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**245) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch144_complete_seed_states': seed,
        'delay_vectors': 4**245,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch145_bound_fifty_eighth_lineage_rotation_states': counts[0][1],
        'epoch145_bound_fifty_eighth_lineage_binding_states': counts[1][1],
        'epoch145_bound_handed_proof_rebind_states': counts[2][1],
        'epoch145_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication119():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**242) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_eighteenth_restart_seed_states': seed,
        'delay_vectors': 4**242,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_nineteenth_restart_states': counts[3][1],
        'bound_one_hundred_nineteenth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership62():
    ...
    def ok(phase, state, generation=4, carried_root=61, target_root=62,
        ...
        return (
            ...
            and (generation, carried_root, target_root, replication, tombstone,
                 witness, prior_source, active_byzantine) == (4,61,62,2,1,2,2,0)
        )
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [
        not ok(10,(3,2,2,2,2)),
        ...
        not ok(10,expected[10],carried_root=60),
        not ok(10,expected[10],target_root=61),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**240) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root62_rollover_states': counts[2][1],
        'bound_root62_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc145(), publication119(), membership62()
    o = {
        'version': V,
        'base': {
            'version':'V193',
        ...
        'epoch145': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch144_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fifty_eighth_lineage_rotation_states':t['epoch145_bound_fifty_eighth_lineage_rotation_states'],
            'bound_fifty_eighth_lineage_binding_states':t['epoch145_bound_fifty_eighth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch145_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch145_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication119': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_eighteenth_restart_seed_states'],
            ...
            'bound_one_hundred_nineteenth_restart_states':s['bound_one_hundred_nineteenth_restart_states'],
            'bound_one_hundred_nineteenth_restart_recoveries':s['bound_one_hundred_nineteenth_restart_recoveries'],
        ...
        'membership62': {
            ...
            'bound_root62_rollover_states':b['bound_root62_rollover_states'],
            'bound_root62_binding_states':b['bound_root62_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 146 by handing the rebound proof to a fifty-ninth source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-nineteenth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-twentieth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-62 rollover, rebind the witness to root 62, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V194 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-145 GC to {t['accepted']:,} states with {t['epoch145_bound_fifty_eighth_lineage_rotation_states']:,} bound fifty-eighth-lineage rotations, "
        f"{t['epoch145_bound_fifty_eighth_lineage_binding_states']:,} bound lineage bindings, "
        f"{t['epoch145_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, and "
        f"{t['epoch145_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_nineteenth_restart_recoveries']:,} fully bound one-hundred-nineteenth-cold-restart recoveries; "
        ...
        f"{b['bound_root62_rollover_states']:,} bound root-62 rollovers, {b['bound_root62_binding_states']:,} bound root-62 bindings, "
