"""WinLoop V164 exact continuation: epoch-115 forty-third-lineage rotation, eighty-ninth cold restart, and root-47 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V164'
BASE_DIGEST = '82df8e634a70151c98cab7644bcf63c2aaf1f84aba4694f8d688bcc0061f8ff6'
BASE_IMPL_SHA = 'd3a57aad4c9eae1bad93ff010e4407317951771d2b50f3d51a892efcfcbf2a2e'
D = 3
...
def gc115():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*208+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**185) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch114_complete_seed_states': seed,
        'delay_vectors': 4**185,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch115_bound_forty_third_lineage_rotation_states': counts[0][1],
        'epoch115_bound_forty_third_lineage_binding_states': counts[1][1],
        'epoch115_bound_handed_proof_rebind_states': counts[2][1],
        'epoch115_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication89():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**182) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_eighth_restart_seed_states': seed,
        'delay_vectors': 4**182,
        'deadline_vectors': z,
        ...
        'bound_eighty_ninth_restart_states': counts[3][1],
        'bound_eighty_ninth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership47():
    ...
    def ok(phase, state, generation=4, carried_root=46, target_root=47,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=45),
        not ok(10,expected[10],target_root=46),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**180) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root47_rollover_states': counts[2][1],
        'bound_root47_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc115(), publication89(), membership47()
    o = {
        'version': V,
        'base': {
            'version':'V163',
        ...
        'epoch115': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch114_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_third_lineage_rotation_states':t['epoch115_bound_forty_third_lineage_rotation_states'],
            'bound_forty_third_lineage_binding_states':t['epoch115_bound_forty_third_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch115_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch115_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication89': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_eighth_restart_seed_states'],
            ...
            'bound_eighty_ninth_restart_states':s['bound_eighty_ninth_restart_states'],
            'bound_eighty_ninth_restart_recoveries':s['bound_eighty_ninth_restart_recoveries'],
        ...
        'membership47': {
            ...
            'bound_root47_rollover_states':b['bound_root47_rollover_states'],
            'bound_root47_binding_states':b['bound_root47_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 116 by handing the rebound proof to a forty-fourth source, binding that source, and preserving the epoch-12 deadline',
            'compose eighty-ninth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a ninetieth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-47 rollover, rebind the witness to root 47, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V164 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-115 GC to {t['accepted']:,} states with {t['epoch115_bound_forty_third_lineage_rotation_states']:,} bound forty-third-lineage rotations, "
        f"{t['epoch115_bound_forty_third_lineage_binding_states']:,} bound lineage bindings, {t['epoch115_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch115_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_eighty_ninth_restart_recoveries']:,} fully bound eighty-ninth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root47_rollover_states']:,} bound root-47 rollovers, "
        f"{b['bound_root47_binding_states']:,} bound root-47 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
