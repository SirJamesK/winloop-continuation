...
D = 3
BASE_V114_EPOCH65_COMPLETE_STATIC_STATES = 576
...
def _gc66_ok(phase, nineteenth_source_handoff, nineteenth_source_binding,
             verifier_binding, carried_root_binding, continuity, *rest):
    if len(rest) != len(CARRIED_NAMES) + 1:
        return False
    *carried, deadline_reset = rest
    trust = (
        nineteenth_source_handoff, nineteenth_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V114 completed epoch 65 only after the eighteenth-source lineage was
    # rotated and bound and the handed proof was rebound. Epoch 66 hands that
    # rebound proof to a nineteenth source, binds it, and then permits verifier completion.
    if carried_root_binding != 2 or continuity != 1:
        return False
    if any(x != 2 for x in carried):
        return False
    return _GC66_EXPECTED.get(phase) == (
        nineteenth_source_handoff, nineteenth_source_binding, verifier_binding
    )
...
def gc66():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC66_EXPECTED.items()]
    assert all(_gc66_ok(*x) for x in tails)
    z = q(87)
    seed = BASE_V114_EPOCH65_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc66_ok(0, 0, 0, 0, *fixed),
        _gc66_ok(2, 2, 0, 0, *fixed),
        _gc66_ok(4, 2, 2, 0, *fixed),
        _gc66_ok(6, 2, 2, 2, *fixed),
        not _gc66_ok(6, 3, 2, 2, *fixed),
        not _gc66_ok(6, 2, 3, 2, *fixed),
        not _gc66_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc66_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 87) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch65_complete_seed_states': seed,
        'delay_vectors': 4 ** 87,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch66_nineteenth_source_handoff_states': handoff * z,
        'epoch66_bound_nineteenth_source_handoff_states': bound_handoff * z,
        'epoch66_nineteenth_source_binding_states': source_binding_count * z,
        'epoch66_bound_nineteenth_source_binding_states': bound_source_binding * z,
        'epoch66_verifier_binding_states': verifier * z,
        'epoch66_bound_verifier_binding_states': bound_verifier * z,
        'epoch66_complete_states': complete * z,
        'unbound_or_conflicting_nineteenth_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_nineteenth_source_binding_acceptances': transition_bad[1] * z,
