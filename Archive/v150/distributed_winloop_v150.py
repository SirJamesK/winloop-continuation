"""WinLoop V150 exact continuation: epoch-101 thirty-sixth-lineage rotation, seventy-fifth cold restart, and root-40 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V150'
BASE_DIGEST = '1d0c9a4f73c2c9f9c02b8e02a3c808cbb2760697f6d5cc791f620be8ee798048'
BASE_IMPL_SHA = 'cef10f9dcee25d82635eff256317bdcfef874f96687a0c254214f566f9ec602e'
D = 3
...
def gc101():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*166+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**157) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch100_complete_seed_states': seed,
        'delay_vectors': 4**157,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch101_bound_thirty_sixth_lineage_rotation_states': counts[0][1],
        'epoch101_bound_thirty_sixth_lineage_binding_states': counts[1][1],
        'epoch101_bound_handed_proof_rebind_states': counts[2][1],
        'epoch101_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication75():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**154) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_fourth_restart_seed_states': seed,
        'delay_vectors': 4**154,
        'deadline_vectors': z,
        ...
        'bound_seventy_fifth_restart_states': counts[3][1],
        'bound_seventy_fifth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership40():
    ...
    def ok(phase, state, generation=4, carried_root=39, target_root=40,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=38),
        not ok(10,expected[10],target_root=39),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**152) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root40_rollover_states': counts[2][1],
        'bound_root40_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc101(), publication75(), membership40()
    o = {
        'version': V,
        'base': {
            'version':'V149',
        ...
        'epoch101': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch100_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_sixth_lineage_rotation_states':t['epoch101_bound_thirty_sixth_lineage_rotation_states'],
            'bound_thirty_sixth_lineage_binding_states':t['epoch101_bound_thirty_sixth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch101_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch101_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication75': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_fourth_restart_seed_states'],
            ...
            'bound_seventy_fifth_restart_states':s['bound_seventy_fifth_restart_states'],
            'bound_seventy_fifth_restart_recoveries':s['bound_seventy_fifth_restart_recoveries'],
        ...
        'membership40': {
            ...
            'bound_root40_rollover_states':b['bound_root40_rollover_states'],
            'bound_root40_binding_states':b['bound_root40_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 102 by handing the rebound proof to a thirty-seventh source, binding that source, and preserving the epoch-12 deadline',
            'compose seventy-fifth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a seventy-sixth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-40 rollover, rebind the witness to root 40, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V150 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-101 GC to {t['accepted']:,} states with {t['epoch101_bound_thirty_sixth_lineage_rotation_states']:,} bound thirty-sixth-lineage rotations, "
        f"{t['epoch101_bound_thirty_sixth_lineage_binding_states']:,} bound lineage bindings, {t['epoch101_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch101_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_seventy_fifth_restart_recoveries']:,} fully bound seventy-fifth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root40_rollover_states']:,} bound root-40 rollovers, "
        f"{b['bound_root40_binding_states']:,} bound root-40 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
