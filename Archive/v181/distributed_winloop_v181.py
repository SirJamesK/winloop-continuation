"""WinLoop V181 exact continuation: epoch-132 fifty-second-source handoff, one-hundred-sixth cold restart, and root-55 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V181'
BASE_DIGEST = 'fb9c7c992e3a40990dee4924e8be1194dfbeaff449cd38b02f9b921b5482eb70'
BASE_IMPL_SHA = 'f2fb32072cac321d9ae2566cf3a66caa723786f3da5801045df3bdf3ea1ad130'
D = 3
...
def gc132():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*259+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**219) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch131_complete_seed_states': seed,
        'delay_vectors': 4**219,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch132_bound_fifty_second_source_handoff_states': counts[0][1],
        'epoch132_bound_fifty_second_source_binding_states': counts[1][1],
        'epoch132_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication106():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**216) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_fifth_restart_seed_states': seed,
        'delay_vectors': 4**216,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_sixth_restart_states': counts[3][1],
        'bound_one_hundred_sixth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership55():
    ...
    def ok(phase, state, generation=4, carried_root=55, target_root=55,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=54),
        not ok(6,expected[6],target_root=54),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**214) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**214,
        'deadline_vectors': z,
        'bound_root55_witness_rebind_states': counts[0][1],
        'bound_root55_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc132(), publication106(), membership55()
    o = {
        'version': V,
        'base': {
            'version':'V180',
        ...
        'epoch132': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch131_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fifty_second_source_handoff_states':t['epoch132_bound_fifty_second_source_handoff_states'],
            'bound_fifty_second_source_binding_states':t['epoch132_bound_fifty_second_source_binding_states'],
            'bound_verifier_binding_states':t['epoch132_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication106': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_fifth_restart_seed_states'],
            ...
            'bound_one_hundred_sixth_restart_states':s['bound_one_hundred_sixth_restart_states'],
            'bound_one_hundred_sixth_restart_recoveries':s['bound_one_hundred_sixth_restart_recoveries'],
        ...
        'membership55': {
            ...
            'bound_root55_witness_rebind_states':b['bound_root55_witness_rebind_states'],
            'bound_root55_witness_binding_states':b['bound_root55_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 133 by rotating the fifty-second-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose one-hundred-sixth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a one-hundred-seventh verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-55 witness rebind, replace the witness source, roll to root 56, bind root 56, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V181 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-132 GC to {t['accepted']:,} states with {t['epoch132_bound_fifty_second_source_handoff_states']:,} bound fifty-second-source handoffs, "
        f"{t['epoch132_bound_fifty_second_source_binding_states']:,} bound fifty-second-source bindings, and {t['epoch132_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_sixth_restart_recoveries']:,} fully bound one-hundred-sixth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root55_witness_rebind_states']:,} bound root-55 witness rebinds, "
        f"{b['bound_root55_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
