...
D = 3
BASE_V127_EPOCH78_COMPLETE_STATIC_STATES = 576
...
CARRIED_NAMES = _EARLY_CARRIED_NAMES + tuple(
    ...
    'twenty_fifth_source_handoff',
    'twenty_fifth_source_binding',
)
...
def _gc79_ok(phase, twenty_fifth_lineage_rotation, twenty_fifth_lineage_binding,
    ...
    *carried, deadline_reset = rest
    trust = (
        twenty_fifth_lineage_rotation, twenty_fifth_lineage_binding,
        new_handed_proof_rebind, verifier_binding, carried_root_binding,
        continuity, *carried,
    )
    if any(x == 3 for x in trust) or deadline_reset != 0:
        return False
    # V127 completes epoch 78 only after twenty-fifth-source handoff, source
    # binding, and verifier completion. Epoch 79 rotates that source lineage,
    ...
    if carried_root_binding != 2 or continuity != 1 or any(x != 2 for x in carried):
        return False
    return _GC79_EXPECTED.get(phase) == (
        twenty_fifth_lineage_rotation, twenty_fifth_lineage_binding,
...
def gc79():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC79_EXPECTED.items()]
    assert all(_gc79_ok(*x) for x in tails)
    z = q(113)
    seed = BASE_V127_EPOCH78_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc79_ok(0, 0, 0, 0, 0, *fixed),
        _gc79_ok(2, 2, 0, 0, 0, *fixed),
        _gc79_ok(4, 2, 2, 0, 0, *fixed),
        _gc79_ok(6, 2, 2, 2, 0, *fixed),
        _gc79_ok(8, 2, 2, 2, 2, *fixed),
        not _gc79_ok(8, 3, 2, 2, 2, *fixed),
        not _gc79_ok(8, 2, 3, 2, 2, *fixed),
        not _gc79_ok(8, 2, 2, 3, 2, *fixed),
        not _gc79_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc79_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 113) * z
    out = {
        'patterns': patterns, 'accepted': static_accepted * z,
        'base_states': static_accepted, 'epoch78_complete_seed_states': seed,
        'delay_vectors': 4 ** 113, 'deadline_vectors': z, 'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch79_twenty_fifth_lineage_rotation_states': rotation * z,
        'epoch79_bound_twenty_fifth_lineage_rotation_states': bound_rotation * z,
        'epoch79_twenty_fifth_lineage_binding_states': lineage_binding * z,
        'epoch79_bound_twenty_fifth_lineage_binding_states': bound_lineage_binding * z,
        'epoch79_handed_proof_rebind_states': proof_rebind * z,
        'epoch79_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch79_verifier_binding_states': verifier * z,
        'epoch79_bound_verifier_binding_states': bound_verifier * z,
        'epoch79_complete_states': complete * z,
        'unbound_or_conflicting_twenty_fifth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twenty_fifth_lineage_binding_acceptances': transition_bad[1] * z,
