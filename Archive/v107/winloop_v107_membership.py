from winloop_v107_core import q

BASE_V106_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem107_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V106 completed the root-18 rollover with replacement/prior-source bindings
    # intact. V107 keeps generation 4 / root 18 fixed, rebinds the witness to
    # root 18, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 18 or target_root != 18:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM107_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root18_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 18, 18, 2, 1, 2, 2, 0)
        for phase, state in _MEM107_EXPECTED.items()
    ]
    assert all(_mem107_ok(*x) for x in tails)
    z = q(66)
    seed = BASE_V106_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 18 for x in tails) * seed
    target_root_regression = sum(x[6] < 18 for x in tails) * seed
    ...
    checks = [
        _mem107_ok(0, 0, 0, 0, 4, 18, 18, 2, 1, 2, 2, 0),
        _mem107_ok(2, 2, 0, 0, 4, 18, 18, 2, 1, 2, 2, 0),
        _mem107_ok(4, 2, 2, 0, 4, 18, 18, 2, 1, 2, 2, 0),
        _mem107_ok(6, 2, 2, 2, 4, 18, 18, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 3, 2, 2, 4, 18, 18, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 3, 2, 4, 18, 18, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 3, 4, 18, 18, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 3, 18, 18, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 18, 17, 2, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 18, 18, 1, 1, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 18, 18, 2, 2, 2, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 18, 18, 2, 1, 3, 2, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 18, 18, 2, 1, 2, 3, 0),
        not _mem107_ok(6, 2, 2, 2, 4, 18, 18, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 7 * (4 ** 3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 66) * z
    return {
        'patterns': patterns,
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root18_witness_rebind_states': rebind * z,
        'bound_root18_witness_rebind_states': bound_rebind * z,
        'root18_witness_binding_states': witness_binding * z,
        'bound_root18_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root18_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root18_witness_binding_acceptances': witness_bad * z,
