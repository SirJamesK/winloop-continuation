...
BASE_V117_BOUND_FORTY_SECOND_RESTART_STATIC_STATES = 27648
...
def _pub118_ok(phase, replacement_churn, successor_binding,
               dual, restart43, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        replacement_churn, successor_binding, dual, restart43, consume, quorum
    )):
        return False
    # V117's forty-second restart is a seed only after successor disappearance,
    # replacement binding, and reconciliation. V118 requires replacement-source
    # churn, successor binding, fresh reconciliation, and restart 43.
    if cache_authority != 0:
        return False
    return _PUB118_EXPECTED.get(phase) == (
        replacement_churn, successor_binding, dual, restart43, consume, quorum
    )


def replacement_churn_forty_third_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB118_EXPECTED.items()
    ]
    assert all(_pub118_ok(*x) for x in tails)
    z = q(90)
    seed = BASE_V117_BOUND_FORTY_SECOND_RESTART_STATIC_STATES
    ...
    restart43 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart43 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub118_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub118_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub118_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub118_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub118_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub118_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub118_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub118_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub118_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub118_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub118_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub118_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub118_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_forty_second_restart_seed_states': seed,
        'delay_vectors': 4 ** 90, 'deadline_vectors': z, 'shared_deadline': 3,
        ...
        'forty_third_verifier_cold_restart_states': restart43 * z,
        'bound_forty_third_restart_states': bound_restart43 * z,
        'bound_forty_third_restart_recoveries': bound_recovery * z,
        'cached_forty_third_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_forty_third_restart_acceptances': restart_bad * z,
