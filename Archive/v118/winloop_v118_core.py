...
D = 3
BASE_V117_EPOCH68_COMPLETE_STATIC_STATES = 576
...
def _gc69_ok(phase, twentieth_lineage_rotation, twentieth_lineage_binding,
    ...
    *carried, deadline_reset = rest
    ...
    if carried_root_binding != 2 or continuity != 1 or any(x != 2 for x in carried):
        return False
    return _GC69_EXPECTED.get(phase) == (
        twentieth_lineage_rotation, twentieth_lineage_binding,
...
def gc69():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC69_EXPECTED.items()]
    assert all(_gc69_ok(*x) for x in tails)
    z = q(93)
    seed = BASE_V117_EPOCH68_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc69_ok(0, 0, 0, 0, 0, *fixed),
        _gc69_ok(2, 2, 0, 0, 0, *fixed),
        _gc69_ok(4, 2, 2, 0, 0, *fixed),
        _gc69_ok(6, 2, 2, 2, 0, *fixed),
        _gc69_ok(8, 2, 2, 2, 2, *fixed),
        not _gc69_ok(8, 3, 2, 2, 2, *fixed),
        not _gc69_ok(8, 2, 3, 2, 2, *fixed),
        not _gc69_ok(8, 2, 2, 3, 2, *fixed),
        not _gc69_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc69_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 93) * z
    out = {
        'patterns': patterns, 'accepted': static_accepted * z,
        'base_states': static_accepted, 'epoch68_complete_seed_states': seed,
        'delay_vectors': 4 ** 93, 'deadline_vectors': z, 'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch69_twentieth_lineage_rotation_states': rotation * z,
        'epoch69_bound_twentieth_lineage_rotation_states': bound_rotation * z,
        'epoch69_twentieth_lineage_binding_states': lineage_binding * z,
        'epoch69_bound_twentieth_lineage_binding_states': bound_lineage_binding * z,
        'epoch69_handed_proof_rebind_states': proof_rebind * z,
        'epoch69_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch69_verifier_binding_states': verifier * z,
        'epoch69_bound_verifier_binding_states': bound_verifier * z,
        'epoch69_complete_states': complete * z,
        'unbound_or_conflicting_twentieth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twentieth_lineage_binding_acceptances': transition_bad[1] * z,
