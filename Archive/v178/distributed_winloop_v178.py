"""WinLoop V178 exact continuation: epoch-129 fiftieth-lineage rotation, one-hundred-third cold restart, and root-54 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V178'
BASE_DIGEST = 'f7d0db87f3341bddd91179f3cfb4d0deea800750fb12e6b0bffcc350a9e8bcd2'
BASE_IMPL_SHA = 'bd68377e1d4770f22f58a73e8ec629c947ebe8ccf6c1cb5771b8ab96a92de1e2'
D = 3
...
def gc129():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*250+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**213) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch128_complete_seed_states': seed,
        'delay_vectors': 4**213,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch129_bound_fiftieth_lineage_rotation_states': counts[0][1],
        'epoch129_bound_fiftieth_lineage_binding_states': counts[1][1],
        'epoch129_bound_handed_proof_rebind_states': counts[2][1],
        'epoch129_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication103():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**210) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_second_restart_seed_states': seed,
        'delay_vectors': 4**210,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_third_restart_states': counts[3][1],
        'bound_one_hundred_third_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership54():
    ...
    def ok(phase, state, generation=4, carried_root=53, target_root=54,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=52),
        not ok(10,expected[10],target_root=53),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**208) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root54_rollover_states': counts[2][1],
        'bound_root54_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc129(), publication103(), membership54()
    o = {
        'version': V,
        'base': {
            'version':'V177',
        ...
        'epoch129': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch128_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fiftieth_lineage_rotation_states':t['epoch129_bound_fiftieth_lineage_rotation_states'],
            'bound_fiftieth_lineage_binding_states':t['epoch129_bound_fiftieth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch129_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch129_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication103': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_second_restart_seed_states'],
            ...
            'bound_one_hundred_third_restart_states':s['bound_one_hundred_third_restart_states'],
            'bound_one_hundred_third_restart_recoveries':s['bound_one_hundred_third_restart_recoveries'],
        ...
        'membership54': {
            ...
            'bound_root54_rollover_states':b['bound_root54_rollover_states'],
            'bound_root54_binding_states':b['bound_root54_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 130 by handing the rebound proof to a fifty-first source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-third-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-fourth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-54 rollover, rebind the witness to root 54, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V178 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-129 GC to {t['accepted']:,} states with {t['epoch129_bound_fiftieth_lineage_rotation_states']:,} bound fiftieth-lineage rotations, "
        f"{t['epoch129_bound_fiftieth_lineage_binding_states']:,} bound lineage bindings, {t['epoch129_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch129_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_one_hundred_third_restart_recoveries']:,} fully bound one-hundred-third-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root54_rollover_states']:,} bound root-54 rollovers, "
        f"{b['bound_root54_binding_states']:,} bound root-54 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
