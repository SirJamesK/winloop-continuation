"""WinLoop V155 exact continuation: epoch-106 thirty-ninth-source handoff, eightieth cold restart, and root-42 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V155'
BASE_DIGEST = 'd6fa08811511c3dd75c3abc6bbfa8e993625e874c20d13702d9a1548513eaf76'
BASE_IMPL_SHA = '5dbb2af22d761d624a9bc16845ecad8c4c8b57784cd3251451cbbc2799dabd3d'
D = 3
...
def gc106():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*181+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**167) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch105_complete_seed_states': seed,
        'delay_vectors': 4**167,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch106_bound_thirty_ninth_source_handoff_states': counts[0][1],
        'epoch106_bound_thirty_ninth_source_binding_states': counts[1][1],
        'epoch106_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication80():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**164) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_ninth_restart_seed_states': seed,
        'delay_vectors': 4**164,
        'deadline_vectors': z,
        ...
        'bound_eightieth_restart_states': counts[3][1],
        'bound_eightieth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership42():
    ...
    def ok(phase, state, generation=4, carried_root=42, target_root=42,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=41),
        not ok(6,expected[6],target_root=41),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**162) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**162,
        'deadline_vectors': z,
        'bound_root42_witness_rebind_states': counts[0][1],
        'bound_root42_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc106(), publication80(), membership42()
    o = {
        'version': V,
        'base': {
            'version':'V154',
        ...
        'epoch106': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch105_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_ninth_source_handoff_states':t['epoch106_bound_thirty_ninth_source_handoff_states'],
            'bound_thirty_ninth_source_binding_states':t['epoch106_bound_thirty_ninth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch106_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication80': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_ninth_restart_seed_states'],
            ...
            'bound_eightieth_restart_states':s['bound_eightieth_restart_states'],
            'bound_eightieth_restart_recoveries':s['bound_eightieth_restart_recoveries'],
        ...
        'membership42': {
            ...
            'bound_root42_witness_rebind_states':b['bound_root42_witness_rebind_states'],
            'bound_root42_witness_binding_states':b['bound_root42_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 107 by rotating the thirty-ninth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose eightieth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and an eighty-first verifier cold restart without cached authority promotion',
            'keep generation 4 after root-42 witness rebind, replace the witness source, roll to root 43, bind root 43, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V155 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-106 GC to {t['accepted']:,} states with {t['epoch106_bound_thirty_ninth_source_handoff_states']:,} bound thirty-ninth-source handoffs, "
        f"{t['epoch106_bound_thirty_ninth_source_binding_states']:,} bound thirty-ninth-source bindings, and {t['epoch106_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_eightieth_restart_recoveries']:,} fully bound eightieth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root42_witness_rebind_states']:,} bound root-42 witness rebinds, "
        f"{b['bound_root42_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
