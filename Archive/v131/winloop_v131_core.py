...
D = 3
BASE_V130_EPOCH81_COMPLETE_STATIC_STATES = 576
...
def _gc82_ok(phase, twenty_seventh_source_handoff, twenty_seventh_source_binding,
             verifier_binding, carried_root_binding, continuity, *rest):
    if len(rest) != len(CARRIED_NAMES) + 1:
        return False
    *carried, deadline_reset = rest
    trust = (
        twenty_seventh_source_handoff, twenty_seventh_source_binding, verifier_binding,
        carried_root_binding, continuity, *carried,
    )
    if any(x == 3 for x in trust) or deadline_reset != 0:
        return False
    # V130 completes epoch 81 only after twenty-sixth-source lineage rotation,
    # lineage binding, handed-proof rebind, and verifier completion. Epoch 82
    # hands that rebound proof to a twenty-seventh source, binds it, and only
    # then permits verifier completion while retaining the original epoch-12 deadline.
    if carried_root_binding != 2 or continuity != 1 or any(x != 2 for x in carried):
        return False
    return _GC82_EXPECTED.get(phase) == (
        twenty_seventh_source_handoff, twenty_seventh_source_binding, verifier_binding
    )
...
def gc82():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC82_EXPECTED.items()]
    assert all(_gc82_ok(*x) for x in tails)
    z = q(119)
    seed = BASE_V130_EPOCH81_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc82_ok(0, 0, 0, 0, *fixed),
        _gc82_ok(2, 2, 0, 0, *fixed),
        _gc82_ok(4, 2, 2, 0, *fixed),
        _gc82_ok(6, 2, 2, 2, *fixed),
        not _gc82_ok(6, 3, 2, 2, *fixed),
        not _gc82_ok(6, 2, 3, 2, *fixed),
        not _gc82_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc82_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 119) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch81_complete_seed_states': seed,
        'delay_vectors': 4 ** 119,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch82_twenty_seventh_source_handoff_states': handoff * z,
        'epoch82_bound_twenty_seventh_source_handoff_states': bound_handoff * z,
        'epoch82_twenty_seventh_source_binding_states': source_binding_count * z,
        'epoch82_bound_twenty_seventh_source_binding_states': bound_source_binding * z,
        'epoch82_verifier_binding_states': verifier * z,
        'epoch82_bound_verifier_binding_states': bound_verifier * z,
        'epoch82_complete_states': complete * z,
        'unbound_or_conflicting_twenty_seventh_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twenty_seventh_source_binding_acceptances': transition_bad[1] * z,
