...
BASE_V130_BOUND_FIFTY_FIFTH_RESTART_STATIC_STATES = 27648
...
def _pub131_ok(phase, successor_disappearance, replacement_binding,
               dual, restart56, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart56,
        consume, quorum
    )):
        return False
    # V130's fifty-fifth restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V131 handles successor disappearance,
    # replacement binding, fresh reconciliation, and restart 56.
    if cache_authority != 0:
        return False
    return _PUB131_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart56,
        consume, quorum
    )


def successor_disappearance_fifty_sixth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB131_EXPECTED.items()
    ]
    assert all(_pub131_ok(*x) for x in tails)
    z = q(116)
    seed = BASE_V130_BOUND_FIFTY_FIFTH_RESTART_STATIC_STATES
    ...
    restart56 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart56 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub131_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub131_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub131_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub131_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub131_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub131_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub131_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub131_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub131_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub131_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub131_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub131_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub131_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 6) * 2
    patterns = static_patterns * (4 ** 116) * z
    return {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'bound_fifty_fifth_restart_seed_states': seed,
        'delay_vectors': 4 ** 116,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'fifty_sixth_verifier_cold_restart_states': restart56 * z,
        'bound_fifty_sixth_restart_states': bound_restart56 * z,
        'bound_fifty_sixth_restart_recoveries': bound_recovery * z,
        'cached_fifty_sixth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_fifty_sixth_restart_acceptances': restart_bad * z,
