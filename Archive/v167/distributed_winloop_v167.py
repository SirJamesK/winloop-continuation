"""WinLoop V167 exact continuation: epoch-118 forty-fifth-source handoff, ninety-second cold restart, and root-48 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V167'
BASE_DIGEST = '993479ae487202702932dddc0873e4a186a3f44e99e7946ceddb44db664986fe'
BASE_IMPL_SHA = 'f476f36f3eaeb7d2f1f333d59c665290538270775f38cf8dc57ebe15a48f26cc'
D = 3
...
def gc118():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*217+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**191) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch117_complete_seed_states': seed,
        'delay_vectors': 4**191,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch118_bound_forty_fifth_source_handoff_states': counts[0][1],
        'epoch118_bound_forty_fifth_source_binding_states': counts[1][1],
        'epoch118_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication92():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**188) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_first_restart_seed_states': seed,
        'delay_vectors': 4**188,
        'deadline_vectors': z,
        ...
        'bound_ninety_second_restart_states': counts[3][1],
        'bound_ninety_second_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership48():
    ...
    def ok(phase, state, generation=4, carried_root=48, target_root=48,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=47),
        not ok(6,expected[6],target_root=47),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**186) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**186,
        'deadline_vectors': z,
        'bound_root48_witness_rebind_states': counts[0][1],
        'bound_root48_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc118(), publication92(), membership48()
    o = {
        'version': V,
        'base': {
            'version':'V166',
        ...
        'epoch118': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch117_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_fifth_source_handoff_states':t['epoch118_bound_forty_fifth_source_handoff_states'],
            'bound_forty_fifth_source_binding_states':t['epoch118_bound_forty_fifth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch118_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication92': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_first_restart_seed_states'],
            ...
            'bound_ninety_second_restart_states':s['bound_ninety_second_restart_states'],
            'bound_ninety_second_restart_recoveries':s['bound_ninety_second_restart_recoveries'],
        ...
        'membership48': {
            ...
            'bound_root48_witness_rebind_states':b['bound_root48_witness_rebind_states'],
            'bound_root48_witness_binding_states':b['bound_root48_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 119 by rotating the forty-fifth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose ninety-second-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a ninety-third verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-48 witness rebind, replace the witness source, roll to root 49, bind root 49, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V167 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-118 GC to {t['accepted']:,} states with {t['epoch118_bound_forty_fifth_source_handoff_states']:,} bound forty-fifth-source handoffs, "
        f"{t['epoch118_bound_forty_fifth_source_binding_states']:,} bound forty-fifth-source bindings, and {t['epoch118_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_ninety_second_restart_recoveries']:,} fully bound ninety-second-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root48_witness_rebind_states']:,} bound root-48 witness rebinds, "
        f"{b['bound_root48_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
