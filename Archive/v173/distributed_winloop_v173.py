"""WinLoop V173 exact continuation: epoch-124 forty-eighth-source handoff, ninety-eighth cold restart, and root-51 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V173'
BASE_DIGEST = 'f0f264da8db532153695b93b1d5c46cffde03f395a474cd859d529e9e37481a2'
BASE_IMPL_SHA = '71ff9651106ccb486a3232f2e438ae56cc6b7f1d7a694723cf350c254aea9bb2'
D = 3
...
def gc124():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*235+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**203) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch123_complete_seed_states': seed,
        'delay_vectors': 4**203,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch124_bound_forty_eighth_source_handoff_states': counts[0][1],
        'epoch124_bound_forty_eighth_source_binding_states': counts[1][1],
        'epoch124_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication98():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**200) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_seventh_restart_seed_states': seed,
        'delay_vectors': 4**200,
        'deadline_vectors': z,
        ...
        'bound_ninety_eighth_restart_states': counts[3][1],
        'bound_ninety_eighth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership51():
    ...
    def ok(phase, state, generation=4, carried_root=51, target_root=51,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=50),
        not ok(6,expected[6],target_root=50),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**198) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**198,
        'deadline_vectors': z,
        'bound_root51_witness_rebind_states': counts[0][1],
        'bound_root51_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc124(), publication98(), membership51()
    o = {
        'version': V,
        'base': {
            'version':'V172',
        ...
        'epoch124': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch123_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_eighth_source_handoff_states':t['epoch124_bound_forty_eighth_source_handoff_states'],
            'bound_forty_eighth_source_binding_states':t['epoch124_bound_forty_eighth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch124_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication98': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_seventh_restart_seed_states'],
            ...
            'bound_ninety_eighth_restart_states':s['bound_ninety_eighth_restart_states'],
            'bound_ninety_eighth_restart_recoveries':s['bound_ninety_eighth_restart_recoveries'],
        ...
        'membership51': {
            ...
            'bound_root51_witness_rebind_states':b['bound_root51_witness_rebind_states'],
            'bound_root51_witness_binding_states':b['bound_root51_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 125 by rotating the forty-eighth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose ninety-eighth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a ninety-ninth verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-51 witness rebind, replace the witness source, roll to root 52, bind root 52, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V173 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-124 GC to {t['accepted']:,} states with {t['epoch124_bound_forty_eighth_source_handoff_states']:,} bound forty-eighth-source handoffs, "
        f"{t['epoch124_bound_forty_eighth_source_binding_states']:,} bound forty-eighth-source bindings, and {t['epoch124_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_ninety_eighth_restart_recoveries']:,} fully bound ninety-eighth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root51_witness_rebind_states']:,} bound root-51 witness rebinds, "
        f"{b['bound_root51_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
