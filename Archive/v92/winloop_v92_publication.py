...
BASE_V91_BOUND_SIXTEENTH_RESTART_STATIC_STATES = 27648
...
def _pub92_ok(phase, replacement_churn, successor_binding,
              dual, restart17, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        replacement_churn, successor_binding, dual, restart17, consume, quorum
    )):
        return False
    # V91's sixteenth restart is a seed only after successor disappearance,
    # replacement binding, and reconciliation. V92 requires another replacement
    # churn, successor binding, fresh reconciliation, and restart 17.
    if cache_authority != 0:
        return False
    return _PUB92_EXPECTED.get(phase) == (
        replacement_churn, successor_binding, dual, restart17, consume, quorum
    )


def replacement_churn_seventeenth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB92_EXPECTED.items()
    ]
    assert all(_pub92_ok(*x) for x in tails)
    z = q(38)
    seed = BASE_V91_BOUND_SIXTEENTH_RESTART_STATIC_STATES
    ...
    restart17 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart17 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub92_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub92_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub92_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub92_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub92_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub92_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub92_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub92_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub92_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub92_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub92_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub92_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub92_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_sixteenth_restart_seed_states': seed,
        'delay_vectors': 4 ** 38,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'seventeenth_verifier_cold_restart_states': restart17 * z,
        'bound_seventeenth_restart_states': bound_restart17 * z,
        'bound_seventeenth_restart_recoveries': bound_recovery * z,
        'cached_seventeenth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_seventeenth_restart_acceptances': restart_bad * z,
