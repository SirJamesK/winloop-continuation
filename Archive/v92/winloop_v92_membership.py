from winloop_v92_core import q

BASE_V91_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem92_ok(phase, witness_source_replacement, replacement_source_binding,
              root11_rollover, root11_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root11_rollover, root11_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V91 leaves a rebound witness current at generation 4 / root 10. V92 binds
    # a new witness source, rolls to root 11, binds that root, and only then lets
    # a replication-quorum churn complete.
    if generation != 4 or carried_root != 10 or target_root != 11:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM92_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root11_rollover, root11_binding, quorum_churn
    )


def root11_rollover_after_root10_witness_source_replacement():
    tails = [
        (phase, *state, 4, 10, 11, 2, 1, 2, 2, 0)
        for phase, state in _MEM92_EXPECTED.items()
    ]
    assert all(_mem92_ok(*x) for x in tails)
    z = q(36)
    seed = BASE_V91_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 10 for x in tails) * seed
    target_root_regression = sum(x[8] < 11 for x in tails) * seed
    ...
    checks = [
        _mem92_ok(0, 0, 0, 0, 0, 0, 4, 10, 11, 2, 1, 2, 2, 0),
        _mem92_ok(2, 2, 0, 0, 0, 0, 4, 10, 11, 2, 1, 2, 2, 0),
        _mem92_ok(4, 2, 2, 0, 0, 0, 4, 10, 11, 2, 1, 2, 2, 0),
        _mem92_ok(6, 2, 2, 2, 0, 0, 4, 10, 11, 2, 1, 2, 2, 0),
        _mem92_ok(8, 2, 2, 2, 2, 0, 4, 10, 11, 2, 1, 2, 2, 0),
        _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 3, 2, 2, 2, 2, 4, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 3, 2, 2, 2, 4, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 3, 2, 2, 4, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 3, 2, 4, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 3, 4, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 3, 10, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 9, 11, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 10, 2, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 11, 1, 1, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 11, 2, 2, 2, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 11, 2, 1, 1, 2, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 11, 2, 1, 2, 3, 0),
        not _mem92_ok(10, 2, 2, 2, 2, 2, 4, 10, 11, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 12 * 12 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 36) * z
    return {
        'patterns': patterns,
        ...
        'root11_rollover_states': rollover * z,
        'bound_root11_rollover_states': bound_rollover * z,
        'root11_binding_states': root_binding * z,
        'bound_root11_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root11_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root11_binding_acceptances': root_binding_bad * z,
