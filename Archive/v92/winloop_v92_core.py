...
D = 3
BASE_V91_EPOCH42_COMPLETE_STATIC_STATES = 576
...
def _gc43_ok(phase, seventh_lineage_rotation, seventh_lineage_binding,
             ...
             seventh_source_handoff, seventh_source_binding, deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    trust = (
        seventh_lineage_rotation, seventh_lineage_binding, handed_proof_rebind,
        ...
        sixth_handoff_binding, sixth_lineage_binding, prior_handed_proof_rebind,
    ...
    if deadline_reset != 0:
        return False
    # V91 completed epoch 42 only after the rebound proof was handed to and
    # bound by a seventh source. Epoch 43 rotates and binds that seventh-source
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC43_EXPECTED.get(phase) == (
        seventh_lineage_rotation, seventh_lineage_binding,
...
def gc43():
    fixed = (2, 1, *([2] * 15), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC43_EXPECTED.items()]
    assert all(_gc43_ok(*x) for x in tails)
    z = q(41)
    seed = BASE_V91_EPOCH42_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(7, 22)]
    deadline_bad = sum(x[22] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc43_ok(0, 0, 0, 0, 0, *fixed),
        _gc43_ok(2, 2, 0, 0, 0, *fixed),
        _gc43_ok(4, 2, 2, 0, 0, *fixed),
        _gc43_ok(6, 2, 2, 2, 0, *fixed),
        _gc43_ok(8, 2, 2, 2, 2, *fixed),
        not _gc43_ok(8, 3, 2, 2, 2, *fixed),
        not _gc43_ok(8, 2, 3, 2, 2, *fixed),
        not _gc43_ok(8, 2, 2, 3, 2, *fixed),
        not _gc43_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc43_ok(8, 2, 2, 2, 2, *broken))

    # Phase, four epoch-43 transitions, carried root, continuity, fifteen
    # carried source/key/lineage/proof/handoff bindings, and deadline-reset bit.
    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** 15) * 2
    patterns = static_patterns * (4 ** 41) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch42_complete_seed_states': seed,
        'delay_vectors': 4 ** 41,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch43_seventh_lineage_rotation_states': rotation * z,
        'epoch43_bound_seventh_lineage_rotation_states': bound_rotation * z,
        'epoch43_seventh_lineage_binding_states': lineage_binding * z,
        'epoch43_bound_seventh_lineage_binding_states': bound_lineage_binding * z,
        'epoch43_handed_proof_rebind_states': proof_rebind * z,
        'epoch43_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch43_verifier_binding_states': verifier * z,
        'epoch43_bound_verifier_binding_states': bound_verifier * z,
        'epoch43_complete_states': complete * z,
        'unbound_or_conflicting_seventh_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_seventh_lineage_binding_acceptances': transition_bad[1] * z,
