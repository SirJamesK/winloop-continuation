...
BASE_V128_BOUND_FIFTY_THIRD_RESTART_STATIC_STATES = 27648
...
def _pub129_ok(phase, successor_disappearance, replacement_binding,
               dual, restart54, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart54,
        consume, quorum
    )):
        return False
    # V128's fifty-third restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V129 handles successor disappearance,
    # replacement binding, fresh reconciliation, and restart 54.
    if cache_authority != 0:
        return False
    return _PUB129_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart54,
        consume, quorum
    )


def successor_disappearance_fifty_fourth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB129_EXPECTED.items()
    ]
    assert all(_pub129_ok(*x) for x in tails)
    z = q(112)
    seed = BASE_V128_BOUND_FIFTY_THIRD_RESTART_STATIC_STATES
    ...
    restart54 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart54 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub129_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub129_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub129_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub129_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub129_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub129_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub129_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub129_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub129_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub129_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub129_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub129_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub129_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_fifty_third_restart_seed_states': seed,
        'delay_vectors': 4 ** 112,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'fifty_fourth_verifier_cold_restart_states': restart54 * z,
        'bound_fifty_fourth_restart_states': bound_restart54 * z,
        'bound_fifty_fourth_restart_recoveries': bound_recovery * z,
        'cached_fifty_fourth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_fifty_fourth_restart_acceptances': restart_bad * z,
