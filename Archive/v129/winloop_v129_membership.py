from winloop_v129_core import q

BASE_V128_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem129_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V128 completed the root-29 rollover with replacement/prior-source bindings
    # intact. V129 keeps generation 4 / root 29 fixed, rebinds the witness to
    # root 29, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 29 or target_root != 29:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM129_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root29_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 29, 29, 2, 1, 2, 2, 0)
        for phase, state in _MEM129_EXPECTED.items()
    ]
    assert all(_mem129_ok(*x) for x in tails)
    z = q(110)
    seed = BASE_V128_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 29 for x in tails) * seed
    target_root_regression = sum(x[6] < 29 for x in tails) * seed
    ...
    checks = [
        _mem129_ok(0, 0, 0, 0, 4, 29, 29, 2, 1, 2, 2, 0),
        _mem129_ok(2, 2, 0, 0, 4, 29, 29, 2, 1, 2, 2, 0),
        _mem129_ok(4, 2, 2, 0, 4, 29, 29, 2, 1, 2, 2, 0),
        _mem129_ok(6, 2, 2, 2, 4, 29, 29, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 3, 2, 2, 4, 29, 29, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 3, 2, 4, 29, 29, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 3, 4, 29, 29, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 3, 29, 29, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 28, 29, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 29, 28, 2, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 29, 29, 1, 1, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 29, 29, 2, 2, 2, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 29, 29, 2, 1, 3, 2, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 29, 29, 2, 1, 2, 3, 0),
        not _mem129_ok(6, 2, 2, 2, 4, 29, 29, 2, 1, 2, 2, 1),
    ]
    ...
    return {
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root29_witness_rebind_states': rebind * z,
        'bound_root29_witness_rebind_states': bound_rebind * z,
        'root29_witness_binding_states': witness_binding * z,
        'bound_root29_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root29_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root29_witness_binding_acceptances': witness_bad * z,
