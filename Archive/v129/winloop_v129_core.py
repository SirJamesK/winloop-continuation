...
D = 3
BASE_V128_EPOCH79_COMPLETE_STATIC_STATES = 576
...
def _gc80_ok(phase, twenty_sixth_source_handoff, twenty_sixth_source_binding,
             verifier_binding, carried_root_binding, continuity, *rest):
    if len(rest) != len(CARRIED_NAMES) + 1:
        return False
    *carried, deadline_reset = rest
    trust = (
        twenty_sixth_source_handoff, twenty_sixth_source_binding, verifier_binding,
        carried_root_binding, continuity, *carried,
    )
    if any(x == 3 for x in trust) or deadline_reset != 0:
        return False
    # V128 completes epoch 79 only after twenty-fifth-source lineage rotation,
    # lineage binding, handed-proof rebind, and verifier completion. Epoch 80
    # hands that rebound proof to a twenty-sixth source, binds it, and only then
    ...
    if carried_root_binding != 2 or continuity != 1 or any(x != 2 for x in carried):
        return False
    return _GC80_EXPECTED.get(phase) == (
        twenty_sixth_source_handoff, twenty_sixth_source_binding, verifier_binding
    )
...
def gc80():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC80_EXPECTED.items()]
    assert all(_gc80_ok(*x) for x in tails)
    z = q(115)
    seed = BASE_V128_EPOCH79_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc80_ok(0, 0, 0, 0, *fixed),
        _gc80_ok(2, 2, 0, 0, *fixed),
        _gc80_ok(4, 2, 2, 0, *fixed),
        _gc80_ok(6, 2, 2, 2, *fixed),
        not _gc80_ok(6, 3, 2, 2, *fixed),
        not _gc80_ok(6, 2, 3, 2, *fixed),
        not _gc80_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc80_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 115) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch79_complete_seed_states': seed,
        'delay_vectors': 4 ** 115,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch80_twenty_sixth_source_handoff_states': handoff * z,
        'epoch80_bound_twenty_sixth_source_handoff_states': bound_handoff * z,
        'epoch80_twenty_sixth_source_binding_states': source_binding_count * z,
        'epoch80_bound_twenty_sixth_source_binding_states': bound_source_binding * z,
        'epoch80_verifier_binding_states': verifier * z,
        'epoch80_bound_verifier_binding_states': bound_verifier * z,
        'epoch80_complete_states': complete * z,
        'unbound_or_conflicting_twenty_sixth_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twenty_sixth_source_binding_acceptances': transition_bad[1] * z,
