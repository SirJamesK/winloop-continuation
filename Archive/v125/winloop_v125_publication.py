...
BASE_V124_BOUND_FORTY_NINTH_RESTART_STATIC_STATES = 27648
...
def _pub125_ok(phase, successor_disappearance, replacement_binding,
               dual, restart50, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart50,
        consume, quorum
    )):
        return False
    # V124's forty-ninth restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V125 handles successor disappearance,
    # replacement binding, fresh reconciliation, and restart 50.
    if cache_authority != 0:
        return False
    return _PUB125_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart50,
        consume, quorum
    )


def successor_disappearance_fiftieth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB125_EXPECTED.items()
    ]
    assert all(_pub125_ok(*x) for x in tails)
    z = q(104)
    seed = BASE_V124_BOUND_FORTY_NINTH_RESTART_STATIC_STATES
    ...
    restart50 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart50 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub125_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub125_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub125_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub125_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub125_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub125_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub125_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub125_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub125_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub125_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub125_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub125_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub125_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_forty_ninth_restart_seed_states': seed,
        'delay_vectors': 4 ** 104,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'fiftieth_verifier_cold_restart_states': restart50 * z,
        'bound_fiftieth_restart_states': bound_restart50 * z,
        'bound_fiftieth_restart_recoveries': bound_recovery * z,
        'cached_fiftieth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_fiftieth_restart_acceptances': restart_bad * z,
