from winloop_v125_core import q

BASE_V124_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem125_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V124 completed the root-27 rollover with replacement/prior-source bindings
    # intact. V125 keeps generation 4 / root 27 fixed, rebinds the witness to
    # root 27, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 27 or target_root != 27:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM125_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root27_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 27, 27, 2, 1, 2, 2, 0)
        for phase, state in _MEM125_EXPECTED.items()
    ]
    assert all(_mem125_ok(*x) for x in tails)
    z = q(102)
    seed = BASE_V124_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 27 for x in tails) * seed
    target_root_regression = sum(x[6] < 27 for x in tails) * seed
    ...
    checks = [
        _mem125_ok(0, 0, 0, 0, 4, 27, 27, 2, 1, 2, 2, 0),
        _mem125_ok(2, 2, 0, 0, 4, 27, 27, 2, 1, 2, 2, 0),
        _mem125_ok(4, 2, 2, 0, 4, 27, 27, 2, 1, 2, 2, 0),
        _mem125_ok(6, 2, 2, 2, 4, 27, 27, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 3, 2, 2, 4, 27, 27, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 3, 2, 4, 27, 27, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 3, 4, 27, 27, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 3, 27, 27, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 27, 26, 2, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 27, 27, 1, 1, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 27, 27, 2, 2, 2, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 27, 27, 2, 1, 3, 2, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 27, 27, 2, 1, 2, 3, 0),
        not _mem125_ok(6, 2, 2, 2, 4, 27, 27, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 7 * (4 ** 3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 102) * z
    return {
        'patterns': patterns,
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root27_witness_rebind_states': rebind * z,
        'bound_root27_witness_rebind_states': bound_rebind * z,
        'root27_witness_binding_states': witness_binding * z,
        'bound_root27_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root27_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root27_witness_binding_acceptances': witness_bad * z,
