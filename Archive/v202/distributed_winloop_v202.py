"""WinLoop V202 exact continuation: epoch-153 sixty-second-lineage rotation, one-hundred-twenty-seventh cold restart, and root-66 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V202'
BASE_DIGEST = '452003633bf644ddc617ae166900bf18d19595410343e46eab3c2770070ded00'
BASE_IMPL_SHA = '1349a415a8419a85dde0bfcf3aa3590efff45f95b7ab6baa6d441b2885279628'
D = 3
...
def gc153():
    ...
    checks += [
        ...
        not ok(8,expected[8],carried=(2,)*322+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**261) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch152_complete_seed_states': seed,
        'delay_vectors': 4**261,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch153_bound_sixty_second_lineage_rotation_states': counts[0][1],
        'epoch153_bound_sixty_second_lineage_binding_states': counts[1][1],
        'epoch153_bound_handed_proof_rebind_states': counts[2][1],
        'epoch153_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication127():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**258) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_twenty_sixth_restart_seed_states': seed,
        'delay_vectors': 4**258,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_twenty_seventh_restart_states': counts[3][1],
        'bound_one_hundred_twenty_seventh_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership66():
    ...
    def ok(phase, state, generation=4, carried_root=65, target_root=66,
        ...
        return (
            ...
            and (generation, carried_root, target_root, replication, tombstone,
                 witness, prior_source, active_byzantine) == (4,65,66,2,1,2,2,0)
        )
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [
        not ok(10,(3,2,2,2,2)),
        ...
        not ok(10,expected[10],carried_root=64),
        not ok(10,expected[10],target_root=65),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**256) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root66_rollover_states': counts[2][1],
        'bound_root66_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc153(), publication127(), membership66()
    o = {
        'version': V,
        'base': {
            'version':'V201',
        ...
        'epoch153': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch152_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_sixty_second_lineage_rotation_states':t['epoch153_bound_sixty_second_lineage_rotation_states'],
            'bound_sixty_second_lineage_binding_states':t['epoch153_bound_sixty_second_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch153_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch153_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication127': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_twenty_sixth_restart_seed_states'],
            ...
            'bound_one_hundred_twenty_seventh_restart_states':s['bound_one_hundred_twenty_seventh_restart_states'],
            'bound_one_hundred_twenty_seventh_restart_recoveries':s['bound_one_hundred_twenty_seventh_restart_recoveries'],
        ...
        'membership66': {
            ...
            'bound_root66_rollover_states':b['bound_root66_rollover_states'],
            'bound_root66_binding_states':b['bound_root66_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 154 by handing the rebound proof to a sixty-third source, binding that source, and preserving the epoch-12 deadline',
            'compose one-hundred-twenty-seventh-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundred-twenty-eighth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-66 rollover, rebind the witness to root 66, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V202 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-153 GC to {t['accepted']:,} states with {t['epoch153_bound_sixty_second_lineage_rotation_states']:,} bound sixty-second-lineage rotations, "
        f"{t['epoch153_bound_sixty_second_lineage_binding_states']:,} bound lineage bindings, "
        f"{t['epoch153_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, and "
        f"{t['epoch153_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_twenty_seventh_restart_recoveries']:,} fully bound one-hundred-twenty-seventh-cold-restart recoveries; "
        ...
        f"{b['bound_root66_rollover_states']:,} bound root-66 rollovers, {b['bound_root66_binding_states']:,} bound root-66 bindings, "
