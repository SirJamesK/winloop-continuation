...
BASE_V126_BOUND_FIFTY_FIRST_RESTART_STATIC_STATES = 27648
...
def _pub127_ok(phase, successor_disappearance, replacement_binding,
               dual, restart52, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart52,
        consume, quorum
    )):
        return False
    # V126's fifty-first restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V127 handles successor disappearance,
    # replacement binding, fresh reconciliation, and restart 52.
    if cache_authority != 0:
        return False
    return _PUB127_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart52,
        consume, quorum
    )


def successor_disappearance_fifty_second_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB127_EXPECTED.items()
    ]
    assert all(_pub127_ok(*x) for x in tails)
    z = q(108)
    seed = BASE_V126_BOUND_FIFTY_FIRST_RESTART_STATIC_STATES
    ...
    restart52 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart52 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub127_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub127_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub127_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub127_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub127_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub127_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub127_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub127_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub127_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub127_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub127_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub127_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub127_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_fifty_first_restart_seed_states': seed,
        'delay_vectors': 4 ** 108,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'fifty_second_verifier_cold_restart_states': restart52 * z,
        'bound_fifty_second_restart_states': bound_restart52 * z,
        'bound_fifty_second_restart_recoveries': bound_recovery * z,
        'cached_fifty_second_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_fifty_second_restart_acceptances': restart_bad * z,
