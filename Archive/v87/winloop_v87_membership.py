...
BASE_V86_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem87_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        replacement_source_binding
    )):
        return False
    # V86 already completed the root-8 rollover. V87 keeps generation 4 and
    # root 8 fixed, rebinds the witness to that root, and only then permits a
    # new replication-quorum churn completion.
    if replacement_source_binding != 2:
        return False
    if generation != 4 or carried_root != 8 or target_root != 8:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM87_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root8_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 8, 8, 2, 1, 2, 0)
        for phase, state in _MEM87_EXPECTED.items()
    ]
    assert all(_mem87_ok(*x) for x in tails)
    z = q(26)
    seed = BASE_V86_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 8 for x in tails) * seed
    ...
    bad = (
        ...
        + tombstone_break + replacement_source_break + active_byz
    )

    checks = [
        _mem87_ok(0, 0, 0, 0, 4, 8, 8, 2, 1, 2, 0),
        _mem87_ok(2, 2, 0, 0, 4, 8, 8, 2, 1, 2, 0),
        _mem87_ok(4, 2, 2, 0, 4, 8, 8, 2, 1, 2, 0),
        _mem87_ok(6, 2, 2, 2, 4, 8, 8, 2, 1, 2, 0),
        not _mem87_ok(6, 3, 2, 2, 4, 8, 8, 2, 1, 2, 0),
        not _mem87_ok(6, 2, 3, 2, 4, 8, 8, 2, 1, 2, 0),
        not _mem87_ok(6, 2, 2, 3, 4, 8, 8, 2, 1, 2, 0),
        not _mem87_ok(6, 2, 2, 2, 3, 8, 8, 2, 1, 2, 0),
        not _mem87_ok(6, 2, 2, 2, 4, 7, 8, 2, 1, 2, 0),
        not _mem87_ok(6, 2, 2, 2, 4, 8, 7, 2, 1, 2, 0),
        not _mem87_ok(6, 2, 2, 2, 4, 8, 8, 1, 1, 2, 0),
        not _mem87_ok(6, 2, 2, 2, 4, 8, 8, 2, 2, 2, 0),
        not _mem87_ok(6, 2, 2, 2, 4, 8, 8, 2, 1, 3, 0),
        not _mem87_ok(6, 2, 2, 2, 4, 8, 8, 2, 1, 2, 1),
    ]

    # Phase, three transition coordinates, generation/root/replication domains,
    # tombstone/replacement-source bindings, and active-Byzantine bit.
    static_patterns = seed * 7 * (4 ** 3) * 6 * 9 * 9 * 4 * 3 * 4 * 2
    ...
    return {
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root8_witness_rebind_states': rebind * z,
        'bound_root8_witness_rebind_states': bound_rebind * z,
        'root8_witness_binding_states': witness_binding * z,
        'bound_root8_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root8_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root8_witness_binding_acceptances': witness_bad * z,
