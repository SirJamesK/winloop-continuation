"""WinLoop V148 exact continuation: epoch-99 thirty-fifth-lineage rotation, seventy-third cold restart, and root-39 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V148'
BASE_DIGEST = '72a40c8a509f48ddd3b72923cf81cccd00dd96d880c474208d3d60244ba0faab'
BASE_IMPL_SHA = '49c6f1fd6e79e43303b05e227d5f7580b14768cb9236ec5950fb00ad9743575f'
D = 3
...
def gc99():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*160+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**153) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch98_complete_seed_states': seed,
        'delay_vectors': 4**153,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch99_bound_thirty_fifth_lineage_rotation_states': counts[0][1],
        'epoch99_bound_thirty_fifth_lineage_binding_states': counts[1][1],
        'epoch99_bound_handed_proof_rebind_states': counts[2][1],
        'epoch99_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication73():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**150) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_second_restart_seed_states': seed,
        'delay_vectors': 4**150,
        'deadline_vectors': z,
        ...
        'bound_seventy_third_restart_states': counts[3][1],
        'bound_seventy_third_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership39():
    ...
    def ok(phase, state, generation=4, carried_root=38, target_root=39,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=37),
        not ok(10,expected[10],target_root=38),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**148) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root39_rollover_states': counts[2][1],
        'bound_root39_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc99(), publication73(), membership39()
    o = {
        'version': V,
        'base': {
            'version':'V147',
        ...
        'epoch99': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch98_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_fifth_lineage_rotation_states':t['epoch99_bound_thirty_fifth_lineage_rotation_states'],
            'bound_thirty_fifth_lineage_binding_states':t['epoch99_bound_thirty_fifth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch99_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch99_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication73': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_second_restart_seed_states'],
            ...
            'bound_seventy_third_restart_states':s['bound_seventy_third_restart_states'],
            'bound_seventy_third_restart_recoveries':s['bound_seventy_third_restart_recoveries'],
        ...
        'membership39': {
            ...
            'bound_root39_rollover_states':b['bound_root39_rollover_states'],
            'bound_root39_binding_states':b['bound_root39_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 100 by handing the rebound proof to a thirty-sixth source, binding that source, and preserving the epoch-12 deadline',
            'compose seventy-third-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a seventy-fourth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-39 rollover, rebind the witness to root 39, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V148 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-99 GC to {t['accepted']:,} states with {t['epoch99_bound_thirty_fifth_lineage_rotation_states']:,} bound thirty-fifth-lineage rotations, "
        f"{t['epoch99_bound_thirty_fifth_lineage_binding_states']:,} bound lineage bindings, {t['epoch99_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch99_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_seventy_third_restart_recoveries']:,} fully bound seventy-third-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root39_rollover_states']:,} bound root-39 rollovers, "
        f"{b['bound_root39_binding_states']:,} bound root-39 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
