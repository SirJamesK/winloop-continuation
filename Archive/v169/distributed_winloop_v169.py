"""WinLoop V169 exact continuation: epoch-120 forty-sixth-source handoff, ninety-fourth cold restart, and root-49 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V169'
BASE_DIGEST = '2d048fd162c2e32fcb3d5c989ac231da522fb23c0fa2673737d492ca0413e0a8'
BASE_IMPL_SHA = 'bda2c17270c8e18467fd8081e630cc3248a0aab7c888f2088915a543aa4ea118'
D = 3
...
def gc120():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*223+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**195) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch119_complete_seed_states': seed,
        'delay_vectors': 4**195,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch120_bound_forty_sixth_source_handoff_states': counts[0][1],
        'epoch120_bound_forty_sixth_source_binding_states': counts[1][1],
        'epoch120_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication94():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**192) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_third_restart_seed_states': seed,
        'delay_vectors': 4**192,
        'deadline_vectors': z,
        ...
        'bound_ninety_fourth_restart_states': counts[3][1],
        'bound_ninety_fourth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership49():
    ...
    def ok(phase, state, generation=4, carried_root=49, target_root=49,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=48),
        not ok(6,expected[6],target_root=48),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**190) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**190,
        'deadline_vectors': z,
        'bound_root49_witness_rebind_states': counts[0][1],
        'bound_root49_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc120(), publication94(), membership49()
    o = {
        'version': V,
        'base': {
            'version':'V168',
        ...
        'epoch120': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch119_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_sixth_source_handoff_states':t['epoch120_bound_forty_sixth_source_handoff_states'],
            'bound_forty_sixth_source_binding_states':t['epoch120_bound_forty_sixth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch120_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication94': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_third_restart_seed_states'],
            ...
            'bound_ninety_fourth_restart_states':s['bound_ninety_fourth_restart_states'],
            'bound_ninety_fourth_restart_recoveries':s['bound_ninety_fourth_restart_recoveries'],
        ...
        'membership49': {
            ...
            'bound_root49_witness_rebind_states':b['bound_root49_witness_rebind_states'],
            'bound_root49_witness_binding_states':b['bound_root49_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 121 by rotating the forty-sixth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose ninety-fourth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a ninety-fifth verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-49 witness rebind, replace the witness source, roll to root 50, bind root 50, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V169 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-120 GC to {t['accepted']:,} states with {t['epoch120_bound_forty_sixth_source_handoff_states']:,} bound forty-sixth-source handoffs, "
        f"{t['epoch120_bound_forty_sixth_source_binding_states']:,} bound forty-sixth-source bindings, and {t['epoch120_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_ninety_fourth_restart_recoveries']:,} fully bound ninety-fourth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root49_witness_rebind_states']:,} bound root-49 witness rebinds, "
        f"{b['bound_root49_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
