"""WinLoop V174 exact continuation: epoch-125 forty-eighth-lineage rotation, ninety-ninth cold restart, and root-52 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V174'
BASE_DIGEST = '2f6217f31a28daba6c95abd5eee7e5832c31bf851603e209de326a9a8964cfd5'
BASE_IMPL_SHA = 'c7a4bca68c9413e03908cce5817484dc42f60c12cabb1474a1d3ea508b041ee6'
D = 3
...
def gc125():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*238+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**205) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch124_complete_seed_states': seed,
        'delay_vectors': 4**205,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch125_bound_forty_eighth_lineage_rotation_states': counts[0][1],
        'epoch125_bound_forty_eighth_lineage_binding_states': counts[1][1],
        'epoch125_bound_handed_proof_rebind_states': counts[2][1],
        'epoch125_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication99():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**202) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_eighth_restart_seed_states': seed,
        'delay_vectors': 4**202,
        'deadline_vectors': z,
        ...
        'bound_ninety_ninth_restart_states': counts[3][1],
        'bound_ninety_ninth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership52():
    ...
    def ok(phase, state, generation=4, carried_root=51, target_root=52,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=50),
        not ok(10,expected[10],target_root=51),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**200) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root52_rollover_states': counts[2][1],
        'bound_root52_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc125(), publication99(), membership52()
    o = {
        'version': V,
        'base': {
            'version':'V173',
        ...
        'epoch125': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch124_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_eighth_lineage_rotation_states':t['epoch125_bound_forty_eighth_lineage_rotation_states'],
            'bound_forty_eighth_lineage_binding_states':t['epoch125_bound_forty_eighth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch125_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch125_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication99': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_eighth_restart_seed_states'],
            ...
            'bound_ninety_ninth_restart_states':s['bound_ninety_ninth_restart_states'],
            'bound_ninety_ninth_restart_recoveries':s['bound_ninety_ninth_restart_recoveries'],
        ...
        'membership52': {
            ...
            'bound_root52_rollover_states':b['bound_root52_rollover_states'],
            'bound_root52_binding_states':b['bound_root52_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 126 by handing the rebound proof to a forty-ninth source, binding that source, and preserving the epoch-12 deadline',
            'compose ninety-ninth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a one-hundredth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-52 rollover, rebind the witness to root 52, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V174 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-125 GC to {t['accepted']:,} states with {t['epoch125_bound_forty_eighth_lineage_rotation_states']:,} bound forty-eighth-lineage rotations, "
        f"{t['epoch125_bound_forty_eighth_lineage_binding_states']:,} bound lineage bindings, {t['epoch125_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch125_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_ninety_ninth_restart_recoveries']:,} fully bound ninety-ninth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root52_rollover_states']:,} bound root-52 rollovers, "
        f"{b['bound_root52_binding_states']:,} bound root-52 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
