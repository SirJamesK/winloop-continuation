...
D = 3
BASE_V101_EPOCH52_COMPLETE_STATIC_STATES = 576
...
def _gc53_ok(phase, twelfth_lineage_rotation, twelfth_lineage_binding,
             ...
             twelfth_source_binding, deadline_reset):
    ...
    if deadline_reset != 0:
        return False
    # V101 completed epoch 52 only after the rebound proof was handed to and
    # bound by a twelfth source. Epoch 53 rotates and binds that source lineage,
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC53_EXPECTED.get(phase) == (
        twelfth_lineage_rotation, twelfth_lineage_binding,
...
def gc53():
    fixed = (2, 1, *([2] * 38), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC53_EXPECTED.items()]
    assert all(_gc53_ok(*x) for x in tails)
    z = q(61)
    seed = BASE_V101_EPOCH52_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(7, 45)]
    deadline_bad = sum(x[45] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc53_ok(0, 0, 0, 0, 0, *fixed),
        _gc53_ok(2, 2, 0, 0, 0, *fixed),
        _gc53_ok(4, 2, 2, 0, 0, *fixed),
        _gc53_ok(6, 2, 2, 2, 0, *fixed),
        _gc53_ok(8, 2, 2, 2, 2, *fixed),
        not _gc53_ok(8, 3, 2, 2, 2, *fixed),
        not _gc53_ok(8, 2, 3, 2, 2, *fixed),
        not _gc53_ok(8, 2, 2, 3, 2, *fixed),
        not _gc53_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc53_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** 38) * 2
    patterns = static_patterns * (4 ** 61) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch52_complete_seed_states': seed,
        'delay_vectors': 4 ** 61,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch53_twelfth_lineage_rotation_states': rotation * z,
        'epoch53_bound_twelfth_lineage_rotation_states': bound_rotation * z,
        'epoch53_twelfth_lineage_binding_states': lineage_binding * z,
        'epoch53_bound_twelfth_lineage_binding_states': bound_lineage_binding * z,
        'epoch53_handed_proof_rebind_states': proof_rebind * z,
        'epoch53_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch53_verifier_binding_states': verifier * z,
        'epoch53_bound_verifier_binding_states': bound_verifier * z,
        'epoch53_complete_states': complete * z,
        'unbound_or_conflicting_twelfth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twelfth_lineage_binding_acceptances': transition_bad[1] * z,
