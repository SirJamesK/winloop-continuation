"""WinLoop V136 exact continuation: epoch-87 twenty-ninth-lineage rotation, sixty-first cold restart, and root-33 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V136'
BASE_DIGEST = '24a02867f79fe8c637a67ec8608c352acf18a1ad3f4cd47b03147651ed8b83a9'
BASE_IMPL_SHA = '3a336dc1fd8c4112000ca1ea1d8bfe734bcc92cc03931c72da35830df4503c27'
D = 3
...
def gc87():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*124+(1,)),
        not ok(8, expected[8], deadline_reset=1),
    ]
    z = q(129); seed = 576
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**129) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'epoch86_complete_seed_states': seed,
        'delay_vectors': 4**129, 'deadline_vectors': z, 'deadline_origin': 'epoch12',
        'epoch87_bound_twenty_ninth_lineage_rotation_states': counts[0][1],
        'epoch87_bound_twenty_ninth_lineage_binding_states': counts[1][1],
        'epoch87_bound_handed_proof_rebind_states': counts[2][1],
        'epoch87_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0, 'checks': checks,
    }
...
def publication61():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**126) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'bound_sixtieth_restart_seed_states': seed,
        'delay_vectors': 4**126, 'deadline_vectors': z,
        ...
        'bound_sixty_first_restart_states': counts[3][1],
        'bound_sixty_first_restart_recoveries': seed * z,
        'bad_acceptances': 0, 'checks': checks,
    }
...
def membership33():
    ...
    def ok(phase, state, generation=4, carried_root=32, target_root=33,
           replication=2, tombstone=1, witness=2, prior_source=2,
           active_byzantine=0):
        return (
            0 <= phase < len(expected) and state == expected[phase]
            and all(x != 3 for x in state) and generation == 4
            and carried_root == 32 and target_root == 33 and replication == 2
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=31), not ok(10,expected[10],target_root=32),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**124) * z
    return {
        ...
        'bound_root33_rollover_states': counts[2][1],
        'bound_root33_binding_states': counts[3][1],
        'bound_replication_quorum_churn_states': counts[4][1],
        'bad_acceptances': 0, 'checks': checks,
    }

def run_validation():
    c, t, s, b = indep(), gc87(), publication61(), membership33()
    o = {
        'version': V,
        'base': {'version':'V135','digest':BASE_DIGEST,'implementation_sha256':BASE_IMPL_SHA},
        ...
        'epoch87': {
            'patterns':t['patterns'],'accepted':t['accepted'],'seed_states':t['epoch86_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],'deadline_vectors':t['deadline_vectors'],'deadline_origin':t['deadline_origin'],
            'bound_twenty_ninth_lineage_rotation_states':t['epoch87_bound_twenty_ninth_lineage_rotation_states'],
            'bound_twenty_ninth_lineage_binding_states':t['epoch87_bound_twenty_ninth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch87_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch87_bound_verifier_binding_states'],'bad_acceptances':t['bad_acceptances'],
        },
        'publication61': {
            'patterns':s['patterns'],'accepted':s['accepted'],'seed_states':s['bound_sixtieth_restart_seed_states'],
            ...
            'bound_sixty_first_restart_states':s['bound_sixty_first_restart_states'],
            'bound_sixty_first_restart_recoveries':s['bound_sixty_first_restart_recoveries'],'bad_acceptances':s['bad_acceptances'],
        },
        'membership33': {
            ...
            'bound_root33_rollover_states':b['bound_root33_rollover_states'],
            'bound_root33_binding_states':b['bound_root33_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 88 by handing the rebound proof to a thirtieth source, binding that source, and preserving the epoch-12 deadline',
            'compose sixty-first-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a sixty-second verifier cold restart without cached authority promotion',
            'keep generation 4 after root-33 rollover, rebind the witness to root 33, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V136 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-87 GC to {t['accepted']:,} states with {t['epoch87_bound_twenty_ninth_lineage_rotation_states']:,} bound twenty-ninth-lineage rotations, "
        f"{t['epoch87_bound_twenty_ninth_lineage_binding_states']:,} bound lineage bindings, {t['epoch87_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch87_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_sixty_first_restart_recoveries']:,} fully bound sixty-first-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root33_rollover_states']:,} bound root-33 rollovers, "
        f"{b['bound_root33_binding_states']:,} bound root-33 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
