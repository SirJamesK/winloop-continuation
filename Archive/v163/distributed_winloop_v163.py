"""WinLoop V163 exact continuation: epoch-114 forty-third-source handoff, eighty-eighth cold restart, and root-46 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V163'
BASE_DIGEST = '9727195879013d29b8f45b7a1388ec5d1e45209e0b2e8bd4fafca0ef95acd5b9'
BASE_IMPL_SHA = 'ade370577361937e46044d282c7a5026bd745cb2ff1ba67b35482833b1d95fba'
D = 3
...
def gc114():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*205+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**183) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch113_complete_seed_states': seed,
        'delay_vectors': 4**183,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch114_bound_forty_third_source_handoff_states': counts[0][1],
        'epoch114_bound_forty_third_source_binding_states': counts[1][1],
        'epoch114_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication88():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**180) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_seventh_restart_seed_states': seed,
        'delay_vectors': 4**180,
        'deadline_vectors': z,
        ...
        'bound_eighty_eighth_restart_states': counts[3][1],
        'bound_eighty_eighth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership46():
    ...
    def ok(phase, state, generation=4, carried_root=46, target_root=46,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=45),
        not ok(6,expected[6],target_root=45),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**178) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**178,
        'deadline_vectors': z,
        'bound_root46_witness_rebind_states': counts[0][1],
        'bound_root46_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc114(), publication88(), membership46()
    o = {
        'version': V,
        'base': {
            'version':'V162',
        ...
        'epoch114': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch113_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_third_source_handoff_states':t['epoch114_bound_forty_third_source_handoff_states'],
            'bound_forty_third_source_binding_states':t['epoch114_bound_forty_third_source_binding_states'],
            'bound_verifier_binding_states':t['epoch114_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication88': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_seventh_restart_seed_states'],
            ...
            'bound_eighty_eighth_restart_states':s['bound_eighty_eighth_restart_states'],
            'bound_eighty_eighth_restart_recoveries':s['bound_eighty_eighth_restart_recoveries'],
        ...
        'membership46': {
            ...
            'bound_root46_witness_rebind_states':b['bound_root46_witness_rebind_states'],
            'bound_root46_witness_binding_states':b['bound_root46_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 115 by rotating the forty-third-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose eighty-eighth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and an eighty-ninth verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-46 witness rebind, replace the witness source, roll to root 47, bind root 47, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V163 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-114 GC to {t['accepted']:,} states with {t['epoch114_bound_forty_third_source_handoff_states']:,} bound forty-third-source handoffs, "
        f"{t['epoch114_bound_forty_third_source_binding_states']:,} bound forty-third-source bindings, and {t['epoch114_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_eighty_eighth_restart_recoveries']:,} fully bound eighty-eighth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root46_witness_rebind_states']:,} bound root-46 witness rebinds, "
        f"{b['bound_root46_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
