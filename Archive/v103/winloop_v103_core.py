...
D = 3
BASE_V102_EPOCH53_COMPLETE_STATIC_STATES = 576
...
def _gc54_ok(phase, thirteenth_source_handoff, thirteenth_source_binding,
    ...
    trust = (
        thirteenth_source_handoff, thirteenth_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V102 completed epoch 53 only after the twelfth-source lineage was rotated
    # and bound and the handed proof was rebound. Epoch 54 hands that rebound
    # proof to a thirteenth source, binds it, and then permits verifier completion.
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC54_EXPECTED.get(phase) == (
        thirteenth_source_handoff, thirteenth_source_binding, verifier_binding
    )
...
def gc54():
    fixed = (2, 1, *([2] * 41), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC54_EXPECTED.items()]
    assert all(_gc54_ok(*x) for x in tails)
    z = q(63)
    seed = BASE_V102_EPOCH53_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(6, 47)]
    deadline_bad = sum(x[47] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc54_ok(0, 0, 0, 0, *fixed),
        _gc54_ok(2, 2, 0, 0, *fixed),
        _gc54_ok(4, 2, 2, 0, *fixed),
        _gc54_ok(6, 2, 2, 2, *fixed),
        not _gc54_ok(6, 3, 2, 2, *fixed),
        not _gc54_ok(6, 2, 3, 2, *fixed),
        not _gc54_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc54_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** 41) * 2
    patterns = static_patterns * (4 ** 63) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch53_complete_seed_states': seed,
        'delay_vectors': 4 ** 63,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch54_thirteenth_source_handoff_states': handoff * z,
        'epoch54_bound_thirteenth_source_handoff_states': bound_handoff * z,
        'epoch54_thirteenth_source_binding_states': source_binding_count * z,
        'epoch54_bound_thirteenth_source_binding_states': bound_source_binding * z,
        'epoch54_verifier_binding_states': verifier * z,
        'epoch54_bound_verifier_binding_states': bound_verifier * z,
        'epoch54_complete_states': complete * z,
        'unbound_or_conflicting_thirteenth_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_thirteenth_source_binding_acceptances': transition_bad[1] * z,
