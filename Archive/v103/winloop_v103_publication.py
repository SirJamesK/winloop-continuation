...
BASE_V102_BOUND_TWENTY_SEVENTH_RESTART_STATIC_STATES = 27648
...
def _pub103_ok(phase, successor_disappearance, replacement_binding,
               dual, restart28, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart28,
        consume, quorum
    )):
        return False
    # V102's twenty-seventh restart is a seed only after replacement-source
    # churn, successor binding, and reconciliation. V103 handles successor
    # disappearance, replacement binding, fresh reconciliation, and restart 28.
    if cache_authority != 0:
        return False
    return _PUB103_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart28,
        consume, quorum
    )


def successor_disappearance_twenty_eighth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB103_EXPECTED.items()
    ]
    assert all(_pub103_ok(*x) for x in tails)
    z = q(60)
    seed = BASE_V102_BOUND_TWENTY_SEVENTH_RESTART_STATIC_STATES
    ...
    restart28 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart28 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub103_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub103_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub103_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub103_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub103_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub103_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub103_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub103_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub103_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub103_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub103_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub103_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub103_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_twenty_seventh_restart_seed_states': seed,
        'delay_vectors': 4 ** 60,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'twenty_eighth_verifier_cold_restart_states': restart28 * z,
        'bound_twenty_eighth_restart_states': bound_restart28 * z,
        'bound_twenty_eighth_restart_recoveries': bound_recovery * z,
        'cached_twenty_eighth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_twenty_eighth_restart_acceptances': restart_bad * z,
