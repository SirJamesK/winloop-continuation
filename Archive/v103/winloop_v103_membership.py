from winloop_v103_core import q

BASE_V102_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem103_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V102 completed the root-16 rollover with replacement/prior-source bindings
    # intact. V103 keeps generation 4 / root 16 fixed, rebinds the witness to
    # root 16, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 16 or target_root != 16:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM103_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root16_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 16, 16, 2, 1, 2, 2, 0)
        for phase, state in _MEM103_EXPECTED.items()
    ]
    assert all(_mem103_ok(*x) for x in tails)
    z = q(58)
    seed = BASE_V102_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 16 for x in tails) * seed
    target_root_regression = sum(x[6] < 16 for x in tails) * seed
    ...
    checks = [
        _mem103_ok(0, 0, 0, 0, 4, 16, 16, 2, 1, 2, 2, 0),
        _mem103_ok(2, 2, 0, 0, 4, 16, 16, 2, 1, 2, 2, 0),
        _mem103_ok(4, 2, 2, 0, 4, 16, 16, 2, 1, 2, 2, 0),
        _mem103_ok(6, 2, 2, 2, 4, 16, 16, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 3, 2, 2, 4, 16, 16, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 3, 2, 4, 16, 16, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 3, 4, 16, 16, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 3, 16, 16, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 15, 16, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 16, 15, 2, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 16, 16, 1, 1, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 16, 16, 2, 2, 2, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 16, 16, 2, 1, 3, 2, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 16, 16, 2, 1, 2, 3, 0),
        not _mem103_ok(6, 2, 2, 2, 4, 16, 16, 2, 1, 2, 2, 1),
    ]
    ...
    return {
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root16_witness_rebind_states': rebind * z,
        'bound_root16_witness_rebind_states': bound_rebind * z,
        'root16_witness_binding_states': witness_binding * z,
        'bound_root16_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root16_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root16_witness_binding_acceptances': witness_bad * z,
