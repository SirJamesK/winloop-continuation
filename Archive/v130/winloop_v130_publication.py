...
BASE_V129_BOUND_FIFTY_FOURTH_RESTART_STATIC_STATES = 27648
...
def _pub130_ok(phase, replacement_churn, successor_binding,
               dual, restart55, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        replacement_churn, successor_binding, dual, restart55, consume, quorum
    )):
        return False
    # V129's fifty-fourth restart is a seed only after successor disappearance,
    # replacement binding, and reconciliation. V130 requires replacement-source
    # churn, successor binding, fresh reconciliation, and restart 55.
    if cache_authority != 0:
        return False
    return _PUB130_EXPECTED.get(phase) == (
        replacement_churn, successor_binding, dual, restart55, consume, quorum
    )


def replacement_churn_fifty_fifth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB130_EXPECTED.items()
    ]
    assert all(_pub130_ok(*x) for x in tails)
    z = q(114)
    seed = BASE_V129_BOUND_FIFTY_FOURTH_RESTART_STATIC_STATES
    ...
    restart55 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart55 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub130_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub130_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub130_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub130_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub130_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub130_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub130_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub130_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub130_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub130_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub130_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub130_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub130_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_fifty_fourth_restart_seed_states': seed,
        'delay_vectors': 4 ** 114, 'deadline_vectors': z, 'shared_deadline': 3,
        ...
        'fifty_fifth_verifier_cold_restart_states': restart55 * z,
        'bound_fifty_fifth_restart_states': bound_restart55 * z,
        'bound_fifty_fifth_restart_recoveries': bound_recovery * z,
        'cached_fifty_fifth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_fifty_fifth_restart_acceptances': restart_bad * z,
