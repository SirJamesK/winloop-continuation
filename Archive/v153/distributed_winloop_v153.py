"""WinLoop V153 exact continuation: epoch-104 thirty-eighth-source handoff, seventy-eighth cold restart, and root-41 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V153'
BASE_DIGEST = '17430082bf575ea4328babf4df75a1b346e3fedac8fda5157d7b50d55d539658'
BASE_IMPL_SHA = 'e6403ea2b35412c4e5137f6b5d5fe369ca6282b9319f1b16ed8963e544a57ccf'
D = 3
...
def gc104():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*175+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**163) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch103_complete_seed_states': seed,
        'delay_vectors': 4**163,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch104_bound_thirty_eighth_source_handoff_states': counts[0][1],
        'epoch104_bound_thirty_eighth_source_binding_states': counts[1][1],
        'epoch104_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication78():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**160) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_seventh_restart_seed_states': seed,
        'delay_vectors': 4**160,
        'deadline_vectors': z,
        ...
        'bound_seventy_eighth_restart_states': counts[3][1],
        'bound_seventy_eighth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership41():
    ...
    def ok(phase, state, generation=4, carried_root=41, target_root=41,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=40),
        not ok(6,expected[6],target_root=40),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**158) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**158,
        'deadline_vectors': z,
        'bound_root41_witness_rebind_states': counts[0][1],
        'bound_root41_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc104(), publication78(), membership41()
    o = {
        'version': V,
        'base': {
            'version':'V152',
        ...
        'epoch104': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch103_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_eighth_source_handoff_states':t['epoch104_bound_thirty_eighth_source_handoff_states'],
            'bound_thirty_eighth_source_binding_states':t['epoch104_bound_thirty_eighth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch104_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication78': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_seventh_restart_seed_states'],
            ...
            'bound_seventy_eighth_restart_states':s['bound_seventy_eighth_restart_states'],
            'bound_seventy_eighth_restart_recoveries':s['bound_seventy_eighth_restart_recoveries'],
        ...
        'membership41': {
            ...
            'bound_root41_witness_rebind_states':b['bound_root41_witness_rebind_states'],
            'bound_root41_witness_binding_states':b['bound_root41_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 105 by rotating the thirty-eighth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose seventy-eighth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a seventy-ninth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-41 witness rebind, replace the witness source, roll to root 42, bind root 42, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V153 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-104 GC to {t['accepted']:,} states with {t['epoch104_bound_thirty_eighth_source_handoff_states']:,} bound thirty-eighth-source handoffs, "
        f"{t['epoch104_bound_thirty_eighth_source_binding_states']:,} bound thirty-eighth-source bindings, and {t['epoch104_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_seventy_eighth_restart_recoveries']:,} fully bound seventy-eighth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root41_witness_rebind_states']:,} bound root-41 witness rebinds, "
        f"{b['bound_root41_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
