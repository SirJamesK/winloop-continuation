from winloop_v110_core import q

BASE_V109_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem110_ok(phase, witness_source_replacement, replacement_source_binding,
               root20_rollover, root20_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root20_rollover, root20_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V109 leaves a rebound witness current at generation 4 / root 19. V110
    # binds a new witness source, rolls to root 20, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 19 or target_root != 20:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM110_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root20_rollover, root20_binding, quorum_churn
    )


def root20_rollover_after_root19_witness_source_replacement():
    tails = [
        (phase, *state, 4, 19, 20, 2, 1, 2, 2, 0)
        for phase, state in _MEM110_EXPECTED.items()
    ]
    assert all(_mem110_ok(*x) for x in tails)
    z = q(72)
    seed = BASE_V109_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 19 for x in tails) * seed
    target_root_regression = sum(x[8] < 20 for x in tails) * seed
    ...
    checks = [
        _mem110_ok(0, 0, 0, 0, 0, 0, 4, 19, 20, 2, 1, 2, 2, 0),
        _mem110_ok(2, 2, 0, 0, 0, 0, 4, 19, 20, 2, 1, 2, 2, 0),
        _mem110_ok(4, 2, 2, 0, 0, 0, 4, 19, 20, 2, 1, 2, 2, 0),
        _mem110_ok(6, 2, 2, 2, 0, 0, 4, 19, 20, 2, 1, 2, 2, 0),
        _mem110_ok(8, 2, 2, 2, 2, 0, 4, 19, 20, 2, 1, 2, 2, 0),
        _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 3, 2, 2, 2, 2, 4, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 3, 2, 2, 2, 4, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 3, 2, 2, 4, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 3, 2, 4, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 3, 4, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 3, 19, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 18, 20, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 19, 2, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 20, 1, 1, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 20, 2, 2, 2, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 20, 2, 1, 1, 2, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 20, 2, 1, 2, 3, 0),
        not _mem110_ok(10, 2, 2, 2, 2, 2, 4, 19, 20, 2, 1, 2, 2, 1),
    ]
    ...
    return {
        ...
        'root20_rollover_states': rollover * z,
        'bound_root20_rollover_states': bound_rollover * z,
        'root20_binding_states': root_binding * z,
        'bound_root20_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root20_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root20_binding_acceptances': root_binding_bad * z,
