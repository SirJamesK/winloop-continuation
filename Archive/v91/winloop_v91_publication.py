...
BASE_V90_BOUND_FIFTEENTH_RESTART_STATIC_STATES = 27648
...
def _pub91_ok(phase, successor_disappearance, replacement_binding,
              dual, restart16, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart16,
        consume, quorum
    )):
        return False
    # V90's fifteenth restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V91 then handles successor-source
    # disappearance, replacement binding, fresh reconciliation, and restart 16.
    if cache_authority != 0:
        return False
    return _PUB91_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart16,
        consume, quorum
    )


def successor_disappearance_sixteenth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB91_EXPECTED.items()
    ]
    assert all(_pub91_ok(*x) for x in tails)
    z = q(36)
    seed = BASE_V90_BOUND_FIFTEENTH_RESTART_STATIC_STATES
    ...
    restart16 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart16 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub91_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub91_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub91_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub91_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub91_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub91_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub91_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub91_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub91_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub91_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub91_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub91_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub91_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_fifteenth_restart_seed_states': seed,
        'delay_vectors': 4 ** 36,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'sixteenth_verifier_cold_restart_states': restart16 * z,
        'bound_sixteenth_restart_states': bound_restart16 * z,
        'bound_sixteenth_restart_recoveries': bound_recovery * z,
        'cached_sixteenth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_sixteenth_restart_acceptances': restart_bad * z,
