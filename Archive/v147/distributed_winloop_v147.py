"""WinLoop V147 exact continuation: epoch-98 thirty-fifth-source handoff, seventy-second cold restart, and root-38 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V147'
BASE_DIGEST = '0b334216ca86ccf3156d201e0aa8543ed412a0dba574c75accef95f406670cda'
BASE_IMPL_SHA = 'a426e470d9713f81ca1bac0346c4df3ffbba44d4cc4cf74f5e4e0012421ba2d6'
D = 3
...
def gc98():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*157+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**151) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch97_complete_seed_states': seed,
        'delay_vectors': 4**151,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch98_bound_thirty_fifth_source_handoff_states': counts[0][1],
        'epoch98_bound_thirty_fifth_source_binding_states': counts[1][1],
        'epoch98_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication72():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**148) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_first_restart_seed_states': seed,
        'delay_vectors': 4**148,
        'deadline_vectors': z,
        ...
        'bound_seventy_second_restart_states': counts[3][1],
        'bound_seventy_second_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership38():
    ...
    def ok(phase, state, generation=4, carried_root=38, target_root=38,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=37),
        not ok(6,expected[6],target_root=37),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**146) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**146,
        'deadline_vectors': z,
        'bound_root38_witness_rebind_states': counts[0][1],
        'bound_root38_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc98(), publication72(), membership38()
    o = {
        'version': V,
        'base': {
            'version':'V146',
        ...
        'epoch98': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch97_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_fifth_source_handoff_states':t['epoch98_bound_thirty_fifth_source_handoff_states'],
            'bound_thirty_fifth_source_binding_states':t['epoch98_bound_thirty_fifth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch98_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication72': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_first_restart_seed_states'],
            ...
            'bound_seventy_second_restart_states':s['bound_seventy_second_restart_states'],
            'bound_seventy_second_restart_recoveries':s['bound_seventy_second_restart_recoveries'],
        ...
        'membership38': {
            ...
            'bound_root38_witness_rebind_states':b['bound_root38_witness_rebind_states'],
            'bound_root38_witness_binding_states':b['bound_root38_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 99 by rotating the thirty-fifth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose seventy-second-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a seventy-third verifier cold restart without cached authority promotion',
            'keep generation 4 after root-38 witness rebind, replace the witness source, roll to root 39, bind root 39, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V147 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-98 GC to {t['accepted']:,} states with {t['epoch98_bound_thirty_fifth_source_handoff_states']:,} bound thirty-fifth-source handoffs, "
        f"{t['epoch98_bound_thirty_fifth_source_binding_states']:,} bound thirty-fifth-source bindings, and {t['epoch98_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_seventy_second_restart_recoveries']:,} fully bound seventy-second-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root38_witness_rebind_states']:,} bound root-38 witness rebinds, "
        f"{b['bound_root38_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
