"""WinLoop V156 exact continuation: epoch-107 thirty-ninth-lineage rotation, eighty-first cold restart, and root-43 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V156'
BASE_DIGEST = 'a2f8a8f3cfe1397b12cbb3367fc4c8ea148cfc863ed4be9b4e4fb6765677047e'
BASE_IMPL_SHA = 'c81688192fbea0611d688e2ef2e01cfbf972fbe7f2e71f2bc4b384e6d930a230'
D = 3
...
def gc107():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*184+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**169) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch106_complete_seed_states': seed,
        'delay_vectors': 4**169,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch107_bound_thirty_ninth_lineage_rotation_states': counts[0][1],
        'epoch107_bound_thirty_ninth_lineage_binding_states': counts[1][1],
        'epoch107_bound_handed_proof_rebind_states': counts[2][1],
        'epoch107_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication81():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**166) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eightieth_restart_seed_states': seed,
        'delay_vectors': 4**166,
        'deadline_vectors': z,
        ...
        'bound_eighty_first_restart_states': counts[3][1],
        'bound_eighty_first_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership43():
    ...
    def ok(phase, state, generation=4, carried_root=42, target_root=43,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=41),
        not ok(10,expected[10],target_root=42),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**164) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root43_rollover_states': counts[2][1],
        'bound_root43_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc107(), publication81(), membership43()
    o = {
        'version': V,
        'base': {
            'version':'V155',
        ...
        'epoch107': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch106_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_ninth_lineage_rotation_states':t['epoch107_bound_thirty_ninth_lineage_rotation_states'],
            'bound_thirty_ninth_lineage_binding_states':t['epoch107_bound_thirty_ninth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch107_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch107_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication81': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eightieth_restart_seed_states'],
            ...
            'bound_eighty_first_restart_states':s['bound_eighty_first_restart_states'],
            'bound_eighty_first_restart_recoveries':s['bound_eighty_first_restart_recoveries'],
        ...
        'membership43': {
            ...
            'bound_root43_rollover_states':b['bound_root43_rollover_states'],
            'bound_root43_binding_states':b['bound_root43_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 108 by handing the rebound proof to a fortieth source, binding that source, and preserving the epoch-12 deadline',
            'compose eighty-first-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and an eighty-second verifier cold restart without cached authority promotion',
            'keep generation 4 after root-43 rollover, rebind the witness to root 43, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V156 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-107 GC to {t['accepted']:,} states with {t['epoch107_bound_thirty_ninth_lineage_rotation_states']:,} bound thirty-ninth-lineage rotations, "
        f"{t['epoch107_bound_thirty_ninth_lineage_binding_states']:,} bound lineage bindings, {t['epoch107_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch107_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_eighty_first_restart_recoveries']:,} fully bound eighty-first-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root43_rollover_states']:,} bound root-43 rollovers, "
        f"{b['bound_root43_binding_states']:,} bound root-43 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
