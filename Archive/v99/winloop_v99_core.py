...
D = 3
BASE_V98_EPOCH49_COMPLETE_STATIC_STATES = 576
...
def _gc50_ok(phase, eleventh_source_handoff, eleventh_source_binding,
    ...
    trust = (
        eleventh_source_handoff, eleventh_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V98 completed epoch 49 only after tenth-lineage rotation/binding and a
    # fresh handed-proof rebind. Epoch 50 hands that proof to an eleventh
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC50_EXPECTED.get(phase) == (
        eleventh_source_handoff, eleventh_source_binding, verifier_binding
    )
...
def gc50():
    fixed = (2, 1, *([2] * 31), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC50_EXPECTED.items()]
    assert all(_gc50_ok(*x) for x in tails)
    z = q(55)
    seed = BASE_V98_EPOCH49_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(6, 37)]
    deadline_bad = sum(x[37] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc50_ok(0, 0, 0, 0, *fixed),
        _gc50_ok(2, 2, 0, 0, *fixed),
        _gc50_ok(4, 2, 2, 0, *fixed),
        _gc50_ok(6, 2, 2, 2, *fixed),
        not _gc50_ok(6, 3, 2, 2, *fixed),
        not _gc50_ok(6, 2, 3, 2, *fixed),
        not _gc50_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc50_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** 31) * 2
    patterns = static_patterns * (4 ** 55) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch49_complete_seed_states': seed,
        'delay_vectors': 4 ** 55,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch50_eleventh_source_handoff_states': handoff * z,
        'epoch50_bound_eleventh_source_handoff_states': bound_handoff * z,
        'epoch50_eleventh_source_binding_states': source_binding_count * z,
        'epoch50_bound_eleventh_source_binding_states': bound_source_binding * z,
        'epoch50_verifier_binding_states': verifier * z,
        'epoch50_bound_verifier_binding_states': bound_verifier * z,
        'epoch50_complete_states': complete * z,
        'unbound_or_conflicting_eleventh_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_eleventh_source_binding_acceptances': transition_bad[1] * z,
