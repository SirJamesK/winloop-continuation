...
BASE_V98_BOUND_TWENTY_THIRD_RESTART_STATIC_STATES = 27648
...
def _pub99_ok(phase, successor_disappearance, replacement_binding,
              dual, restart24, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart24,
        consume, quorum
    )):
        return False
    # V98's twenty-third restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V99 then handles successor-source
    # disappearance, replacement binding, fresh reconciliation, and restart 24.
    if cache_authority != 0:
        return False
    return _PUB99_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart24,
        consume, quorum
    )


def successor_disappearance_twenty_fourth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB99_EXPECTED.items()
    ]
    assert all(_pub99_ok(*x) for x in tails)
    z = q(52)
    seed = BASE_V98_BOUND_TWENTY_THIRD_RESTART_STATIC_STATES
    ...
    restart24 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart24 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub99_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub99_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub99_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub99_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub99_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub99_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub99_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub99_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub99_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub99_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub99_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub99_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub99_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_twenty_third_restart_seed_states': seed,
        'delay_vectors': 4 ** 52,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'twenty_fourth_verifier_cold_restart_states': restart24 * z,
        'bound_twenty_fourth_restart_states': bound_restart24 * z,
        'bound_twenty_fourth_restart_recoveries': bound_recovery * z,
        'cached_twenty_fourth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_twenty_fourth_restart_acceptances': restart_bad * z,
