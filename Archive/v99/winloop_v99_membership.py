from winloop_v99_core import q

BASE_V98_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem99_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V98 completed the root-14 rollover with replacement/prior-source bindings
    # intact. V99 keeps generation 4 / root 14 fixed, rebinds the witness to
    # root 14, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 14 or target_root != 14:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM99_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root14_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 14, 14, 2, 1, 2, 2, 0)
        for phase, state in _MEM99_EXPECTED.items()
    ]
    assert all(_mem99_ok(*x) for x in tails)
    z = q(50)
    seed = BASE_V98_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 14 for x in tails) * seed
    target_root_regression = sum(x[6] < 14 for x in tails) * seed
    ...
    checks = [
        _mem99_ok(0, 0, 0, 0, 4, 14, 14, 2, 1, 2, 2, 0),
        _mem99_ok(2, 2, 0, 0, 4, 14, 14, 2, 1, 2, 2, 0),
        _mem99_ok(4, 2, 2, 0, 4, 14, 14, 2, 1, 2, 2, 0),
        _mem99_ok(6, 2, 2, 2, 4, 14, 14, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 3, 2, 2, 4, 14, 14, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 3, 2, 4, 14, 14, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 3, 4, 14, 14, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 3, 14, 14, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 14, 13, 2, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 14, 14, 1, 1, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 14, 14, 2, 2, 2, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 14, 14, 2, 1, 3, 2, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 14, 14, 2, 1, 2, 3, 0),
        not _mem99_ok(6, 2, 2, 2, 4, 14, 14, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 7 * (4 ** 3) * 6 * 15 * 15 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 50) * z
    return {
        'patterns': patterns,
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root14_witness_rebind_states': rebind * z,
        'bound_root14_witness_rebind_states': bound_rebind * z,
        'root14_witness_binding_states': witness_binding * z,
        'bound_root14_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root14_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root14_witness_binding_acceptances': witness_bad * z,
