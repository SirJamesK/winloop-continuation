"""WinLoop V158 exact continuation: epoch-109 fortieth-lineage rotation, eighty-third cold restart, and root-44 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V158'
BASE_DIGEST = '3804b07e96cb5d35586c9472e4deff835af0fb966d340330b588d83fb6d43973'
BASE_IMPL_SHA = '9868da75f0937a6987ed88e13a1dc6cf6830c892357940f9ee5bd59731a18a8f'
D = 3
...
def gc109():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*190+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**173) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch108_complete_seed_states': seed,
        'delay_vectors': 4**173,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch109_bound_fortieth_lineage_rotation_states': counts[0][1],
        'epoch109_bound_fortieth_lineage_binding_states': counts[1][1],
        'epoch109_bound_handed_proof_rebind_states': counts[2][1],
        'epoch109_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication83():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**170) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_second_restart_seed_states': seed,
        'delay_vectors': 4**170,
        'deadline_vectors': z,
        ...
        'bound_eighty_third_restart_states': counts[3][1],
        'bound_eighty_third_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership44():
    ...
    def ok(phase, state, generation=4, carried_root=43, target_root=44,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=42),
        not ok(10,expected[10],target_root=43),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**168) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root44_rollover_states': counts[2][1],
        'bound_root44_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc109(), publication83(), membership44()
    o = {
        'version': V,
        'base': {
            'version':'V157',
        ...
        'epoch109': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch108_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fortieth_lineage_rotation_states':t['epoch109_bound_fortieth_lineage_rotation_states'],
            'bound_fortieth_lineage_binding_states':t['epoch109_bound_fortieth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch109_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch109_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication83': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_second_restart_seed_states'],
            ...
            'bound_eighty_third_restart_states':s['bound_eighty_third_restart_states'],
            'bound_eighty_third_restart_recoveries':s['bound_eighty_third_restart_recoveries'],
        ...
        'membership44': {
            ...
            'bound_root44_rollover_states':b['bound_root44_rollover_states'],
            'bound_root44_binding_states':b['bound_root44_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 110 by handing the rebound proof to a forty-first source, binding that source, and preserving the epoch-12 deadline',
            'compose eighty-third-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and an eighty-fourth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-44 rollover, rebind the witness to root 44, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V158 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-109 GC to {t['accepted']:,} states with {t['epoch109_bound_fortieth_lineage_rotation_states']:,} bound fortieth-lineage rotations, "
        f"{t['epoch109_bound_fortieth_lineage_binding_states']:,} bound lineage bindings, {t['epoch109_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch109_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_eighty_third_restart_recoveries']:,} fully bound eighty-third-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root44_rollover_states']:,} bound root-44 rollovers, "
        f"{b['bound_root44_binding_states']:,} bound root-44 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
