from winloop_v88_core import q

BASE_V87_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem88_ok(phase, witness_source_replacement, replacement_source_binding,
              root9_rollover, root9_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root9_rollover, root9_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V87 leaves a rebound witness current at generation 4 / root 8. V88 binds
    # a new witness source, rolls to root 9, binds that root, and only then lets
    # a replication-quorum churn complete.
    if generation != 4 or carried_root != 8 or target_root != 9:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM88_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root9_rollover, root9_binding, quorum_churn
    )


def root9_rollover_after_rebound_witness_replacement():
    tails = [
        (phase, *state, 4, 8, 9, 2, 1, 2, 2, 0)
        for phase, state in _MEM88_EXPECTED.items()
    ]
    assert all(_mem88_ok(*x) for x in tails)
    z = q(28)
    seed = BASE_V87_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 8 for x in tails) * seed
    target_root_regression = sum(x[8] < 9 for x in tails) * seed
    ...
    checks = [
        _mem88_ok(0, 0, 0, 0, 0, 0, 4, 8, 9, 2, 1, 2, 2, 0),
        _mem88_ok(2, 2, 0, 0, 0, 0, 4, 8, 9, 2, 1, 2, 2, 0),
        _mem88_ok(4, 2, 2, 0, 0, 0, 4, 8, 9, 2, 1, 2, 2, 0),
        _mem88_ok(6, 2, 2, 2, 0, 0, 4, 8, 9, 2, 1, 2, 2, 0),
        _mem88_ok(8, 2, 2, 2, 2, 0, 4, 8, 9, 2, 1, 2, 2, 0),
        _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 3, 2, 2, 2, 2, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 3, 2, 2, 2, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 3, 2, 2, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 3, 2, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 3, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 3, 8, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 7, 9, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 8, 2, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 9, 1, 1, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 9, 2, 2, 2, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 9, 2, 1, 1, 2, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 9, 2, 1, 2, 3, 0),
        not _mem88_ok(10, 2, 2, 2, 2, 2, 4, 8, 9, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 10 * 10 * 4 * 3 * 4 * 4 * 2
    ...
    return {
        ...
        'root9_rollover_states': rollover * z,
        'bound_root9_rollover_states': bound_rollover * z,
        'root9_binding_states': root_binding * z,
        'bound_root9_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root9_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root9_binding_acceptances': root_binding_bad * z,
