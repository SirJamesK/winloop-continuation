from winloop_v114_core import q

BASE_V113_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem114_ok(phase, witness_source_replacement, replacement_source_binding,
               root22_rollover, root22_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root22_rollover, root22_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V113 leaves a rebound witness current at generation 4 / root 21. V114
    # binds a new witness source, rolls to root 22, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 21 or target_root != 22:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM114_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root22_rollover, root22_binding, quorum_churn
    )


def root22_rollover_after_root21_witness_source_replacement():
    tails = [
        (phase, *state, 4, 21, 22, 2, 1, 2, 2, 0)
        for phase, state in _MEM114_EXPECTED.items()
    ]
    assert all(_mem114_ok(*x) for x in tails)
    z = q(80)
    seed = BASE_V113_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 21 for x in tails) * seed
    target_root_regression = sum(x[8] < 22 for x in tails) * seed
    ...
    checks = [
        _mem114_ok(0, 0, 0, 0, 0, 0, 4, 21, 22, 2, 1, 2, 2, 0),
        _mem114_ok(2, 2, 0, 0, 0, 0, 4, 21, 22, 2, 1, 2, 2, 0),
        _mem114_ok(4, 2, 2, 0, 0, 0, 4, 21, 22, 2, 1, 2, 2, 0),
        _mem114_ok(6, 2, 2, 2, 0, 0, 4, 21, 22, 2, 1, 2, 2, 0),
        _mem114_ok(8, 2, 2, 2, 2, 0, 4, 21, 22, 2, 1, 2, 2, 0),
        _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 3, 2, 2, 2, 2, 4, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 3, 2, 2, 2, 4, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 3, 2, 2, 4, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 3, 2, 4, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 3, 4, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 3, 21, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 20, 22, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 21, 2, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 22, 1, 1, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 22, 2, 2, 2, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 22, 2, 1, 1, 2, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 22, 2, 1, 2, 3, 0),
        not _mem114_ok(10, 2, 2, 2, 2, 2, 4, 21, 22, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 80) * z
    return {
        'patterns': patterns,
        ...
        'root22_rollover_states': rollover * z,
        'bound_root22_rollover_states': bound_rollover * z,
        'root22_binding_states': root_binding * z,
        'bound_root22_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root22_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root22_binding_acceptances': root_binding_bad * z,
