...
D = 3
BASE_V113_EPOCH64_COMPLETE_STATIC_STATES = 576
...
def _gc65_ok(phase, eighteenth_lineage_rotation, eighteenth_lineage_binding,
    ...
    *carried, deadline_reset = rest
    ...
    if deadline_reset != 0:
        return False
    # V113 completed epoch 64 only after the rebound proof was handed to and
    # bound by an eighteenth source. Epoch 65 rotates and binds that source
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC65_EXPECTED.get(phase) == (
        eighteenth_lineage_rotation, eighteenth_lineage_binding,
...
def gc65():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC65_EXPECTED.items()]
    assert all(_gc65_ok(*x) for x in tails)
    z = q(85)
    seed = BASE_V113_EPOCH64_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc65_ok(0, 0, 0, 0, 0, *fixed),
        _gc65_ok(2, 2, 0, 0, 0, *fixed),
        _gc65_ok(4, 2, 2, 0, 0, *fixed),
        _gc65_ok(6, 2, 2, 2, 0, *fixed),
        _gc65_ok(8, 2, 2, 2, 2, *fixed),
        not _gc65_ok(8, 3, 2, 2, 2, *fixed),
        not _gc65_ok(8, 2, 3, 2, 2, *fixed),
        not _gc65_ok(8, 2, 2, 3, 2, *fixed),
        not _gc65_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc65_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 85) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch64_complete_seed_states': seed,
        'delay_vectors': 4 ** 85,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch65_eighteenth_lineage_rotation_states': rotation * z,
        'epoch65_bound_eighteenth_lineage_rotation_states': bound_rotation * z,
        'epoch65_eighteenth_lineage_binding_states': lineage_binding * z,
        'epoch65_bound_eighteenth_lineage_binding_states': bound_lineage_binding * z,
        'epoch65_handed_proof_rebind_states': proof_rebind * z,
        'epoch65_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch65_verifier_binding_states': verifier * z,
        'epoch65_bound_verifier_binding_states': bound_verifier * z,
        'epoch65_complete_states': complete * z,
        'unbound_or_conflicting_eighteenth_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_eighteenth_lineage_binding_acceptances': transition_bad[1] * z,
