"""WinLoop V132 exact continuation: epoch-83 twenty-seventh-lineage rotation, fifty-seventh cold restart, and root-31 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V132'
BASE_DIGEST = 'fbe2af2075272e7bfe830cf3a452314603e7a9124bdfff9a7466ace7f691b448'
BASE_IMPL_SHA = '6ca8baee84bd21c38e91d31837f370ad1e83e2501224c7ed9ab7f2c3e3e452cb'
D = 3
...
def gc83():
    ...
    checks += [not ok(8, (3,2,2,2)), not ok(8, (2,3,2,2)),
               not ok(8, (2,2,3,2)), not ok(8, (2,2,2,3)),
               not ok(8, expected[8], root=1),
               not ok(8, expected[8], continuity=0),
               not ok(8, expected[8], carried=(2,)*112+(1,)),
               not ok(8, expected[8], deadline_reset=1)]
    z = q(121); seed = 576
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**121) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'epoch82_complete_seed_states': seed,
        'delay_vectors': 4**121, 'deadline_vectors': z, 'deadline_origin': 'epoch12',
        'epoch83_bound_twenty_seventh_lineage_rotation_states': counts[0][1],
        'epoch83_bound_twenty_seventh_lineage_binding_states': counts[1][1],
        'epoch83_bound_handed_proof_rebind_states': counts[2][1],
        'epoch83_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0, 'checks': checks,
    }
...
def publication57():
    ...
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [not ok(10,(3,2,2,2,2,2)), not ok(10,(2,3,2,2,2,2)),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**118) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'bound_fifty_sixth_restart_seed_states': seed,
        'delay_vectors': 4**118, 'deadline_vectors': z,
        'bound_replacement_source_churn_states': counts[0][1],
        'bound_successor_source_binding_states': counts[1][1],
        'bound_fifty_seventh_restart_recoveries': seed * z,
        'bad_acceptances': 0, 'checks': checks,
    }
...
def membership31():
    ...
    def ok(phase, state, generation=4, carried_root=30, target_root=31,
           replication=2, tombstone=1, witness=2, prior_source=2, active_byzantine=0):
        return (
            0 <= phase < len(expected) and state == expected[phase]
            and all(x != 3 for x in state) and generation == 4
            and carried_root == 30 and target_root == 31 and replication == 2
            and tombstone == 1 and witness == 2 and prior_source == 2
            and active_byzantine == 0
        )
    checks = [ok(i, s) for i, s in enumerate(expected)]
    checks += [not ok(10,(3,2,2,2,2)), not ok(10,(2,3,2,2,2)),
               not ok(10,(2,2,3,2,2)), not ok(10,(2,2,2,3,2)),
               not ok(10,(2,2,2,2,3)), not ok(10,expected[10],generation=3),
               not ok(10,expected[10],carried_root=29), not ok(10,expected[10],target_root=30),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**116) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'bound_quorum_churn_seed_states': seed,
        'delay_vectors': 4**116, 'deadline_vectors': z,
        'bound_witness_source_replacement_states': counts[0][1],
        'bound_root31_rollover_states': counts[2][1],
...
def run_validation():
    c, t, s, b = indep(), gc83(), publication57(), membership31()
    o = {
        'version': V,
        'base': {'version':'V131','digest':BASE_DIGEST,'implementation_sha256':BASE_IMPL_SHA},
        ...
        'epoch83': {
            'patterns':t['patterns'],'accepted':t['accepted'],'seed_states':t['epoch82_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],'deadline_vectors':t['deadline_vectors'],'deadline_origin':t['deadline_origin'],
            'bound_twenty_seventh_lineage_rotation_states':t['epoch83_bound_twenty_seventh_lineage_rotation_states'],
            'bound_twenty_seventh_lineage_binding_states':t['epoch83_bound_twenty_seventh_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch83_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch83_bound_verifier_binding_states'],'bad_acceptances':t['bad_acceptances'],
        },
        'publication57': {
            'patterns':s['patterns'],'accepted':s['accepted'],'seed_states':s['bound_fifty_sixth_restart_seed_states'],
            ...
            'bound_fifty_seventh_restart_recoveries':s['bound_fifty_seventh_restart_recoveries'],'bad_acceptances':s['bad_acceptances'],
        },
        'membership31': {
            ...
            'bound_root31_rollover_states':b['bound_root31_rollover_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 84 by handing the rebound proof to a twenty-eighth source, binding that source, and preserving the epoch-12 deadline',
            'compose fifty-seventh-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a fifty-eighth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-31 rollover, rebind the witness to root 31, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V132 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-83 GC to {t['accepted']:,} states with {t['epoch83_bound_twenty_seventh_lineage_rotation_states']:,} bound twenty-seventh-lineage rotations, "
        f"{t['epoch83_bound_twenty_seventh_lineage_binding_states']:,} bound lineage bindings, {t['epoch83_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch83_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_fifty_seventh_restart_recoveries']:,} fully bound fifty-seventh-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root31_rollover_states']:,} bound root-31 rollovers, and "
