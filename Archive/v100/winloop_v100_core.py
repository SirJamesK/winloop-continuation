...
D = 3
BASE_V99_EPOCH50_COMPLETE_STATIC_STATES = 576
...
def _gc51_ok(phase, eleventh_lineage_rotation, eleventh_lineage_binding,
             ...
             eleventh_source_handoff, eleventh_source_binding, deadline_reset):
    ...
    if deadline_reset != 0:
        return False
    # V99 completed epoch 50 only after the rebound proof was handed to and
    # bound by an eleventh source. Epoch 51 rotates and binds that source
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC51_EXPECTED.get(phase) == (
        eleventh_lineage_rotation, eleventh_lineage_binding,
...
def gc51():
    fixed = (2, 1, *([2] * 33), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC51_EXPECTED.items()]
    assert all(_gc51_ok(*x) for x in tails)
    z = q(57)
    seed = BASE_V99_EPOCH50_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(7, 40)]
    deadline_bad = sum(x[40] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc51_ok(0, 0, 0, 0, 0, *fixed),
        _gc51_ok(2, 2, 0, 0, 0, *fixed),
        _gc51_ok(4, 2, 2, 0, 0, *fixed),
        _gc51_ok(6, 2, 2, 2, 0, *fixed),
        _gc51_ok(8, 2, 2, 2, 2, *fixed),
        not _gc51_ok(8, 3, 2, 2, 2, *fixed),
        not _gc51_ok(8, 2, 3, 2, 2, *fixed),
        not _gc51_ok(8, 2, 2, 3, 2, *fixed),
        not _gc51_ok(8, 2, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc51_ok(8, 2, 2, 2, 2, *broken))

    static_patterns = seed * 9 * (4 ** 4) * 4 * 3 * (4 ** 33) * 2
    patterns = static_patterns * (4 ** 57) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch50_complete_seed_states': seed,
        'delay_vectors': 4 ** 57,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch51_eleventh_lineage_rotation_states': rotation * z,
        'epoch51_bound_eleventh_lineage_rotation_states': bound_rotation * z,
        'epoch51_eleventh_lineage_binding_states': lineage_binding * z,
        'epoch51_bound_eleventh_lineage_binding_states': bound_lineage_binding * z,
        'epoch51_handed_proof_rebind_states': proof_rebind * z,
        'epoch51_bound_handed_proof_rebind_states': bound_proof_rebind * z,
        'epoch51_verifier_binding_states': verifier * z,
        'epoch51_bound_verifier_binding_states': bound_verifier * z,
        'epoch51_complete_states': complete * z,
        'unbound_or_conflicting_eleventh_lineage_rotation_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_eleventh_lineage_binding_acceptances': transition_bad[1] * z,
