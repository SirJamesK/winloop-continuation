from winloop_v124_core import q

BASE_V123_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem124_ok(phase, witness_source_replacement, replacement_source_binding,
               root27_rollover, root27_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root27_rollover, root27_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V123 leaves a rebound witness current at generation 4 / root 26. V124
    # binds a new witness source, rolls to root 27, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 26 or target_root != 27:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM124_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root27_rollover, root27_binding, quorum_churn
    )


def root27_rollover_after_root26_witness_source_replacement():
    tails = [
        (phase, *state, 4, 26, 27, 2, 1, 2, 2, 0)
        for phase, state in _MEM124_EXPECTED.items()
    ]
    assert all(_mem124_ok(*x) for x in tails)
    z = q(100)
    seed = BASE_V123_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 26 for x in tails) * seed
    target_root_regression = sum(x[8] < 27 for x in tails) * seed
    ...
    checks = [
        _mem124_ok(0, 0, 0, 0, 0, 0, 4, 26, 27, 2, 1, 2, 2, 0),
        _mem124_ok(2, 2, 0, 0, 0, 0, 4, 26, 27, 2, 1, 2, 2, 0),
        _mem124_ok(4, 2, 2, 0, 0, 0, 4, 26, 27, 2, 1, 2, 2, 0),
        _mem124_ok(6, 2, 2, 2, 0, 0, 4, 26, 27, 2, 1, 2, 2, 0),
        _mem124_ok(8, 2, 2, 2, 2, 0, 4, 26, 27, 2, 1, 2, 2, 0),
        _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 3, 2, 2, 2, 2, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 3, 2, 2, 2, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 3, 2, 2, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 3, 2, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 3, 4, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 3, 26, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 25, 27, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 26, 2, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 27, 1, 1, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 27, 2, 2, 2, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 27, 2, 1, 1, 2, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 27, 2, 1, 2, 3, 0),
        not _mem124_ok(10, 2, 2, 2, 2, 2, 4, 26, 27, 2, 1, 2, 2, 1),
    ]
    ...
    return {
        ...
        'delay_vectors': 4 ** 100, 'deadline_vectors': z, 'shared_deadline': 3,
        ...
        'root27_rollover_states': rollover * z,
        'bound_root27_rollover_states': bound_rollover * z,
        'root27_binding_states': root_binding * z,
        'bound_root27_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root27_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root27_binding_acceptances': root_binding_bad * z,
