"""WinLoop V179 exact continuation: epoch-130 fifty-first-source handoff, one-hundred-fourth cold restart, and root-54 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V179'
BASE_DIGEST = '4322827075fb701a166d4eb76162355dbd77fe3d23745ca529c404183a35db08'
BASE_IMPL_SHA = 'a8ced41416d596db6a41c3b22bb2208b69488e46ef6172a361971ec46f6fb761'
D = 3
...
def gc130():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*253+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**215) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch129_complete_seed_states': seed,
        'delay_vectors': 4**215,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch130_bound_fifty_first_source_handoff_states': counts[0][1],
        'epoch130_bound_fifty_first_source_binding_states': counts[1][1],
        'epoch130_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication104():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**212) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_third_restart_seed_states': seed,
        'delay_vectors': 4**212,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_fourth_restart_states': counts[3][1],
        'bound_one_hundred_fourth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership54():
    ...
    def ok(phase, state, generation=4, carried_root=54, target_root=54,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=53),
        not ok(6,expected[6],target_root=53),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**210) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**210,
        'deadline_vectors': z,
        'bound_root54_witness_rebind_states': counts[0][1],
        'bound_root54_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc130(), publication104(), membership54()
    o = {
        'version': V,
        'base': {
            'version':'V178',
        ...
        'epoch130': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch129_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fifty_first_source_handoff_states':t['epoch130_bound_fifty_first_source_handoff_states'],
            'bound_fifty_first_source_binding_states':t['epoch130_bound_fifty_first_source_binding_states'],
            'bound_verifier_binding_states':t['epoch130_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication104': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_third_restart_seed_states'],
            ...
            'bound_one_hundred_fourth_restart_states':s['bound_one_hundred_fourth_restart_states'],
            'bound_one_hundred_fourth_restart_recoveries':s['bound_one_hundred_fourth_restart_recoveries'],
        ...
        'membership54': {
            ...
            'bound_root54_witness_rebind_states':b['bound_root54_witness_rebind_states'],
            'bound_root54_witness_binding_states':b['bound_root54_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 131 by rotating the fifty-first-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose one-hundred-fourth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a one-hundred-fifth verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-54 witness rebind, replace the witness source, roll to root 55, bind root 55, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V179 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-130 GC to {t['accepted']:,} states with {t['epoch130_bound_fifty_first_source_handoff_states']:,} bound fifty-first-source handoffs, "
        f"{t['epoch130_bound_fifty_first_source_binding_states']:,} bound fifty-first-source bindings, and {t['epoch130_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_fourth_restart_recoveries']:,} fully bound one-hundred-fourth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root54_witness_rebind_states']:,} bound root-54 witness rebinds, "
        f"{b['bound_root54_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
