...
BASE_V112_BOUND_THIRTY_SEVENTH_RESTART_STATIC_STATES = 27648
...
def _pub113_ok(phase, successor_disappearance, replacement_binding,
               dual, restart38, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart38,
        consume, quorum
    )):
        return False
    # V112's thirty-seventh restart is a seed only after replacement-source
    # churn, successor binding, and reconciliation. V113 handles successor
    # disappearance, replacement binding, fresh reconciliation, and restart 38.
    if cache_authority != 0:
        return False
    return _PUB113_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart38,
        consume, quorum
    )


def successor_disappearance_thirty_eighth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB113_EXPECTED.items()
    ]
    assert all(_pub113_ok(*x) for x in tails)
    z = q(80)
    seed = BASE_V112_BOUND_THIRTY_SEVENTH_RESTART_STATIC_STATES
    ...
    restart38 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart38 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub113_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub113_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub113_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub113_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub113_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub113_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub113_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub113_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub113_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub113_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub113_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub113_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub113_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_thirty_seventh_restart_seed_states': seed,
        'delay_vectors': 4 ** 80,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'thirty_eighth_verifier_cold_restart_states': restart38 * z,
        'bound_thirty_eighth_restart_states': bound_restart38 * z,
        'bound_thirty_eighth_restart_recoveries': bound_recovery * z,
        'cached_thirty_eighth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_thirty_eighth_restart_acceptances': restart_bad * z,
