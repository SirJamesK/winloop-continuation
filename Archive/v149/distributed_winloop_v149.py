"""WinLoop V149 exact continuation: epoch-100 thirty-sixth-source handoff, seventy-fourth cold restart, and root-39 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V149'
BASE_DIGEST = '8859796c5b9e5888bff5a7e6a7e01a2f6f1a52015e65c93ea9cf387121232be2'
BASE_IMPL_SHA = '955762ea633af64e9ad1b158ec614a3c0ac3211b1b77c3762caaee33fa27c7c5'
D = 3
...
def gc100():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*163+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**155) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch99_complete_seed_states': seed,
        'delay_vectors': 4**155,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch100_bound_thirty_sixth_source_handoff_states': counts[0][1],
        'epoch100_bound_thirty_sixth_source_binding_states': counts[1][1],
        'epoch100_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication74():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**152) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_third_restart_seed_states': seed,
        'delay_vectors': 4**152,
        'deadline_vectors': z,
        ...
        'bound_seventy_fourth_restart_states': counts[3][1],
        'bound_seventy_fourth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership39():
    ...
    def ok(phase, state, generation=4, carried_root=39, target_root=39,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=38),
        not ok(6,expected[6],target_root=38),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**150) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**150,
        'deadline_vectors': z,
        'bound_root39_witness_rebind_states': counts[0][1],
        'bound_root39_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc100(), publication74(), membership39()
    o = {
        'version': V,
        'base': {
            'version':'V148',
        ...
        'epoch100': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch99_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_sixth_source_handoff_states':t['epoch100_bound_thirty_sixth_source_handoff_states'],
            'bound_thirty_sixth_source_binding_states':t['epoch100_bound_thirty_sixth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch100_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication74': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_third_restart_seed_states'],
            ...
            'bound_seventy_fourth_restart_states':s['bound_seventy_fourth_restart_states'],
            'bound_seventy_fourth_restart_recoveries':s['bound_seventy_fourth_restart_recoveries'],
        ...
        'membership39': {
            ...
            'bound_root39_witness_rebind_states':b['bound_root39_witness_rebind_states'],
            'bound_root39_witness_binding_states':b['bound_root39_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 101 by rotating the thirty-sixth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose seventy-fourth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a seventy-fifth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-39 witness rebind, replace the witness source, roll to root 40, bind root 40, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V149 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-100 GC to {t['accepted']:,} states with {t['epoch100_bound_thirty_sixth_source_handoff_states']:,} bound thirty-sixth-source handoffs, "
        f"{t['epoch100_bound_thirty_sixth_source_binding_states']:,} bound thirty-sixth-source bindings, and {t['epoch100_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_seventy_fourth_restart_recoveries']:,} fully bound seventy-fourth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root39_witness_rebind_states']:,} bound root-39 witness rebinds, "
        f"{b['bound_root39_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
