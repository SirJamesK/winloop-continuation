"""WinLoop V185 exact continuation: epoch-136 fifty-fourth-source handoff, one-hundred-tenth cold restart, and root-57 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V185'
BASE_DIGEST = 'baa3dda4dc109f217b21361d39dc0766efb64165e58d83a6399a36c6146d6b18'
BASE_IMPL_SHA = '3d0fb5ff9179e7477735ef7ec81a31d7a2d4f343f4af94643d2b8fd4c161e1a1'
D = 3
...
def gc136():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*271+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**227) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch135_complete_seed_states': seed,
        'delay_vectors': 4**227,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch136_bound_fifty_fourth_source_handoff_states': counts[0][1],
        'epoch136_bound_fifty_fourth_source_binding_states': counts[1][1],
        'epoch136_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication110():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**224) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_one_hundred_ninth_restart_seed_states': seed,
        'delay_vectors': 4**224,
        'deadline_vectors': z,
        ...
        'bound_one_hundred_tenth_restart_states': counts[3][1],
        'bound_one_hundred_tenth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership57():
    ...
    def ok(phase, state, generation=4, carried_root=57, target_root=57,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=56),
        not ok(6,expected[6],target_root=56),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**222) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**222,
        'deadline_vectors': z,
        'bound_root57_witness_rebind_states': counts[0][1],
        'bound_root57_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc136(), publication110(), membership57()
    o = {
        'version': V,
        'base': {
            'version':'V184',
        ...
        'epoch136': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch135_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_fifty_fourth_source_handoff_states':t['epoch136_bound_fifty_fourth_source_handoff_states'],
            'bound_fifty_fourth_source_binding_states':t['epoch136_bound_fifty_fourth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch136_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication110': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_one_hundred_ninth_restart_seed_states'],
            ...
            'bound_one_hundred_tenth_restart_states':s['bound_one_hundred_tenth_restart_states'],
            'bound_one_hundred_tenth_restart_recoveries':s['bound_one_hundred_tenth_restart_recoveries'],
        ...
        'membership57': {
            ...
            'bound_root57_witness_rebind_states':b['bound_root57_witness_rebind_states'],
            'bound_root57_witness_binding_states':b['bound_root57_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 137 by rotating the fifty-fourth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose one-hundred-tenth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a one-hundred-eleventh verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-57 witness rebind, replace the witness source, roll to root 58, bind root 58, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V185 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-136 GC to {t['accepted']:,} states with {t['epoch136_bound_fifty_fourth_source_handoff_states']:,} bound fifty-fourth-source handoffs, "
        f"{t['epoch136_bound_fifty_fourth_source_binding_states']:,} bound fifty-fourth-source bindings, and {t['epoch136_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundred_tenth_restart_recoveries']:,} fully bound one-hundred-tenth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root57_witness_rebind_states']:,} bound root-57 witness rebinds, "
        f"{b['bound_root57_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
