from winloop_v117_core import q

BASE_V116_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem117_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V116 completed the root-23 rollover with replacement/prior-source bindings
    # intact. V117 keeps generation 4 / root 23 fixed, rebinds the witness to
    # root 23, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 23 or target_root != 23:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM117_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root23_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 23, 23, 2, 1, 2, 2, 0)
        for phase, state in _MEM117_EXPECTED.items()
    ]
    assert all(_mem117_ok(*x) for x in tails)
    z = q(86)
    seed = BASE_V116_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 23 for x in tails) * seed
    target_root_regression = sum(x[6] < 23 for x in tails) * seed
    ...
    checks = [
        _mem117_ok(0, 0, 0, 0, 4, 23, 23, 2, 1, 2, 2, 0),
        _mem117_ok(2, 2, 0, 0, 4, 23, 23, 2, 1, 2, 2, 0),
        _mem117_ok(4, 2, 2, 0, 4, 23, 23, 2, 1, 2, 2, 0),
        _mem117_ok(6, 2, 2, 2, 4, 23, 23, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 3, 2, 2, 4, 23, 23, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 3, 2, 4, 23, 23, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 3, 4, 23, 23, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 3, 23, 23, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 22, 23, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 23, 22, 2, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 23, 23, 1, 1, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 23, 23, 2, 2, 2, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 23, 23, 2, 1, 3, 2, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 23, 23, 2, 1, 2, 3, 0),
        not _mem117_ok(6, 2, 2, 2, 4, 23, 23, 2, 1, 2, 2, 1),
    ]
    ...
    return {
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root23_witness_rebind_states': rebind * z,
        'bound_root23_witness_rebind_states': bound_rebind * z,
        'root23_witness_binding_states': witness_binding * z,
        'bound_root23_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root23_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root23_witness_binding_acceptances': witness_bad * z,
