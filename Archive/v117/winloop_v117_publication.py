...
BASE_V116_BOUND_FORTY_FIRST_RESTART_STATIC_STATES = 27648
...
def _pub117_ok(phase, successor_disappearance, replacement_binding,
               dual, restart42, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart42,
        consume, quorum
    )):
        return False
    # V116's forty-first restart is a seed only after replacement-source
    # churn, successor binding, and reconciliation. V117 handles successor
    # disappearance, replacement binding, fresh reconciliation, and restart 42.
    if cache_authority != 0:
        return False
    return _PUB117_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart42,
        consume, quorum
    )


def successor_disappearance_forty_second_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB117_EXPECTED.items()
    ]
    assert all(_pub117_ok(*x) for x in tails)
    z = q(88)
    seed = BASE_V116_BOUND_FORTY_FIRST_RESTART_STATIC_STATES
    ...
    restart42 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart42 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub117_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub117_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub117_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub117_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub117_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub117_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub117_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub117_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub117_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub117_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub117_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub117_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub117_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_forty_first_restart_seed_states': seed,
        'delay_vectors': 4 ** 88,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'forty_second_verifier_cold_restart_states': restart42 * z,
        'bound_forty_second_restart_states': bound_restart42 * z,
        'bound_forty_second_restart_recoveries': bound_recovery * z,
        'cached_forty_second_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_forty_second_restart_acceptances': restart_bad * z,
