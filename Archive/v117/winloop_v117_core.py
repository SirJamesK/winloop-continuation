...
D = 3
BASE_V116_EPOCH67_COMPLETE_STATIC_STATES = 576
...
CARRIED_NAMES = (
    ...
    'nineteenth_lineage_rotation',
    'nineteenth_lineage_binding',
    'nineteenth_handed_proof_rebind',
)
...
def _gc68_ok(phase, twentieth_source_handoff, twentieth_source_binding,
             verifier_binding, carried_root_binding, continuity, *rest):
    if len(rest) != len(CARRIED_NAMES) + 1:
        return False
    *carried, deadline_reset = rest
    trust = (
        twentieth_source_handoff, twentieth_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V116 completed epoch 67 only after the nineteenth-source lineage was
    # rotated and bound and the handed proof was rebound. Epoch 68 hands that
    # rebound proof to a twentieth source, binds it, and then permits verifier completion.
    if carried_root_binding != 2 or continuity != 1:
        return False
    if any(x != 2 for x in carried):
        return False
    return _GC68_EXPECTED.get(phase) == (
        twentieth_source_handoff, twentieth_source_binding, verifier_binding
    )
...
def gc68():
    ...
    tails = [(phase, *state, *fixed) for phase, state in _GC68_EXPECTED.items()]
    assert all(_gc68_ok(*x) for x in tails)
    z = q(91)
    seed = BASE_V116_EPOCH67_COMPLETE_STATIC_STATES
    ...
    checks = [
        _gc68_ok(0, 0, 0, 0, *fixed),
        _gc68_ok(2, 2, 0, 0, *fixed),
        _gc68_ok(4, 2, 2, 0, *fixed),
        _gc68_ok(6, 2, 2, 2, *fixed),
        not _gc68_ok(6, 3, 2, 2, *fixed),
        not _gc68_ok(6, 2, 3, 2, *fixed),
        not _gc68_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc68_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** len(CARRIED_NAMES)) * 2
    patterns = static_patterns * (4 ** 91) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch67_complete_seed_states': seed,
        'delay_vectors': 4 ** 91,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch68_twentieth_source_handoff_states': handoff * z,
        'epoch68_bound_twentieth_source_handoff_states': bound_handoff * z,
        'epoch68_twentieth_source_binding_states': source_binding_count * z,
        'epoch68_bound_twentieth_source_binding_states': bound_source_binding * z,
        'epoch68_verifier_binding_states': verifier * z,
        'epoch68_bound_verifier_binding_states': bound_verifier * z,
        'epoch68_complete_states': complete * z,
        'unbound_or_conflicting_twentieth_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twentieth_source_binding_acceptances': transition_bad[1] * z,
