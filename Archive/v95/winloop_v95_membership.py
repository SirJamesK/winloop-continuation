...
BASE_V94_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem95_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V94 completed the root-12 rollover with replacement/prior-source bindings
    # intact. V95 keeps generation 4 / root 12 fixed, rebinds the witness to
    # root 12, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 12 or target_root != 12:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM95_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root12_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 12, 12, 2, 1, 2, 2, 0)
        for phase, state in _MEM95_EXPECTED.items()
    ]
    assert all(_mem95_ok(*x) for x in tails)
    z = q(42)
    seed = BASE_V94_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 12 for x in tails) * seed
    target_root_regression = sum(x[6] < 12 for x in tails) * seed
    ...
    checks = [
        _mem95_ok(0, 0, 0, 0, 4, 12, 12, 2, 1, 2, 2, 0),
        _mem95_ok(2, 2, 0, 0, 4, 12, 12, 2, 1, 2, 2, 0),
        _mem95_ok(4, 2, 2, 0, 4, 12, 12, 2, 1, 2, 2, 0),
        _mem95_ok(6, 2, 2, 2, 4, 12, 12, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 3, 2, 2, 4, 12, 12, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 3, 2, 4, 12, 12, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 3, 4, 12, 12, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 3, 12, 12, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 11, 12, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 12, 11, 2, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 12, 12, 1, 1, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 12, 12, 2, 2, 2, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 12, 12, 2, 1, 3, 2, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 12, 12, 2, 1, 2, 3, 0),
        not _mem95_ok(6, 2, 2, 2, 4, 12, 12, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 7 * (4 ** 3) * 6 * 13 * 13 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 42) * z
    return {
        'patterns': patterns,
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root12_witness_rebind_states': rebind * z,
        'bound_root12_witness_rebind_states': bound_rebind * z,
        'root12_witness_binding_states': witness_binding * z,
        'bound_root12_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root12_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root12_witness_binding_acceptances': witness_bad * z,
