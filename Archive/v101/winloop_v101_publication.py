...
BASE_V100_BOUND_TWENTY_FIFTH_RESTART_STATIC_STATES = 27648
...
def _pub101_ok(phase, successor_disappearance, replacement_binding,
               dual, restart26, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart26,
        consume, quorum
    )):
        return False
    # V100's twenty-fifth restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V101 handles successor-source
    # disappearance, replacement binding, fresh reconciliation, and restart 26.
    if cache_authority != 0:
        return False
    return _PUB101_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart26,
        consume, quorum
    )


def successor_disappearance_twenty_sixth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB101_EXPECTED.items()
    ]
    assert all(_pub101_ok(*x) for x in tails)
    z = q(56)
    seed = BASE_V100_BOUND_TWENTY_FIFTH_RESTART_STATIC_STATES
    ...
    restart26 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart26 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub101_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub101_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub101_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub101_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub101_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub101_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub101_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub101_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub101_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub101_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub101_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub101_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub101_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_twenty_fifth_restart_seed_states': seed,
        'delay_vectors': 4 ** 56,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'twenty_sixth_verifier_cold_restart_states': restart26 * z,
        'bound_twenty_sixth_restart_states': bound_restart26 * z,
        'bound_twenty_sixth_restart_recoveries': bound_recovery * z,
        'cached_twenty_sixth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_twenty_sixth_restart_acceptances': restart_bad * z,
