...
D = 3
BASE_V100_EPOCH51_COMPLETE_STATIC_STATES = 576
...
def _gc52_ok(phase, twelfth_source_handoff, twelfth_source_binding,
             ...
             eleventh_handed_proof_rebind, deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    trust = (
        twelfth_source_handoff, twelfth_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V100 completed epoch 51 only after eleventh-lineage rotation/binding and
    # a fresh handed-proof rebind. Epoch 52 hands that rebound proof to a
    # twelfth source, binds the source, and only then permits verifier completion.
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC52_EXPECTED.get(phase) == (
        twelfth_source_handoff, twelfth_source_binding, verifier_binding
    )
...
def gc52():
    fixed = (2, 1, *([2] * 36), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC52_EXPECTED.items()]
    assert all(_gc52_ok(*x) for x in tails)
    z = q(59)
    seed = BASE_V100_EPOCH51_COMPLETE_STATIC_STATES
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(6, 42)]
    deadline_bad = sum(x[42] != 0 for x in tails) * seed
    bad = sum(transition_bad) + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc52_ok(0, 0, 0, 0, *fixed),
        _gc52_ok(2, 2, 0, 0, *fixed),
        _gc52_ok(4, 2, 2, 0, *fixed),
        _gc52_ok(6, 2, 2, 2, *fixed),
        not _gc52_ok(6, 3, 2, 2, *fixed),
        not _gc52_ok(6, 2, 3, 2, *fixed),
        not _gc52_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc52_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** 36) * 2
    patterns = static_patterns * (4 ** 59) * z
    out = {
        'patterns': patterns,
        'accepted': static_accepted * z,
        'base_states': static_accepted,
        'epoch51_complete_seed_states': seed,
        'delay_vectors': 4 ** 59,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch52_twelfth_source_handoff_states': handoff * z,
        'epoch52_bound_twelfth_source_handoff_states': bound_handoff * z,
        'epoch52_twelfth_source_binding_states': source_binding_count * z,
        'epoch52_bound_twelfth_source_binding_states': bound_source_binding * z,
        'epoch52_verifier_binding_states': verifier * z,
        'epoch52_bound_verifier_binding_states': bound_verifier * z,
        'epoch52_complete_states': complete * z,
        'unbound_or_conflicting_twelfth_source_handoff_acceptances': transition_bad[0] * z,
        'unbound_or_conflicting_twelfth_source_binding_acceptances': transition_bad[1] * z,
