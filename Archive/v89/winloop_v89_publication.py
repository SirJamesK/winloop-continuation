...
BASE_V88_BOUND_THIRTEENTH_RESTART_STATIC_STATES = 27648
...
def _pub89_ok(phase, successor_disappearance, replacement_binding,
              dual, restart14, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart14,
        consume, quorum
    )):
        return False
    # V88's thirteenth restart is a seed only when it completed against a
    # rebound and reconciled source set. V89 then requires successor-source
    # disappearance, replacement binding, re-reconciliation, and a fourteenth
    # verifier cold restart. Cached verifier state never promotes to authority.
    if cache_authority != 0:
        return False
    return _PUB89_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart14,
        consume, quorum
    )


def successor_disappearance_fourteenth_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB89_EXPECTED.items()
    ]
    assert all(_pub89_ok(*x) for x in tails)
    z = q(32)
    seed = BASE_V88_BOUND_THIRTEENTH_RESTART_STATIC_STATES
    ...
    restart14 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart14 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub89_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub89_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub89_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub89_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub89_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub89_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub89_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub89_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub89_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub89_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub89_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub89_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub89_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_thirteenth_restart_seed_states': seed,
        'delay_vectors': 4 ** 32,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'fourteenth_verifier_cold_restart_states': restart14 * z,
        'bound_fourteenth_restart_states': bound_restart14 * z,
        'bound_fourteenth_restart_recoveries': bound_recovery * z,
        'cached_fourteenth_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_fourteenth_restart_acceptances': restart_bad * z,
