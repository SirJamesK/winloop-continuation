from winloop_v89_core import q

BASE_V88_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem89_ok(phase, witness_rebind, witness_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        return False
    # V88 completed the root-9 rollover with the replacement and prior source
    # bindings intact. V89 keeps generation 4 / root 9 fixed, rebinds the
    # witness to root 9, and only then permits another replication-quorum churn.
    if replacement_source_binding != 2 or prior_source_binding != 2:
        return False
    if generation != 4 or carried_root != 9 or target_root != 9:
        return False
    if replication != 2 or tombstone_binding != 1:
        return False
    if active_byzantine != 0:
        return False
    return _MEM89_EXPECTED.get(phase) == (
        witness_rebind, witness_binding, quorum_churn
    )


def root9_witness_rebind_quorum_churn():
    tails = [
        (phase, *state, 4, 9, 9, 2, 1, 2, 2, 0)
        for phase, state in _MEM89_EXPECTED.items()
    ]
    assert all(_mem89_ok(*x) for x in tails)
    z = q(30)
    seed = BASE_V88_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[5] < 9 for x in tails) * seed
    target_root_regression = sum(x[6] < 9 for x in tails) * seed
    ...
    checks = [
        _mem89_ok(0, 0, 0, 0, 4, 9, 9, 2, 1, 2, 2, 0),
        _mem89_ok(2, 2, 0, 0, 4, 9, 9, 2, 1, 2, 2, 0),
        _mem89_ok(4, 2, 2, 0, 4, 9, 9, 2, 1, 2, 2, 0),
        _mem89_ok(6, 2, 2, 2, 4, 9, 9, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 3, 2, 2, 4, 9, 9, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 3, 2, 4, 9, 9, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 3, 4, 9, 9, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 3, 9, 9, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 8, 9, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 9, 8, 2, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 9, 9, 1, 1, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 9, 9, 2, 2, 2, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 9, 9, 2, 1, 3, 2, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 9, 9, 2, 1, 2, 3, 0),
        not _mem89_ok(6, 2, 2, 2, 4, 9, 9, 2, 1, 2, 2, 1),
    ]

    # Phase, three transition coordinates, generation/root/replication domains,
    # tombstone/replacement/prior-source bindings, and active-Byzantine bit.
    static_patterns = seed * 7 * (4 ** 3) * 6 * 10 * 10 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 30) * z
    return {
        'patterns': patterns,
        ...
        'deadline_vectors': z,
        'shared_deadline': 3,
        'root9_witness_rebind_states': rebind * z,
        'bound_root9_witness_rebind_states': bound_rebind * z,
        'root9_witness_binding_states': witness_binding * z,
        'bound_root9_witness_binding_states': bound_witness_binding * z,
        ...
        'unbound_or_conflicting_root9_witness_rebind_acceptances': rebind_bad * z,
        'unbound_or_conflicting_root9_witness_binding_acceptances': witness_bad * z,
