"""WinLoop V159 exact continuation: epoch-110 forty-first-source handoff, eighty-fourth cold restart, and root-44 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V159'
BASE_DIGEST = 'f3d3df7343986d4f4ea79d6bb9d2024a923732a5bac4328e1c60f88792484a47'
BASE_IMPL_SHA = '5638ca4cdc3402b9c474133bc34c0098c1be05ae565d71283ac843cf0437aabb'
D = 3
...
def gc110():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*193+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**175) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch109_complete_seed_states': seed,
        'delay_vectors': 4**175,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch110_bound_forty_first_source_handoff_states': counts[0][1],
        'epoch110_bound_forty_first_source_binding_states': counts[1][1],
        'epoch110_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication84():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**172) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_third_restart_seed_states': seed,
        'delay_vectors': 4**172,
        'deadline_vectors': z,
        ...
        'bound_eighty_fourth_restart_states': counts[3][1],
        'bound_eighty_fourth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership44():
    ...
    def ok(phase, state, generation=4, carried_root=44, target_root=44,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=43),
        not ok(6,expected[6],target_root=43),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**170) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**170,
        'deadline_vectors': z,
        'bound_root44_witness_rebind_states': counts[0][1],
        'bound_root44_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc110(), publication84(), membership44()
    o = {
        'version': V,
        'base': {
            'version':'V158',
        ...
        'epoch110': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch109_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_first_source_handoff_states':t['epoch110_bound_forty_first_source_handoff_states'],
            'bound_forty_first_source_binding_states':t['epoch110_bound_forty_first_source_binding_states'],
            'bound_verifier_binding_states':t['epoch110_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication84': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_third_restart_seed_states'],
            ...
            'bound_eighty_fourth_restart_states':s['bound_eighty_fourth_restart_states'],
            'bound_eighty_fourth_restart_recoveries':s['bound_eighty_fourth_restart_recoveries'],
        ...
        'membership44': {
            ...
            'bound_root44_witness_rebind_states':b['bound_root44_witness_rebind_states'],
            'bound_root44_witness_binding_states':b['bound_root44_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 111 by rotating the forty-first-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose eighty-fourth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and an eighty-fifth verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-44 witness rebind, replace the witness source, roll to root 45, bind root 45, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V159 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-110 GC to {t['accepted']:,} states with {t['epoch110_bound_forty_first_source_handoff_states']:,} bound forty-first-source handoffs, "
        f"{t['epoch110_bound_forty_first_source_binding_states']:,} bound forty-first-source bindings, and {t['epoch110_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_eighty_fourth_restart_recoveries']:,} fully bound eighty-fourth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root44_witness_rebind_states']:,} bound root-44 witness rebinds, "
        f"{b['bound_root44_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
