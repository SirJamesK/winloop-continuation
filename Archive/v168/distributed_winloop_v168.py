"""WinLoop V168 exact continuation: epoch-119 forty-fifth-lineage rotation, ninety-third cold restart, and root-49 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V168'
BASE_DIGEST = 'afc726e917e537c41d479ccfcc1a5e542aa1adcefc449ce9cad6386ebeae7738'
BASE_IMPL_SHA = 'c7377a493468d804715eaf1706fc15dd313dfc600362450daac7c8d1834ec831'
D = 3
...
def gc119():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*220+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**193) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch118_complete_seed_states': seed,
        'delay_vectors': 4**193,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch119_bound_forty_fifth_lineage_rotation_states': counts[0][1],
        'epoch119_bound_forty_fifth_lineage_binding_states': counts[1][1],
        'epoch119_bound_handed_proof_rebind_states': counts[2][1],
        'epoch119_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication93():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**190) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_second_restart_seed_states': seed,
        'delay_vectors': 4**190,
        'deadline_vectors': z,
        ...
        'bound_ninety_third_restart_states': counts[3][1],
        'bound_ninety_third_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership49():
    ...
    def ok(phase, state, generation=4, carried_root=48, target_root=49,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=47),
        not ok(10,expected[10],target_root=48),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**188) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root49_rollover_states': counts[2][1],
        'bound_root49_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc119(), publication93(), membership49()
    o = {
        'version': V,
        'base': {
            'version':'V167',
        ...
        'epoch119': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch118_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_fifth_lineage_rotation_states':t['epoch119_bound_forty_fifth_lineage_rotation_states'],
            'bound_forty_fifth_lineage_binding_states':t['epoch119_bound_forty_fifth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch119_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch119_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication93': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_second_restart_seed_states'],
            ...
            'bound_ninety_third_restart_states':s['bound_ninety_third_restart_states'],
            'bound_ninety_third_restart_recoveries':s['bound_ninety_third_restart_recoveries'],
        ...
        'membership49': {
            ...
            'bound_root49_rollover_states':b['bound_root49_rollover_states'],
            'bound_root49_binding_states':b['bound_root49_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 120 by handing the rebound proof to a forty-sixth source, binding that source, and preserving the epoch-12 deadline',
            'compose ninety-third-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and a ninety-fourth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-49 rollover, rebind the witness to root 49, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V168 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-119 GC to {t['accepted']:,} states with {t['epoch119_bound_forty_fifth_lineage_rotation_states']:,} bound forty-fifth-lineage rotations, "
        f"{t['epoch119_bound_forty_fifth_lineage_binding_states']:,} bound lineage bindings, {t['epoch119_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch119_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_ninety_third_restart_recoveries']:,} fully bound ninety-third-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root49_rollover_states']:,} bound root-49 rollovers, "
        f"{b['bound_root49_binding_states']:,} bound root-49 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
