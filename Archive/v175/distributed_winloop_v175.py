"""WinLoop V175 exact continuation: epoch-126 forty-ninth-source handoff, one-hundredth cold restart, and root-52 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V175'
BASE_DIGEST = 'd3b789fd29e8ea22557e04d4927d30c19ee57a5ed19aadb6b88e65c89b2f99d3'
BASE_IMPL_SHA = '6bb55d9d6199961128259da3a04b4f6e7d81ba08406f6c5e2fcb11604fafec20'
D = 3
...
def gc126():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*241+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**207) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch125_complete_seed_states': seed,
        'delay_vectors': 4**207,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch126_bound_forty_ninth_source_handoff_states': counts[0][1],
        'epoch126_bound_forty_ninth_source_binding_states': counts[1][1],
        'epoch126_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication100():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**204) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_ninety_ninth_restart_seed_states': seed,
        'delay_vectors': 4**204,
        'deadline_vectors': z,
        ...
        'bound_one_hundredth_restart_states': counts[3][1],
        'bound_one_hundredth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership52():
    ...
    def ok(phase, state, generation=4, carried_root=52, target_root=52,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=51),
        not ok(6,expected[6],target_root=51),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**202) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**202,
        'deadline_vectors': z,
        'bound_root52_witness_rebind_states': counts[0][1],
        'bound_root52_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc126(), publication100(), membership52()
    o = {
        'version': V,
        'base': {
            'version':'V174',
        ...
        'epoch126': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch125_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_ninth_source_handoff_states':t['epoch126_bound_forty_ninth_source_handoff_states'],
            'bound_forty_ninth_source_binding_states':t['epoch126_bound_forty_ninth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch126_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication100': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_ninety_ninth_restart_seed_states'],
            ...
            'bound_one_hundredth_restart_states':s['bound_one_hundredth_restart_states'],
            'bound_one_hundredth_restart_recoveries':s['bound_one_hundredth_restart_recoveries'],
        ...
        'membership52': {
            ...
            'bound_root52_witness_rebind_states':b['bound_root52_witness_rebind_states'],
            'bound_root52_witness_binding_states':b['bound_root52_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 127 by rotating the forty-ninth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose one-hundredth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and a one-hundred-first verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-52 witness rebind, replace the witness source, roll to root 53, bind root 53, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V175 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-126 GC to {t['accepted']:,} states with {t['epoch126_bound_forty_ninth_source_handoff_states']:,} bound forty-ninth-source handoffs, "
        f"{t['epoch126_bound_forty_ninth_source_binding_states']:,} bound forty-ninth-source bindings, and {t['epoch126_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_one_hundredth_restart_recoveries']:,} fully bound one-hundredth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root52_witness_rebind_states']:,} bound root-52 witness rebinds, "
        f"{b['bound_root52_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
