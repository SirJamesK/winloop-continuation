from winloop_v106_core import q

BASE_V105_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem106_ok(phase, witness_source_replacement, replacement_source_binding,
               root18_rollover, root18_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root18_rollover, root18_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V105 leaves a rebound witness current at generation 4 / root 17. V106
    # binds a new witness source, rolls to root 18, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 17 or target_root != 18:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM106_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root18_rollover, root18_binding, quorum_churn
    )


def root18_rollover_after_root17_witness_source_replacement():
    tails = [
        (phase, *state, 4, 17, 18, 2, 1, 2, 2, 0)
        for phase, state in _MEM106_EXPECTED.items()
    ]
    assert all(_mem106_ok(*x) for x in tails)
    z = q(64)
    seed = BASE_V105_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 17 for x in tails) * seed
    target_root_regression = sum(x[8] < 18 for x in tails) * seed
    ...
    checks = [
        _mem106_ok(0, 0, 0, 0, 0, 0, 4, 17, 18, 2, 1, 2, 2, 0),
        _mem106_ok(2, 2, 0, 0, 0, 0, 4, 17, 18, 2, 1, 2, 2, 0),
        _mem106_ok(4, 2, 2, 0, 0, 0, 4, 17, 18, 2, 1, 2, 2, 0),
        _mem106_ok(6, 2, 2, 2, 0, 0, 4, 17, 18, 2, 1, 2, 2, 0),
        _mem106_ok(8, 2, 2, 2, 2, 0, 4, 17, 18, 2, 1, 2, 2, 0),
        _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 3, 2, 2, 2, 2, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 3, 2, 2, 2, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 3, 2, 2, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 3, 2, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 3, 4, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 3, 17, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 16, 18, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 17, 2, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 18, 1, 1, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 18, 2, 2, 2, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 18, 2, 1, 1, 2, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 18, 2, 1, 2, 3, 0),
        not _mem106_ok(10, 2, 2, 2, 2, 2, 4, 17, 18, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 64) * z
    return {
        'patterns': patterns,
        ...
        'root18_rollover_states': rollover * z,
        'bound_root18_rollover_states': bound_rollover * z,
        'root18_binding_states': root_binding * z,
        'bound_root18_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root18_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root18_binding_acceptances': root_binding_bad * z,
