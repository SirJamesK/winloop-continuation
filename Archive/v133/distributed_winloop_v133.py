"""WinLoop V133 exact continuation: epoch-84 twenty-eighth-source handoff, fifty-eighth cold restart, and root-31 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V133'
BASE_DIGEST = '6738a57adcaf2976dc7689c235a716d58b7730208c005b803e765bf645ce1697'
BASE_IMPL_SHA = 'f71478033963f22e4e69eb80280b2c76e74b6f46c671ec4292af41d322abef80'
D = 3
...
def gc84():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*115+(1,)),
        not ok(6, expected[6], deadline_reset=1),
    ]
    z = q(123); seed = 576
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**123) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'epoch83_complete_seed_states': seed,
        'delay_vectors': 4**123, 'deadline_vectors': z, 'deadline_origin': 'epoch12',
        'epoch84_bound_twenty_eighth_source_handoff_states': counts[0][1],
        'epoch84_bound_twenty_eighth_source_binding_states': counts[1][1],
        'epoch84_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0, 'checks': checks,
    }
...
def publication58():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**120) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'bound_fifty_seventh_restart_seed_states': seed,
        ...
        'bound_fifty_eighth_restart_states': counts[3][1],
        'bound_fifty_eighth_restart_recoveries': seed * z,
        'bad_acceptances': 0, 'checks': checks,
    }
...
def membership31():
    ...
    def ok(phase, state, generation=4, carried_root=31, target_root=31,
           replication=2, tombstone=1, witness_source=2, prior_source=2,
           active_byzantine=0):
        return (
            0 <= phase < len(expected) and state == expected[phase]
            and all(x != 3 for x in state) and generation == 4
            and carried_root == 31 and target_root == 31 and replication == 2
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=30), not ok(6,expected[6],target_root=30),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**118) * z
    return {
        'patterns': patterns, 'accepted': accepted, 'bound_quorum_churn_seed_states': seed,
        'delay_vectors': 4**118, 'deadline_vectors': z,
        'bound_root31_witness_rebind_states': counts[0][1],
        'bound_root31_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc84(), publication58(), membership31()
    o = {
        'version': V,
        'base': {'version':'V132','digest':BASE_DIGEST,'implementation_sha256':BASE_IMPL_SHA},
        ...
        'epoch84': {
            'patterns':t['patterns'],'accepted':t['accepted'],'seed_states':t['epoch83_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],'deadline_vectors':t['deadline_vectors'],'deadline_origin':t['deadline_origin'],
            'bound_twenty_eighth_source_handoff_states':t['epoch84_bound_twenty_eighth_source_handoff_states'],
            'bound_twenty_eighth_source_binding_states':t['epoch84_bound_twenty_eighth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch84_bound_verifier_binding_states'],'bad_acceptances':t['bad_acceptances'],
        },
        'publication58': {
            'patterns':s['patterns'],'accepted':s['accepted'],'seed_states':s['bound_fifty_seventh_restart_seed_states'],
            ...
            'bound_fifty_eighth_restart_states':s['bound_fifty_eighth_restart_states'],
            'bound_fifty_eighth_restart_recoveries':s['bound_fifty_eighth_restart_recoveries'],'bad_acceptances':s['bad_acceptances'],
        },
        'membership31': {
            ...
            'bound_root31_witness_rebind_states':b['bound_root31_witness_rebind_states'],
            'bound_root31_witness_binding_states':b['bound_root31_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 85 by rotating the twenty-eighth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose fifty-eighth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a fifty-ninth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-31 witness rebind, replace the witness source, roll to root 32, bind root 32, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V133 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-84 GC to {t['accepted']:,} states with {t['epoch84_bound_twenty_eighth_source_handoff_states']:,} bound twenty-eighth-source handoffs, "
        f"{t['epoch84_bound_twenty_eighth_source_binding_states']:,} bound twenty-eighth-source bindings, and {t['epoch84_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_fifty_eighth_restart_recoveries']:,} fully bound fifty-eighth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root31_witness_rebind_states']:,} bound root-31 witness rebinds, "
        f"{b['bound_root31_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
