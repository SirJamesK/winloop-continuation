...
BASE_V97_BOUND_TWENTY_SECOND_RESTART_STATIC_STATES = 27648
...
def _pub98_ok(phase, replacement_churn, successor_binding,
              dual, restart23, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        replacement_churn, successor_binding, dual, restart23, consume, quorum
    )):
        return False
    # V97's twenty-second restart is a seed only after successor disappearance,
    # replacement binding, and reconciliation. V98 requires replacement-source
    # churn, successor binding, fresh reconciliation, and restart 23.
    if cache_authority != 0:
        return False
    return _PUB98_EXPECTED.get(phase) == (
        replacement_churn, successor_binding, dual, restart23, consume, quorum
    )


def replacement_churn_twenty_third_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB98_EXPECTED.items()
    ]
    assert all(_pub98_ok(*x) for x in tails)
    z = q(50)
    seed = BASE_V97_BOUND_TWENTY_SECOND_RESTART_STATIC_STATES
    ...
    restart23 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart23 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub98_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub98_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub98_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub98_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub98_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub98_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub98_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub98_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub98_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub98_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub98_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub98_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub98_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_twenty_second_restart_seed_states': seed,
        'delay_vectors': 4 ** 50,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'bound_twenty_third_restart_states': bound_restart23 * z,
        'bound_twenty_third_restart_recoveries': bound_recovery * z,
        'cached_twenty_third_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_twenty_third_restart_acceptances': restart_bad * z,
