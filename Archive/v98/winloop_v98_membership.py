from winloop_v98_core import q

BASE_V97_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem98_ok(phase, witness_source_replacement, replacement_source_binding,
              root14_rollover, root14_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root14_rollover, root14_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V97 leaves a rebound witness current at generation 4 / root 13. V98
    # binds a new witness source, rolls to root 14, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 13 or target_root != 14:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM98_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root14_rollover, root14_binding, quorum_churn
    )


def root14_rollover_after_root13_witness_source_replacement():
    tails = [
        (phase, *state, 4, 13, 14, 2, 1, 2, 2, 0)
        for phase, state in _MEM98_EXPECTED.items()
    ]
    assert all(_mem98_ok(*x) for x in tails)
    z = q(48)
    seed = BASE_V97_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 13 for x in tails) * seed
    target_root_regression = sum(x[8] < 14 for x in tails) * seed
    ...
    checks = [
        _mem98_ok(0, 0, 0, 0, 0, 0, 4, 13, 14, 2, 1, 2, 2, 0),
        _mem98_ok(2, 2, 0, 0, 0, 0, 4, 13, 14, 2, 1, 2, 2, 0),
        _mem98_ok(4, 2, 2, 0, 0, 0, 4, 13, 14, 2, 1, 2, 2, 0),
        _mem98_ok(6, 2, 2, 2, 0, 0, 4, 13, 14, 2, 1, 2, 2, 0),
        _mem98_ok(8, 2, 2, 2, 2, 0, 4, 13, 14, 2, 1, 2, 2, 0),
        _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 3, 2, 2, 2, 2, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 3, 2, 2, 2, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 3, 2, 2, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 3, 2, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 3, 4, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 3, 13, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 12, 14, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 13, 2, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 14, 1, 1, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 14, 2, 2, 2, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 14, 2, 1, 1, 2, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 14, 2, 1, 2, 3, 0),
        not _mem98_ok(10, 2, 2, 2, 2, 2, 4, 13, 14, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 15 * 15 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 48) * z
    return {
        'patterns': patterns,
        ...
        'root14_rollover_states': rollover * z,
        'bound_root14_rollover_states': bound_rollover * z,
        'root14_binding_states': root_binding * z,
        'bound_root14_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root14_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root14_binding_acceptances': root_binding_bad * z,
