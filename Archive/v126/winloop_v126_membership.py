from winloop_v126_core import q

BASE_V125_BOUND_QUORUM_CHURN_STATIC_STATES = 760
...
def _mem126_ok(phase, witness_source_replacement, replacement_source_binding,
               root28_rollover, root28_binding, quorum_churn,
    ...
    if any(x == 3 for x in (
        ...
        root28_rollover, root28_binding, quorum_churn, prior_source_binding
    )):
        return False
    # V125 leaves a rebound witness current at generation 4 / root 27. V126
    # binds a new witness source, rolls to root 28, binds that root, and only
    # then lets a replication-quorum churn complete.
    if generation != 4 or carried_root != 27 or target_root != 28:
        return False
    if replication != 2 or tombstone_binding != 1 or witness_binding != 2:
        return False
    if prior_source_binding != 2 or active_byzantine != 0:
        return False
    return _MEM126_EXPECTED.get(phase) == (
        witness_source_replacement, replacement_source_binding,
        root28_rollover, root28_binding, quorum_churn
    )


def root28_rollover_after_root27_witness_source_replacement():
    tails = [
        (phase, *state, 4, 27, 28, 2, 1, 2, 2, 0)
        for phase, state in _MEM126_EXPECTED.items()
    ]
    assert all(_mem126_ok(*x) for x in tails)
    z = q(104)
    seed = BASE_V125_BOUND_QUORUM_CHURN_STATIC_STATES
    ...
    carried_root_regression = sum(x[7] < 27 for x in tails) * seed
    target_root_regression = sum(x[8] < 28 for x in tails) * seed
    ...
    checks = [
        _mem126_ok(0, 0, 0, 0, 0, 0, 4, 27, 28, 2, 1, 2, 2, 0),
        _mem126_ok(2, 2, 0, 0, 0, 0, 4, 27, 28, 2, 1, 2, 2, 0),
        _mem126_ok(4, 2, 2, 0, 0, 0, 4, 27, 28, 2, 1, 2, 2, 0),
        _mem126_ok(6, 2, 2, 2, 0, 0, 4, 27, 28, 2, 1, 2, 2, 0),
        _mem126_ok(8, 2, 2, 2, 2, 0, 4, 27, 28, 2, 1, 2, 2, 0),
        _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 3, 2, 2, 2, 2, 4, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 3, 2, 2, 2, 4, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 3, 2, 2, 4, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 3, 2, 4, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 3, 4, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 3, 27, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 26, 28, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 27, 2, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 28, 1, 1, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 28, 2, 2, 2, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 28, 2, 1, 1, 2, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 28, 2, 1, 2, 3, 0),
        not _mem126_ok(10, 2, 2, 2, 2, 2, 4, 27, 28, 2, 1, 2, 2, 1),
    ]

    static_patterns = seed * 11 * (4 ** 5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2
    patterns = static_patterns * (4 ** 104) * z
    return {
        'patterns': patterns, 'accepted': static_accepted * z,
        'base_states': static_accepted, 'bound_quorum_churn_seed_states': seed,
        'delay_vectors': 4 ** 104, 'deadline_vectors': z, 'shared_deadline': 3,
        ...
        'root28_rollover_states': rollover * z,
        'bound_root28_rollover_states': bound_rollover * z,
        'root28_binding_states': root_binding * z,
        'bound_root28_binding_states': bound_root_binding * z,
        ...
        'unbound_or_conflicting_root28_rollover_acceptances': rollover_bad * z,
        'unbound_or_conflicting_root28_binding_acceptances': root_binding_bad * z,
