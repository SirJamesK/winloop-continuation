"""WinLoop V145 exact continuation: epoch-96 thirty-fourth-source handoff, seventieth cold restart, and root-37 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V145'
BASE_DIGEST = '4c10b24d11b22884ff40ff4c8f4c8b02452bace93f4fe1ae0d9898631cc3e8cd'
BASE_IMPL_SHA = 'f9e087367cdb8e872549c8b65cd0669de145cae97a802e0574b5526712dc02eb'
D = 3
...
def gc96():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*151+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**147) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch95_complete_seed_states': seed,
        'delay_vectors': 4**147,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch96_bound_thirty_fourth_source_handoff_states': counts[0][1],
        'epoch96_bound_thirty_fourth_source_binding_states': counts[1][1],
        'epoch96_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication70():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**144) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_sixty_ninth_restart_seed_states': seed,
        'delay_vectors': 4**144,
        'deadline_vectors': z,
        ...
        'bound_seventieth_restart_states': counts[3][1],
        'bound_seventieth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership37():
    ...
    def ok(phase, state, generation=4, carried_root=37, target_root=37,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=36),
        not ok(6,expected[6],target_root=36),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**142) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**142,
        'deadline_vectors': z,
        'bound_root37_witness_rebind_states': counts[0][1],
        'bound_root37_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc96(), publication70(), membership37()
    o = {
        'version': V,
        'base': {
            'version':'V144',
        ...
        'epoch96': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch95_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_fourth_source_handoff_states':t['epoch96_bound_thirty_fourth_source_handoff_states'],
            'bound_thirty_fourth_source_binding_states':t['epoch96_bound_thirty_fourth_source_binding_states'],
            'bound_verifier_binding_states':t['epoch96_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication70': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_sixty_ninth_restart_seed_states'],
            ...
            'bound_seventieth_restart_states':s['bound_seventieth_restart_states'],
            'bound_seventieth_restart_recoveries':s['bound_seventieth_restart_recoveries'],
        ...
        'membership37': {
            ...
            'bound_root37_witness_rebind_states':b['bound_root37_witness_rebind_states'],
            'bound_root37_witness_binding_states':b['bound_root37_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 97 by rotating the thirty-fourth-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose seventieth-restart recovery with replacement-source churn, successor binding, fresh reconciliation, and a seventy-first verifier cold restart without cached authority promotion',
            'keep generation 4 after root-37 witness rebind, replace the witness source, roll to root 38, bind root 38, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V145 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-96 GC to {t['accepted']:,} states with {t['epoch96_bound_thirty_fourth_source_handoff_states']:,} bound thirty-fourth-source handoffs, "
        f"{t['epoch96_bound_thirty_fourth_source_binding_states']:,} bound thirty-fourth-source bindings, and {t['epoch96_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_seventieth_restart_recoveries']:,} fully bound seventieth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root37_witness_rebind_states']:,} bound root-37 witness rebinds, "
        f"{b['bound_root37_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
