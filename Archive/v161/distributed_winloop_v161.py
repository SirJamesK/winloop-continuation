"""WinLoop V161 exact continuation: epoch-112 forty-second-source handoff, eighty-sixth cold restart, and root-45 witness rebind."""
from itertools import product
from math import comb
import hashlib, json

V = 'V161'
BASE_DIGEST = 'bfe50fea5241d4d06161ae84b40c7ae64443aed6b1b54d875922b291df242284'
BASE_IMPL_SHA = '0b8f6a2d0bbd3e7ac17f7f2d5cd8033be5ed7554fd35ff9d299485e6f2e35af9'
D = 3
...
def gc112():
    ...
    checks += [
        ...
        not ok(6, expected[6], carried=(2,)*199+(1,)),
    ...
    patterns = seed * 7 * (4**3) * 4 * 3 * (4**carried_count) * 2 * (4**179) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch111_complete_seed_states': seed,
        'delay_vectors': 4**179,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch112_bound_forty_second_source_handoff_states': counts[0][1],
        'epoch112_bound_forty_second_source_binding_states': counts[1][1],
        'epoch112_bound_verifier_binding_states': counts[2][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication86():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**176) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_eighty_fifth_restart_seed_states': seed,
        'delay_vectors': 4**176,
        'deadline_vectors': z,
        ...
        'bound_eighty_sixth_restart_states': counts[3][1],
        'bound_eighty_sixth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership45():
    ...
    def ok(phase, state, generation=4, carried_root=45, target_root=45,
    ...
    checks += [
        ...
        not ok(6,expected[6],carried_root=44),
        not ok(6,expected[6],target_root=44),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 7 * (4**3) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**174) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'delay_vectors': 4**174,
        'deadline_vectors': z,
        'bound_root45_witness_rebind_states': counts[0][1],
        'bound_root45_witness_binding_states': counts[1][1],
...
def run_validation():
    c, t, s, b = indep(), gc112(), publication86(), membership45()
    o = {
        'version': V,
        'base': {
            'version':'V160',
        ...
        'epoch112': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch111_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_forty_second_source_handoff_states':t['epoch112_bound_forty_second_source_handoff_states'],
            'bound_forty_second_source_binding_states':t['epoch112_bound_forty_second_source_binding_states'],
            'bound_verifier_binding_states':t['epoch112_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication86': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_eighty_fifth_restart_seed_states'],
            ...
            'bound_eighty_sixth_restart_states':s['bound_eighty_sixth_restart_states'],
            'bound_eighty_sixth_restart_recoveries':s['bound_eighty_sixth_restart_recoveries'],
        ...
        'membership45': {
            ...
            'bound_root45_witness_rebind_states':b['bound_root45_witness_rebind_states'],
            'bound_root45_witness_binding_states':b['bound_root45_witness_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 113 by rotating the forty-second-source lineage, binding that lineage, rebinding the handed proof, and preserving the epoch-12 deadline',
            'compose eighty-sixth-restart recovery with replacement-source churn, successor-source binding, fresh reconciliation, and an eighty-seventh verifier cold restart without cached authority promotion',
            'keep generation 4 after the root-45 witness rebind, replace the witness source, roll to root 46, bind root 46, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V161 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-112 GC to {t['accepted']:,} states with {t['epoch112_bound_forty_second_source_handoff_states']:,} bound forty-second-source handoffs, "
        f"{t['epoch112_bound_forty_second_source_binding_states']:,} bound forty-second-source bindings, and {t['epoch112_bound_verifier_binding_states']:,} bound verifier completions; "
        f"admits {s['accepted']:,} publication states with {s['bound_eighty_sixth_restart_recoveries']:,} fully bound eighty-sixth-cold-restart recoveries; "
        f"and admits {b['accepted']:,} membership states with {b['bound_root45_witness_rebind_states']:,} bound root-45 witness rebinds, "
        f"{b['bound_root45_witness_binding_states']:,} bound witness renewals, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
