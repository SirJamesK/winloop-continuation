"""WinLoop V154 exact continuation: epoch-105 thirty-eighth-lineage rotation, seventy-ninth cold restart, and root-42 rollover."""
from itertools import product
from math import comb
import hashlib, json

V = 'V154'
BASE_DIGEST = '34b2597dfdc52db53fc6d8af5451852420af1c78150edb28159df60eec7d9610'
BASE_IMPL_SHA = '18e20967e7550d18d659ac7263e9f45d32b3f1d9293796ae2a98f2098d474835'
D = 3
...
def gc105():
    ...
    checks += [
        ...
        not ok(8, expected[8], carried=(2,)*178+(1,)),
    ...
    patterns = seed * 9 * (4**4) * 4 * 3 * (4**carried_count) * 2 * (4**165) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'epoch104_complete_seed_states': seed,
        'delay_vectors': 4**165,
        'deadline_vectors': z,
        'deadline_origin': 'epoch12',
        'epoch105_bound_thirty_eighth_lineage_rotation_states': counts[0][1],
        'epoch105_bound_thirty_eighth_lineage_binding_states': counts[1][1],
        'epoch105_bound_handed_proof_rebind_states': counts[2][1],
        'epoch105_bound_verifier_binding_states': counts[3][1],
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def publication79():
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**6) * 2 * (4**162) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        'bound_seventy_eighth_restart_seed_states': seed,
        'delay_vectors': 4**162,
        'deadline_vectors': z,
        ...
        'bound_seventy_ninth_restart_states': counts[3][1],
        'bound_seventy_ninth_restart_recoveries': seed * z,
        'bad_acceptances': 0,
        'checks': checks,
    }
...
def membership42():
    ...
    def ok(phase, state, generation=4, carried_root=41, target_root=42,
    ...
    checks += [
        ...
        not ok(10,expected[10],carried_root=40),
        not ok(10,expected[10],target_root=41),
    ...
    counts = _stage_counts(expected, seed, z)
    accepted = len(expected) * seed * z
    patterns = seed * 11 * (4**5) * 6 * 16 * 16 * 4 * 3 * 4 * 4 * 2 * (4**160) * z
    return {
        'patterns': patterns,
        'accepted': accepted,
        ...
        'bound_root42_rollover_states': counts[2][1],
        'bound_root42_binding_states': counts[3][1],
...
def run_validation():
    c, t, s, b = indep(), gc105(), publication79(), membership42()
    o = {
        'version': V,
        'base': {
            'version':'V153',
        ...
        'epoch105': {
            'patterns':t['patterns'],
            'accepted':t['accepted'],
            'seed_states':t['epoch104_complete_seed_states'],
            'delay_vectors':t['delay_vectors'],
            'deadline_vectors':t['deadline_vectors'],
            'deadline_origin':t['deadline_origin'],
            'bound_thirty_eighth_lineage_rotation_states':t['epoch105_bound_thirty_eighth_lineage_rotation_states'],
            'bound_thirty_eighth_lineage_binding_states':t['epoch105_bound_thirty_eighth_lineage_binding_states'],
            'bound_handed_proof_rebind_states':t['epoch105_bound_handed_proof_rebind_states'],
            'bound_verifier_binding_states':t['epoch105_bound_verifier_binding_states'],
            'bad_acceptances':t['bad_acceptances'],
        },
        'publication79': {
            'patterns':s['patterns'],
            'accepted':s['accepted'],
            'seed_states':s['bound_seventy_eighth_restart_seed_states'],
            ...
            'bound_seventy_ninth_restart_states':s['bound_seventy_ninth_restart_states'],
            'bound_seventy_ninth_restart_recoveries':s['bound_seventy_ninth_restart_recoveries'],
        ...
        'membership42': {
            ...
            'bound_root42_rollover_states':b['bound_root42_rollover_states'],
            'bound_root42_binding_states':b['bound_root42_binding_states'],
        ...
        'next': [
            ...
            'extend anchor GC through epoch 106 by handing the rebound proof to a thirty-ninth source, binding that source, and preserving the epoch-12 deadline',
            'compose seventy-ninth-restart recovery with successor-source disappearance, replacement-source binding, fresh reconciliation, and an eightieth verifier cold restart without cached authority promotion',
            'keep generation 4 after root-42 rollover, rebind the witness to root 42, renew the witness binding, and require replication-quorum churn without tombstone or prior-source discontinuity',
            'retain V21 routing until the >=2000-seed replacement bar clears',
        ],
    }
    o['headline'] = (
        f"V154 keeps cross-role credit at {c['conservative_cross_role_credit']} with no committed external independence certificate, "
        f"extends epoch-105 GC to {t['accepted']:,} states with {t['epoch105_bound_thirty_eighth_lineage_rotation_states']:,} bound thirty-eighth-lineage rotations, "
        f"{t['epoch105_bound_thirty_eighth_lineage_binding_states']:,} bound lineage bindings, {t['epoch105_bound_handed_proof_rebind_states']:,} bound handed-proof rebinds, "
        f"and {t['epoch105_bound_verifier_binding_states']:,} bound verifier completions; admits {s['accepted']:,} publication states with "
        f"{s['bound_seventy_ninth_restart_recoveries']:,} fully bound seventy-ninth-cold-restart recoveries; and admits {b['accepted']:,} membership states with "
        f"{b['bound_witness_source_replacement_states']:,} bound witness-source replacements, {b['bound_root42_rollover_states']:,} bound root-42 rollovers, "
        f"{b['bound_root42_binding_states']:,} bound root-42 bindings, and {b['bound_replication_quorum_churn_states']:,} bound quorum-churn completions, "
