...
D = 3
BASE_V92_EPOCH43_COMPLETE_STATIC_STATES = 576
...
def _gc44_ok(phase, eighth_source_handoff, eighth_source_binding,
             ...
             sixth_lineage_binding, prior_handed_proof_rebind,
             seventh_source_handoff, seventh_source_binding,
             seventh_lineage_binding, handed_proof_rebind, deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    if any(x == 3 for x in (
        eighth_source_handoff, eighth_source_binding, verifier_binding,
        ...
        sixth_lineage_binding, prior_handed_proof_rebind,
        seventh_source_handoff, seventh_source_binding,
        seventh_lineage_binding, handed_proof_rebind,
    )):
        return False
    if deadline_reset != 0:
        return False
    # V92 completed epoch 43 only after seventh-lineage binding and a fresh
    # handed-proof rebind. Epoch 44 hands that rebound proof to an eighth
    # source, binds the new source, and only then permits verifier completion.
    ...
    carried = (
        ...
        prior_handed_proof_rebind, seventh_source_handoff, seventh_source_binding,
        seventh_lineage_binding, handed_proof_rebind,
    )
    if any(x != 2 for x in carried):
        return False
    return _GC44_EXPECTED.get(phase) == (
        eighth_source_handoff, eighth_source_binding, verifier_binding
    )
...
def gc44():
    fixed = (2, 1, *([2] * 17), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC44_EXPECTED.items()]
    assert all(_gc44_ok(*x) for x in tails)
    z = q(43)
    seed = BASE_V92_EPOCH43_COMPLETE_STATIC_STATES
    static_accepted = seed * len(tails)

    handoff = sum(x[1] in (1, 2) for x in tails) * seed
    bound_handoff = sum(x[1] == 2 for x in tails) * seed
    eighth_binding = sum(x[2] in (1, 2) for x in tails) * seed
    bound_eighth_binding = sum(x[2] == 2 for x in tails) * seed
    ...
    eighth_binding_bad = sum(x[2] == 3 for x in tails) * seed
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(6, 23)]
    deadline_bad = sum(x[23] != 0 for x in tails) * seed
    bad = handoff_bad + eighth_binding_bad + verifier_bad + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc44_ok(0, 0, 0, 0, *fixed),
        _gc44_ok(2, 2, 0, 0, *fixed),
        _gc44_ok(4, 2, 2, 0, *fixed),
        _gc44_ok(6, 2, 2, 2, *fixed),
        not _gc44_ok(6, 3, 2, 2, *fixed),
        not _gc44_ok(6, 2, 3, 2, *fixed),
        not _gc44_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:  # continuity must be exactly 1
            ...
            broken[i] = 1
        checks.append(not _gc44_ok(6, 2, 2, 2, *broken))

    # Phase, three epoch-44 transitions, carried root, continuity, seventeen
    # carried source/key/lineage/proof/handoff bindings, and deadline-reset bit.
    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** 17) * 2
    patterns = static_patterns * (4 ** 43) * z
    names = (
        'source_binding', 'rotated_key_binding', 'third_source_binding',
        'fourth_source_binding', 'fourth_lineage_binding', 'fourth_proof_binding',
        'fifth_source_binding', 'fifth_lineage_binding', 'fifth_proof_binding',
        'sixth_source_binding', 'sixth_handoff_binding', 'sixth_lineage_binding',
        'prior_handed_proof_rebind', 'seventh_source_handoff',
    ...
    out = {
        ...
        'epoch43_complete_seed_states': seed,
        'delay_vectors': 4 ** 43,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch44_eighth_source_handoff_states': handoff * z,
        'epoch44_bound_eighth_source_handoff_states': bound_handoff * z,
        'epoch44_eighth_source_binding_states': eighth_binding * z,
        'epoch44_bound_eighth_source_binding_states': bound_eighth_binding * z,
        'epoch44_verifier_binding_states': verifier * z,
        'epoch44_bound_verifier_binding_states': bound_verifier * z,
        'epoch44_complete_states': complete * z,
        'unbound_or_conflicting_eighth_source_handoff_acceptances': handoff_bad * z,
        'unbound_or_conflicting_eighth_source_binding_acceptances': eighth_binding_bad * z,
