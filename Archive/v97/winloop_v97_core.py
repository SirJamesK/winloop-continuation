...
D = 3
BASE_V96_EPOCH47_COMPLETE_STATIC_STATES = 576
...
def _gc48_ok(phase, tenth_source_handoff, tenth_source_binding,
             ...
             handed_proof_rebind, deadline_reset):
    # 3 denotes unknown/unbound/conflicting trust-bearing evidence.
    if any(x == 3 for x in (
        tenth_source_handoff, tenth_source_binding, verifier_binding,
    ...
    if deadline_reset != 0:
        return False
    # V96 completed epoch 47 only after ninth-lineage rotation/binding and a
    # fresh handed-proof rebind. Epoch 48 hands that proof to a tenth source,
    ...
    if any(x != 2 for x in carried):
        return False
    return _GC48_EXPECTED.get(phase) == (
        tenth_source_handoff, tenth_source_binding, verifier_binding
    )
...
def gc48():
    fixed = (2, 1, *([2] * 26), 0)
    tails = [(phase, *state, *fixed) for phase, state in _GC48_EXPECTED.items()]
    assert all(_gc48_ok(*x) for x in tails)
    z = q(51)
    seed = BASE_V96_EPOCH47_COMPLETE_STATIC_STATES
    static_accepted = seed * len(tails)

    handoff = sum(x[1] in (1, 2) for x in tails) * seed
    bound_handoff = sum(x[1] == 2 for x in tails) * seed
    tenth_binding = sum(x[2] in (1, 2) for x in tails) * seed
    bound_tenth_binding = sum(x[2] == 2 for x in tails) * seed
    ...
    tenth_binding_bad = sum(x[2] == 3 for x in tails) * seed
    ...
    carried_bad = [sum(x[i] != 2 for x in tails) * seed for i in range(6, 32)]
    deadline_bad = sum(x[32] != 0 for x in tails) * seed
    bad = handoff_bad + tenth_binding_bad + verifier_bad + root_bad + continuity_break + sum(carried_bad) + deadline_bad

    checks = [
        _gc48_ok(0, 0, 0, 0, *fixed),
        _gc48_ok(2, 2, 0, 0, *fixed),
        _gc48_ok(4, 2, 2, 0, *fixed),
        _gc48_ok(6, 2, 2, 2, *fixed),
        not _gc48_ok(6, 3, 2, 2, *fixed),
        not _gc48_ok(6, 2, 3, 2, *fixed),
        not _gc48_ok(6, 2, 2, 3, *fixed),
    ]
    for i in range(len(fixed)):
        ...
        if i == 1:
            ...
            broken[i] = 1
        checks.append(not _gc48_ok(6, 2, 2, 2, *broken))

    static_patterns = seed * 7 * (4 ** 3) * 4 * 3 * (4 ** 26) * 2
    patterns = static_patterns * (4 ** 51) * z
    names = (
        'source_binding', 'rotated_key_binding', 'third_source_binding',
        'fourth_source_binding', 'fourth_lineage_binding', 'fourth_proof_binding',
        'fifth_source_binding', 'fifth_lineage_binding', 'fifth_proof_binding',
    ...
    out = {
        ...
        'epoch47_complete_seed_states': seed,
        'delay_vectors': 4 ** 51,
        'deadline_vectors': z,
        'shared_deadline': 3,
        'deadline_origin': 'epoch12',
        'epoch48_tenth_source_handoff_states': handoff * z,
        'epoch48_bound_tenth_source_handoff_states': bound_handoff * z,
        'epoch48_tenth_source_binding_states': tenth_binding * z,
        'epoch48_bound_tenth_source_binding_states': bound_tenth_binding * z,
        'epoch48_verifier_binding_states': verifier * z,
        'epoch48_bound_verifier_binding_states': bound_verifier * z,
        'epoch48_complete_states': complete * z,
        'unbound_or_conflicting_tenth_source_handoff_acceptances': handoff_bad * z,
        'unbound_or_conflicting_tenth_source_binding_acceptances': tenth_binding_bad * z,
