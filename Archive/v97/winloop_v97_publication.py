...
BASE_V96_BOUND_TWENTY_FIRST_RESTART_STATIC_STATES = 27648
...
def _pub97_ok(phase, successor_disappearance, replacement_binding,
              dual, restart22, cache_authority, consume, quorum):
    if any(x == 3 for x in (
        successor_disappearance, replacement_binding, dual, restart22,
        consume, quorum
    )):
        return False
    # V96's twenty-first restart is a seed only after replacement-source churn,
    # successor binding, and reconciliation. V97 then handles successor-source
    # disappearance, replacement binding, fresh reconciliation, and restart 22.
    if cache_authority != 0:
        return False
    return _PUB97_EXPECTED.get(phase) == (
        successor_disappearance, replacement_binding, dual, restart22,
        consume, quorum
    )


def successor_disappearance_twenty_second_restart():
    tails = [
        (phase, state[0], state[1], state[2], state[3], 0, state[4], state[5])
        for phase, state in _PUB97_EXPECTED.items()
    ]
    assert all(_pub97_ok(*x) for x in tails)
    z = q(48)
    seed = BASE_V96_BOUND_TWENTY_FIRST_RESTART_STATIC_STATES
    ...
    restart22 = sum(x[4] in (1, 2) for x in tails) * seed
    bound_restart22 = sum(x[4] == 2 for x in tails) * seed
    ...
    checks = [
        _pub97_ok(0, 0, 0, 0, 0, 0, 0, 0),
        _pub97_ok(2, 2, 0, 0, 0, 0, 0, 0),
        _pub97_ok(4, 2, 2, 0, 0, 0, 0, 0),
        _pub97_ok(6, 2, 2, 2, 0, 0, 0, 0),
        _pub97_ok(8, 2, 2, 2, 2, 0, 0, 0),
        _pub97_ok(10, 2, 2, 2, 2, 0, 2, 2),
        not _pub97_ok(10, 3, 2, 2, 2, 0, 2, 2),
        not _pub97_ok(10, 2, 3, 2, 2, 0, 2, 2),
        not _pub97_ok(10, 2, 2, 3, 2, 0, 2, 2),
        not _pub97_ok(10, 2, 2, 2, 3, 0, 2, 2),
        not _pub97_ok(10, 2, 2, 2, 2, 1, 2, 2),
        not _pub97_ok(10, 2, 2, 2, 2, 0, 3, 2),
        not _pub97_ok(10, 2, 2, 2, 2, 0, 2, 1),
    ]
    ...
    return {
        ...
        'bound_twenty_first_restart_seed_states': seed,
        'delay_vectors': 4 ** 48,
        'deadline_vectors': z,
        'shared_deadline': 3,
        ...
        'twenty_second_verifier_cold_restart_states': restart22 * z,
        'bound_twenty_second_restart_states': bound_restart22 * z,
        'bound_twenty_second_restart_recoveries': bound_recovery * z,
        'cached_twenty_second_restart_authority_acceptances': cached * z,
        ...
        'unbound_or_conflicting_twenty_second_restart_acceptances': restart_bad * z,
